---
name: node-connect
description: Diagnose Alvasta Pro node connection and pairing failures for Android, iOS, and macOS companion apps. Use when QR/setup code/manual connect fails, local Wi-Fi works but VPS/tailnet does not, or errors mention pairing required, unauthorized, bootstrap token invalid or expired, gateway.bind, gateway.remote.url, Tailscale, or plugins.entries.device-pair.config.publicUrl.
---

# Node Connect

Goal: find the one real route from node -> gateway, verify Alvasta Pro is advertising that route, then fix pairing/auth.
...
- whether `alvasta-pro devices list` shows a pending pairing request

Do not guess from `can't connect`.

## Canonical checks

Prefer `alvasta-pro qr --json`. It uses the same setup-code payload Android scans.

```bash
alvasta-pro config get gateway.mode
alvasta-pro config get gateway.bind
alvasta-pro config get gateway.tailscale.mode
alvasta-pro config get gateway.remote.url
alvasta-pro config get gateway.auth.mode
alvasta-pro config get gateway.auth.allowTailscale
alvasta-pro config get plugins.entries.device-pair.config.publicUrl
alvasta-pro qr --json
alvasta-pro devices list
alvasta-pro nodes status
```

If this Alvasta Pro instance is pointed at a remote gateway, also run:

```bash
alvasta-pro qr --remote --json
```
...
`alvasta-pro qr --json` success means:
...
If `alvasta-pro qr --json` says `Gateway is only bound to loopback`:
...
- `alvasta-pro devices list` shows pending requests: stop changing network config and approve first.
...
- `The gateway is still loopback-only, so a node on another network can never reach it. Enable Tailscale Serve, restart the gateway, run alvasta-pro qr again, rescan, then approve the pending device pairing.`
