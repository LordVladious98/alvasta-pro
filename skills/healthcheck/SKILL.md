---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for Alvasta Pro deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, Alvasta Pro cron scheduling for periodic checks, or version status checks on a machine running Alvasta Pro (laptop, workstation, Pi, VPS).
---

# Alvasta Pro Host Hardening

## Overview

Assess and harden the host running Alvasta Pro, then align it to a user-defined risk tolerance without breaking access. Use Alvasta Pro security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim Alvasta Pro changes the host firewall, SSH, or OS updates; it does not.
...
5. Alvasta Pro gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if Alvasta Pro can access sensitive data.
...
### 2) Run Alvasta Pro security audits (read-only)

As part of the default read-only checks, run `alvasta-pro security audit --deep`. Only offer alternatives if the user requests them:

1. `alvasta-pro security audit` (faster, non-probing)
2. `alvasta-pro security audit --json` (structured output)

Offer to apply Alvasta Pro safe defaults (numbered):

1. `alvasta-pro security audit --fix`

Be explicit that `--fix` only tightens Alvasta Pro defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check Alvasta Pro version/update status (read-only)

As part of the default read-only checks, run `alvasta-pro update status`.
...
- Credential hygiene notes (location of Alvasta Pro creds, prefer disk encryption)
...
- Alvasta Pro security audit (re-run)
...
After Alvasta Pro install or first hardening pass, run at least one baseline audit and version check:

- `alvasta-pro security audit`
- `alvasta-pro security audit --deep`
- `alvasta-pro update status`

Ongoing monitoring is recommended. Use the Alvasta Pro cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `alvasta-pro cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `alvasta-pro update status`
...
Before creating, `alvasta-pro cron list` and match on exact `name`. If found, `alvasta-pro cron edit <id> ...`.
If not found, `alvasta-pro cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `alvasta-pro update status` (preferred for source checkouts and channels)
2. `npm view alvasta-pro version` (published npm version)

## Alvasta Pro command accuracy

Use only supported commands and flags:

- `alvasta-pro security audit [--deep] [--fix] [--json]`
- `alvasta-pro status` / `alvasta-pro status --deep`
- `alvasta-pro health --json`
- `alvasta-pro update status`
- `alvasta-pro cron add|list|runs|run`

Do not invent CLI flags or imply Alvasta Pro enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by Alvasta Pro compaction:
