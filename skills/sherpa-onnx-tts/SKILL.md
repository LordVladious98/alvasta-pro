...
1. Download the runtime for your OS (extracts into `$OPENCLAW_STATE_DIR/tools/sherpa-onnx-tts/runtime`, default `~/.alvasta-pro/tools/sherpa-onnx-tts/runtime`)
2. Download a voice model (extracts into `$OPENCLAW_STATE_DIR/tools/sherpa-onnx-tts/models`, default `~/.alvasta-pro/tools/sherpa-onnx-tts/models`)

Resolve the active state directory first:

```bash
STATE_DIR="${OPENCLAW_STATE_DIR:-$HOME/.alvasta-pro}"
```

Then write those resolved paths into the active Alvasta Pro config file (`$OPENCLAW_CONFIG_PATH`, default `~/.alvasta-pro/alvasta-pro.json`):
