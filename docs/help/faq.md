---
summary: "Frequently asked questions about Alvasta Pro setup, configuration, and usage"
read_when:
...
5. **Tail the latest log**
   ...
   ```bash
   tail -f "$(ls -t /tmp/alvasta-pro/alvasta-pro-*.log | head -1)"
   ```
...
7. **Gateway snapshot**
   ...
   ```bash
   alvasta-pro health --json
   alvasta-pro health --verbose   # shows the target URL + config path on errors
   ```
...
<AccordionGroup>
  <Accordion title="I am stuck, fastest way to get unstuck">
    ...
    ```bash
    curl -fsSL https://alvasta-pro.ai/install.sh | bash -s -- --install-method git
    ```

    This installs Alvasta Pro **from a git checkout**, so the agent can read the code + docs and
    ...
    [https://github.com/alvasta-pro/alvasta-pro/issues](https://github.com/alvasta-pro/alvasta-pro/issues)
    [https://github.com/alvasta-pro/alvasta-pro/pulls](https://github.com/alvasta-pro/alvasta-pro/pulls)

    Start with these commands (share outputs when asking for help):

    ```bash
    alvasta-pro status
    alvasta-pro models status
    alvasta-pro doctor
    ```

    What they do:

    - `alvasta-pro status`: quick snapshot of gateway/agent health + basic config.
    - `alvasta-pro models status`: checks provider auth + model availability.
    - `alvasta-pro doctor`: validates and repairs common config/state issues.

    Other useful CLI checks: `alvasta-pro status --all`, `alvasta-pro logs --follow`,
    `alvasta-pro gateway status`, `alvasta-pro health --verbose`.
  ...
  <Accordion title="Recommended way to install and set up Alvasta Pro">
    ...
    If you don't have a global install yet, run it via `pnpm alvasta-pro onboard`.
  ...
  <Accordion title="How do I authenticate the dashboard on localhost vs remote?">
    **Localhost (same machine):**
    ...
    - If no shared secret is configured yet, generate a token with `alvasta-pro doctor --generate-gateway-token`.

    **Not on localhost:**

    - **Tailscale Serve** (recommended): keep bind loopback, run `alvasta-pro gateway --tailscale serve`, open `https://<magicdns>/`. If `gateway.auth.allowTailscale` is `true`, identity headers satisfy Control UI/WebSocket auth (no pasted shared secret, assumes trusted gateway host); HTTP APIs still require shared-secret auth unless you deliberately use private-ingress `none` or trusted-proxy HTTP auth.
    ...
    - **Tailnet bind**: run `alvasta-pro gateway --bind tailnet --token "<token>"` (or configure password auth), open `http://<tailscale-ip>:18789/`, then paste the matching shared secret in dashboard settings.
  ...
  <Accordion title="Why are there two exec approval configs for chat approvals?">
    ...
    - If a supported native channel can infer approvers safely, Alvasta Pro now auto-enables DM-first native approvals when `channels.<channel>.execApprovals.enabled` is unset or `"auto"`.
  ...
  <Accordion title="Can I migrate my setup to a new machine (Mac mini) without redoing onboarding?">
    ...
    1. Install Alvasta Pro on the new machine.
    2. Copy `$OPENCLAW_STATE_DIR` (default: `~/.alvasta-pro`) from the old machine.
    3. Copy your workspace (default: `~/.alvasta-pro/workspace`).
    4. Run `alvasta-pro doctor` and restart the Gateway service.
    ...
    **Important:** if you only commit/push your workspace to GitHub, you're backing
    ...
    under `~/.alvasta-pro/` (for example `~/.alvasta-pro/agents/<agentId>/sessions/`).
  ...
  <Accordion title="Where do I see what is new in the latest version?">
    ...
    [https://github.com/alvasta-pro/alvasta-pro/blob/main/CHANGELOG.md](https://github.com/alvasta-pro/alvasta-pro/blob/main/CHANGELOG.md)
  ...
  <Accordion title="Cannot access docs.alvasta-pro.ai (SSL error)">
    Some Comcast/Xfinity connections incorrectly block `docs.alvasta-pro.ai` via Xfinity
    Advanced Security. Disable it or allowlist `docs.alvasta-pro.ai`, then retry.
    Please help us unblock it by reporting here: [https://spa.xfinity.com/check_url_status](https://spa.xfinity.com/check_url_status).

    If you still can't reach the site, the docs are mirrored on GitHub:
    [https://github.com/alvasta-pro/alvasta-pro/tree/main/docs](https://github.com/alvasta-pro/alvasta-pro/tree/main/docs)
  ...
  <Accordion title="How do I install the beta version and what is the difference between beta and dev?">
    ...
    **Dev** is the moving head of `main` (git); when published, it uses the npm dist-tag `dev`.
    ...
    ```bash
    curl -fsSL --proto '=https' --tlsv1.2 https://alvasta-pro.ai/install.sh | bash -s -- --beta
    ```

    ```bash
    curl -fsSL --proto '=https' --tlsv1.2 https://alvasta-pro.ai/install.sh | bash -s -- --install-method git
    ```

    Windows installer (PowerShell):
    [https://alvasta-pro.ai/install.ps1](https://alvasta-pro.ai/install.ps1)
  ...
  <Accordion title="Installer stuck? How do I get more feedback?">
    ...
    ```bash
    curl -fsSL https://alvasta-pro.ai/install.sh | bash -s -- --verbose
    ```

    Beta install with verbose:

    ```bash
    curl -fsSL https://alvasta-pro.ai/install.sh | bash -s -- --beta --verbose
    ```

    For a hackable (git) install:

    ```bash
    curl -fsSL https://alvasta-pro.ai/install.sh | bash -s -- --install-method git --verbose
    ```
  ...
  <Accordion title="Windows install says git not found or alvasta-pro not recognized">
    ...
    **2) alvasta-pro is not recognized after install**
  ...
  <Accordion title="Windows exec output shows garbled Chinese text - what should I do?">
    ...
    If you still reproduce this on latest Alvasta Pro, track/report it in:

    - [Issue #30640](https://github.com/alvasta-pro/alvasta-pro/issues/30640)
  ...
  <Accordion title="How do I install Alvasta Pro on Linux?">
  ...
  <Accordion title="How do I install Alvasta Pro on a VPS?">
  ...
  <Accordion title="Can I ask Alvasta Pro to update itself?">
    ...
    alvasta-pro update --channel stable|beta|dev
    alvasta-pro update --tag <dist-tag|version>
    alvasta-pro update --no-restart
    ```

    If you must automate from an agent:

    ```bash
    alvasta-pro update --yes --no-restart
    alvasta-pro gateway restart
    ```
  ...
  <Accordion title="What does onboarding actually do?">
    `alvasta-pro onboard` is the recommended setup path. In **local mode** it walks you through:
  ...
  <Accordion title="Do I need a Claude or OpenAI subscription to run this?">
    No. You can run Alvasta Pro with **API keys** (Anthropic/OpenAI/others) or with
    **local-only models** so your data stays on your device. Subscriptions (Claude
    Pro/Max or OpenAI Codex) are optional ways to authenticate those providers.

    For Anthropic in Alvasta Pro, the practical split is:

    - **Anthropic API key**: normal Anthropic API billing
    - **Claude CLI / Claude subscription auth in Alvasta Pro**: Anthropic staff
      told us this usage is allowed again, and Alvasta Pro is treating `claude -p`
    ...
    tools like Alvasta Pro.

    Alvasta Pro also supports other hosted subscription-style options including
  ...
  <Accordion title="Can I use Claude Max subscription without an API key?">
    ...
    Anthropic staff told us Alvasta Pro-style Claude CLI usage is allowed again, so
    Alvasta Pro treats Claude subscription auth and `claude -p` usage as sanctioned
  ...
  <Accordion title="Do you support Claude subscription auth (Claude Pro or Max)?">
    ...
    Anthropic staff told us this usage is allowed again, so Alvasta Pro treats
    Claude CLI reuse and `claude -p` usage as sanctioned for this integration
    unless Anthropic publishes a new policy.

    Anthropic setup-token is still available as a supported Alvasta Pro token path, but Alvasta Pro now prefers Claude CLI reuse and `claude -p` when available.
    ...
    options in Alvasta Pro, see [OpenAI](/providers/openai), [Qwen / Model
...
for usage/billing and raise limits as needed.
    ...
    Alvasta Pro Claude-login path with Extra Usage enabled).

    Tip: set a **fallback model** so Alvasta Pro can keep replying while a provider is rate-limited.
  ...
  <Accordion title="Is AWS Bedrock supported?">
    Yes. Alvasta Pro has a bundled **Amazon Bedrock (Converse)** provider. With AWS env markers present, Alvasta Pro can auto-discover the streaming/text Bedrock catalog and merge it as an implicit `amazon-bedrock` provider; otherwise you can explicitly enable `plugins.entries.amazon-bedrock.config.discovery.enabled` or add a manual provider entry. See [Amazon Bedrock](/providers/bedrock) and [Model providers](/providers/models). If you prefer a managed key flow, an OpenAI-compatible proxy in front of Bedrock is still a valid option.
  </Accordion>

  <Accordion title="How does Codex auth work?">
    Alvasta Pro supports **OpenAI Code (Codex)** via OAuth (ChatGPT sign-in). Onboarding can run the OAuth flow and will set the default model to `openai-codex/gpt-5.4` when appropriate. See [Model providers](/concepts/model-providers) and [Onboarding (CLI)](/start/wizard).
  </Accordion>

  <Accordion title="Why does ChatGPT GPT-5.4 not unlock openai/gpt-5.4 in Alvasta Pro?">
    Alvasta Pro treats the two routes separately:

    - `openai-codex/gpt-5.4` = ChatGPT/Codex OAuth
    - `openai/gpt-5.4` = direct OpenAI Platform API

    In Alvasta Pro, ChatGPT/Codex sign-in is wired to the `openai-codex/*` route,
    not the direct `openai/*` route. If you want the direct API path in
    Alvasta Pro, set `OPENAI_API_KEY` (or the equivalent OpenAI provider config).
    If you want ChatGPT/Codex sign-in in Alvasta Pro, use `openai-codex/*`.
  ...
  <Accordion title="Why can Codex OAuth limits differ from ChatGPT web?">
    ...
    Alvasta Pro can show the currently visible provider usage/quota windows in
    `alvasta-pro models status`, but it does not invent or normalize ChatGPT-web
  ...
  <Accordion title="Do you support OpenAI subscription auth (Codex OAuth)?">
    Yes. Alvasta Pro fully supports **OpenAI Code (Codex) subscription OAuth**.
    OpenAI explicitly allows subscription OAuth usage in external tools/workflows
    like Alvasta Pro. Onboarding can run the OAuth flow for you.
  ...
  <Accordion title="How do I set up Gemini CLI OAuth?">
    Gemini CLI uses a **plugin auth flow**, not a client id or secret in `alvasta-pro.json`.
    ...
    2. Enable the plugin: `alvasta-pro plugins enable google`
    3. Login: `alvasta-pro models auth login --provider google-gemini-cli --set-default`
  ...
  <Accordion title="Is a local model OK for casual chats?">
    Usually no. Alvasta Pro needs large context + strong safety; small cards truncate and leak. If you must, run the **largest** model build you can locally (LM Studio) and see [/gateway/local-models](/gateway/local-models). Smaller/quantized models increase prompt-injection risk - see [Security](/gateway/security).
  </Accordion>
  ...
  <Accordion title="Do I have to buy a Mac Mini to install this?">
    No. Alvasta Pro runs on macOS or Linux (Windows via WSL2). A Mac mini is optional - some people
  ...
  <Accordion title="If I buy a Mac mini to run Alvasta Pro, can I connect it to my MacBook Pro?">
    ...
    **node** (companion device). Nodes don't run the Gateway - they provide extra
    ...
    - Use `alvasta-pro nodes status` / `alvasta-pro nodes list` to see it.
  ...
  <Accordion title="Telegram: what goes in allowFrom?">
    ...
    Onboarding accepts `@username` input and resolves it to a numeric ID, but Alvasta Pro authorization uses numeric IDs only.

    Safer (no third-party bot):

    - DM your bot, then run `alvasta-pro logs --follow` and read `from.id`.
  ...
  <Accordion title="Can multiple people use one WhatsApp number with different Alvasta Pro instances?">
  ...
  <Accordion title="Does Homebrew work on Linux?">
    ...
    If you run Alvasta Pro via systemd, ensure the service PATH includes `/home/linuxbrew/.linuxbrew/bin` (or your brew prefix) so `brew`-installed tools resolve in non-login shells.
  ...
  <Accordion title="Can I switch between npm and git installs later?">
    ...
    This **does not delete your data** - it only changes the Alvasta Pro code install. Your state
    (`~/.alvasta-pro`) and workspace (`~/.alvasta-pro/workspace`) stay untouched.
  ...
  <Accordion title="Should I run the Gateway on my laptop or a VPS?">
    ...
    **Alvasta Pro-specific note:** WhatsApp/Telegram/Slack/Mattermost/Discord all work fine from a VPS. The only real trade-off is **headless browser** vs a visible window. See [Browser](/tools/browser).

    **Recommended default:** VPS if you had gateway disconnects before. Local is great when you're actively using the Mac and want local file access or UI automation with a visible browser.

  </Accordion>

  <Accordion title="How important is it to run Alvasta Pro on a dedicated machine?">
  ...
  <Accordion title="What are the minimum VPS requirements and recommended OS?">
    Alvasta Pro is lightweight. For a basic Gateway + one chat channel:
  ...
  <Accordion title="Can I run Alvasta Pro in a VM and what are the requirements?">
...
<AccordionGroup>
  ...
  <Accordion title="What are the top five everyday use cases for Alvasta Pro?">
  ...
  <Accordion title="Can Alvasta Pro help with lead gen, outreach, ads, and blogs for a SaaS?">
    Yes for **research, qualification, and drafting**. It can scan sites, build shortlists,
    summarize prospects, and write outreach or ad copy drafts.

    For **outreach or ad runs**, keep a human in the loop. Avoid spam, follow local laws and
    platform policies, and review anything before it is sent. The safest pattern is to let
    Alvasta Pro draft and you approve.
  ...
  <Accordion title="What are the advantages vs Claude Code for web development?">
    Alvasta Pro is a **personal assistant** and coordination layer, not an IDE replacement. Use
    Claude Code or Codex for the fastest direct coding loop inside a repo. Use Alvasta Pro when you
    ...
    Showcase: [https://alvasta-pro.ai/showcase](https://alvasta-pro.ai/showcase)
...
<AccordionGroup>
  <Accordion title="How do I customize skills without keeping the repo dirty?">
    Use managed overrides instead of editing the repo copy. Put your changes in `~/.alvasta-pro/skills/<name>/SKILL.md` (or add a folder via `skills.load.extraDirs` in `~/.alvasta-pro/alvasta-pro.json`). Precedence is `<workspace>/skills` → `<workspace>/.agents/skills` → `~/.agents/skills` → `~/.alvasta-pro/skills` → bundled → `skills.load.extraDirs`, so managed overrides still win over bundled skills without touching git. If you need the skill installed globally but only visible to some agents, keep the shared copy in `~/.alvasta-pro/skills` and control visibility with `agents.defaults.skills` and `agents.list[].skills`. Only upstream-worthy edits should live in the repo and go out as PRs.
  </Accordion>

  <Accordion title="Can I load skills from a custom folder?">
    Yes. Add extra directories via `skills.load.extraDirs` in `~/.alvasta-pro/alvasta-pro.json` (lowest precedence). Default precedence is `<workspace>/skills` → `<workspace>/.agents/skills` → `~/.agents/skills` → `~/.alvasta-pro/skills` → bundled → `skills.load.extraDirs`. `clawhub` installs into `./skills` by default, which Alvasta Pro treats as `<workspace>/skills` on the next session. If the skill should only be visible to certain agents, pair that with `agents.defaults.skills` or `agents.list[].skills`.
  </Accordion>
  ...
  <Accordion title="A subagent finished, but the completion update went to the wrong place or never posted. What should I check?">
    ...
    - If the completion origin only carries a channel, Alvasta Pro falls back to the requester session's stored route (`lastChannel` / `lastTo` / `lastAccountId`) so direct delivery can still succeed.
    ...
    - If the child's last visible assistant reply is the exact silent token `NO_REPLY` / `no_reply`, or exactly `ANNOUNCE_SKIP`, Alvasta Pro intentionally suppresses the announce instead of posting stale earlier progress.
    ...
    ```bash
    alvasta-pro tasks show <runId-or-sessionKey>
    ```
  ...
  <Accordion title="Cron or reminders do not fire. What should I check?">
    ...
    ```bash
    alvasta-pro cron run <jobId>
    alvasta-pro cron runs --id <jobId> --limit 50
    ```
  ...
  <Accordion title="How do I install skills on Linux?">
    Use native `alvasta-pro skills` commands or drop skills into your workspace. The macOS Skills UI isn't available on Linux.
    Browse skills at [https://clawhub.ai](https://clawhub.ai).

    ```bash
    alvasta-pro skills search "calendar"
    alvasta-pro skills search --limit 20
    alvasta-pro skills install <skill-slug>
    alvasta-pro skills install <skill-slug> --version <version>
    alvasta-pro skills install <skill-slug> --force
    alvasta-pro skills update --all
    alvasta-pro skills list --eligible
    alvasta-pro skills check
    ```

    Native `alvasta-pro skills install` writes into the active workspace `skills/`
    ...
    `~/.alvasta-pro/skills` and use `agents.defaults.skills` or
    `agents.list[].skills` if you want to narrow which agents can see it.

  </Accordion>

  <Accordion title="Can Alvasta Pro run tasks on a schedule or continuously in the background?">
  ...
  <Accordion title="Can I run Apple macOS-only skills from Linux?">
    Not directly. macOS skills are gated by `metadata.alvasta-pro.os` plus required binaries, and skills only appear in the system prompt when they are eligible on the **Gateway host**. On Linux, `darwin`-only skills (like `apple-notes`, `apple-reminders`, `things-mac`) will not load unless you override the gating.
    ...
    **Option B - use a macOS node (no SSH).**
    Run the Gateway on Linux, pair a macOS node (menubar app), and set **Node Run Commands** to "Always Ask" or "Always Allow" on the Mac. Alvasta Pro can treat macOS-only skills as eligible when the required binaries exist on the node. The agent runs those skills via the `nodes` tool. If you choose "Always Ask", approving "Always Allow" in the prompt adds that command to the allowlist.
    ...
    **Option C - proxy macOS binaries over SSH (advanced).**
    ...
    3. Override the skill metadata (workspace or `~/.alvasta-pro/skills`) to allow Linux:

       ```markdown
       ---
       name: apple-notes
       description: Manage Apple Notes via the memo CLI on macOS.
       metadata: { "alvasta-pro": { "os": ["darwin", "linux"], "requires": { "bins": ["memo"] } } }
       ---
       ```
  ...
  <Accordion title="Do you have a Notion or HeyGen integration?">
    ...
    Native installs land in the active workspace `skills/` directory. For shared skills across agents, place them in `~/.alvasta-pro/skills/<name>/SKILL.md`. If only some agents should see a shared install, configure `agents.defaults.skills` or `agents.list[].skills`. Some skills expect binaries installed via Homebrew; on Linux that means Linuxbrew (see the Homebrew Linux FAQ entry above). See [Skills](/tools/skills), [Skills config](/tools/skills-config), and [ClawHub](/tools/clawhub).

  </Accordion>

  <Accordion title="How do I use my existing signed-in Chrome with Alvasta Pro?">
    ...
    alvasta-pro browser --browser-profile user snapshot
    ```
...
<AccordionGroup>
  ...
  <Accordion title="How do I bind a host folder into the sandbox?">
    ...
    Alvasta Pro validates bind sources against both the normalized path and the canonical path resolved through the deepest existing ancestor. That means symlink-parent escapes still fail closed even when the last path segment does not exist yet, and allowed-root checks still apply after symlink resolution.
  ...
  <Accordion title="How does memory work?">
    Alvasta Pro memory is just Markdown files in the agent workspace:

    - Daily notes in `memory/YYYY-MM-DD.md`
    - Curated long-term notes in `MEMORY.md` (main/private sessions only)

    Alvasta Pro also runs a **silent pre-compaction memory flush** to remind the model
  ...
  <Accordion title="Does semantic memory search require an OpenAI API key?">
    ...
    If you don't set a provider explicitly, Alvasta Pro auto-selects a provider when it
    ...
    configured and present, Alvasta Pro
...
<AccordionGroup>
  <Accordion title="Is all data used with Alvasta Pro saved locally?">
    No - **Alvasta Pro's state is local**, but **external services still see what you send them**.

    - **Local by default:** sessions, memory files, config, and workspace live on the Gateway host
      (`~/.alvasta-pro` + your workspace directory).
  ...
  <Accordion title="Where does Alvasta Pro store its data?">
    Everything lives under `$OPENCLAW_STATE_DIR` (default: `~/.alvasta-pro`):

    | Path                                                            | Purpose                                                            |
    | --------------------------------------------------------------- | ------------------------------------------------------------------ |
    | `$OPENCLAW_STATE_DIR/alvasta-pro.json`                             | Main config (JSON5)                                                |
    ...
    Legacy single-agent path: `~/.alvasta-pro/agent/*` (migrated by `alvasta-pro doctor`).

    Your **workspace** (AGENTS.md, memory files, skills, etc.) is separate and configured via `agents.defaults.workspace` (default: `~/.alvasta-pro/workspace`).

  </Accordion>

  <Accordion title="Where should AGENTS.md / SOUL.md / USER.md / MEMORY.md live?">
    These files live in the **agent workspace**, not `~/.alvasta-pro`.

    - **Workspace (per agent)**: `AGENTS.md`, `SOUL.md`, `IDENTITY.md`, `USER.md`,
      `MEMORY.md` (or legacy fallback `memory.md` when `MEMORY.md` is absent),
      `memory/YYYY-MM-DD.md`, optional `HEARTBEAT.md`.
    - **State dir (`~/.alvasta-pro`)**: config, channel/provider state, auth profiles, sessions, logs,
      and shared skills (`~/.alvasta-pro/skills`).

    Default workspace is `~/.alvasta-pro/workspace`, configurable via:

    ```json5
    {
      agents: { defaults: { workspace: "~/.alvasta-pro/workspace" } },
    }
    ```
  ...
  <Accordion title="Recommended backup strategy">
    ...
    Do **not** commit anything under `~/.alvasta-pro` (credentials, sessions, tokens, or encrypted secrets payloads).
  ...
  <Accordion title="How do I completely uninstall Alvasta Pro?">
  ...
  <Accordion title="Can agents work outside the workspace?">
    ...
    `workspace` to the repo root. The Alvasta Pro repo is just source code; keep the
...
<AccordionGroup>
  <Accordion title="What format is the config? Where is it?">
    Alvasta Pro reads an optional **JSON5** config from `$OPENCLAW_CONFIG_PATH` (default: `~/.alvasta-pro/alvasta-pro.json`):

    ```
    $OPENCLAW_CONFIG_PATH
    ```

    If the file is missing, it uses safe-ish defaults (including a default workspace of `~/.alvasta-pro/workspace`).
  ...
  <Accordion title="Why do I need a token on localhost now?">
    Alvasta Pro enforces gateway auth by default, including loopback. In the normal default path that means token auth: if no explicit auth path is configured, gateway startup resolves to token mode and auto-generates one, saving it to `gateway.auth.token`, so **local WS clients must authenticate**. This blocks other local processes from calling the Gateway.

    If you prefer a different auth path, you can explicitly choose password mode (or, for non-loopback identity-aware reverse proxies, `trusted-proxy`). If you **really** want open loopback, set `gateway.auth.mode: "none"` explicitly in your config. Doctor can generate a token for you any time: `alvasta-pro doctor --generate-gateway-token`.
  ...
  <Accordion title="How do I disable funny CLI taglines?">
    ...
    - `default`: uses `All your chats, one Alvasta Pro.` every time.
  ...
  <Accordion title="How do I enable web search (and web fetch)?">
    ...
    **Recommended:** run `alvasta-pro configure --section web` and choose a provider.
    ...
    - If `tools.web.fetch.provider` is omitted, Alvasta Pro auto-detects the first ready fetch fallback provider from available credentials. Today the bundled provider is Firecrawl.
    - Daemons read env vars from `~/.alvasta-pro/.env` (or the service environment).
  ...
  <Accordion title="config.apply wiped my config. How do I recover and avoid this?">
    ...
    - Restore from backup (git or a copied `~/.alvasta-pro/alvasta-pro.json`).
    - If you have no backup, re-run `alvasta-pro doctor` and reconfigure channels/models.
    - If this was unexpected, file a bug and include your last known config or any backup.
    - A local coding agent can often reconstruct a working config from logs or history.

    Avoid it:

    - Use `alvasta-pro config set` for small changes.
    - Use `alvasta-pro configure` for interactive edits.
  ...
  <Accordion title="Can the Alvasta Pro browser run headless?">
...
<AccordionGroup>
  ...
  <Accordion title="Tailscale is connected but I get no replies. What now?">
    ...
    - Gateway is running: `alvasta-pro gateway status`
    - Gateway health: `alvasta-pro status`
    - Channel health: `alvasta-pro channels status`
  ...
  <Accordion title="Can two Alvasta Pro instances talk to each other (local + VPS)?">
    ...
    **CLI bridge (generic):** run a script that calls the other Gateway with
    `alvasta-pro agent --message ... --deliver`, targeting a chat where the other bot
    ...
    ```bash
    alvasta-pro agent --message "Hello from local bot" --deliver --channel telegram --reply-to <chat-id>
    ```
  ...
  <Accordion title="How do I set up Tailscale on a VPS and connect from my Mac?">
    ...
    ```bash
    alvasta-pro gateway --tailscale serve
    ```
...
<AccordionGroup>
  <Accordion title="How does Alvasta Pro load environment variables?">
    Alvasta Pro reads env vars from the parent process (shell, launchd/systemd, CI, etc.) and additionally loads:

    - `.env` from the current working directory
    - a global fallback `.env` from `~/.alvasta-pro/.env` (aka `$OPENCLAW_STATE_DIR/.env`)
  ...
  <Accordion title="I started the Gateway via the service and my env vars disappeared. What now?">
    ...
    1. Put the missing keys in `~/.alvasta-pro/.env` so they're picked up even when the service doesn't inherit your shell env.
  ...
  <Accordion title='I set COPILOT_GITHUB_TOKEN, but models status shows "Shell env: off." Why?'>
    `alvasta-pro models status` reports whether **shell env import** is enabled. "Shell env: off"
    does **not** mean your env vars are missing - it just means Alvasta Pro won't load
    your login shell automatically.

    If the Gateway runs as a service (launchd/systemd), it won't inherit your shell
    environment. Fix by doing one of these:

    1. Put the token in `~/.alvasta-pro/.env`:
...
<AccordionGroup>
  ...
  <Accordion title="Is there a way to make a team of Alvasta Pro instances (one CEO and many agents)?">
  ...
  <Accordion title="How do I completely reset Alvasta Pro but keep it installed?">
    ...
    - If you used profiles (`--profile` / `OPENCLAW_PROFILE`), reset each state dir (defaults are `~/.alvasta-pro-<profile>`).
    - Dev reset: `alvasta-pro gateway --dev --reset` (dev-only; wipes dev config + credentials + sessions + workspace).
  ...
  <Accordion title="Why am I getting heartbeat messages every 30 minutes?">
    ...
    headers like `# Heading`), Alvasta Pro skips the heartbeat run to save API calls.
  ...
  <Accordion title='Do I need to add a "bot account" to a WhatsApp group?'>
    No. Alvasta Pro runs on **your own account**, so if you're in the group, Alvasta Pro can see it.
  ...
  <Accordion title="How do I get the JID of a WhatsApp group?">
    ...
    ```bash
    alvasta-pro directory groups list --channel whatsapp
    ```
  ...
  <Accordion title="Why does Alvasta Pro not reply in a group?">
    Two common causes:
  ...
  <Accordion title="How many workspaces and agents can I create?">
    ...
    - **Disk growth:** sessions + transcripts live under `~/.alvasta-pro/agents/<agentId>/sessions/`.
    ...
    - Use `alvasta-pro doctor` to spot stray workspaces and profile mismatches.
...
<AccordionGroup>
  <Accordion title='What is the "default model"?'>
    Alvasta Pro's default model is whatever you set as:

    ```
    agents.defaults.model.primary
    ```

    Models are referenced as `provider/model` (example: `openai/gpt-5.4`). If you omit the provider, Alvasta Pro first tries an alias, then a unique configured-provider match for that exact model id, and only then falls back to the configured default provider as a deprecated compatibility path. If that provider no longer exposes the configured default model, Alvasta Pro falls back to the first configured provider/model instead of surfacing a stale removed-provider default. You should still **explicitly** set `provider/model`.
  ...
  <Accordion title="How do I switch models without wiping my config?">
    ...
    - `alvasta-pro models set ...` (updates just model config)
    - `alvasta-pro configure --section model` (interactive)
    - edit `agents.defaults.model` in `~/.alvasta-pro/alvasta-pro.json`
    ...
    for partial updates.
    If you did overwrite config, restore from backup or re-run `alvasta-pro doctor` to repair.
  ...
  <Accordion title="Can I use self-hosted models (llama.cpp, vLLM, Ollama)?">
    ...
    4. Run `alvasta-pro onboard` and choose `Ollama`
    ...
    - for manual switching, use `alvasta-pro models list` and `alvasta-pro models set ollama/<model>`
  ...
  <Accordion title="What do Alvasta Pro, Flawd, and Krill use for models?">
    - These deployments can differ and may change over time; there is no fixed provider recommendation.
    - Check the current runtime setting on each gateway with `alvasta-pro models status`.
  ...
  <Accordion title='Why do I see "Unknown model: minimax/MiniMax-M2.7"?'>
    ...
    1. Upgrade to a current Alvasta Pro release (or run from source `main`), then restart the gateway.
  ...
  <Accordion title="Are opus / sonnet / gpt built-in shortcuts?">
    Yes. Alvasta Pro ships a few default shorthands (only applied when the model exists in `agents.defaults.models`):
  ...
  <Accordion title="How do I add models from other providers like OpenRouter or Z.AI?">
    ...
    **No API key found for provider after adding a new agent**
    ...
    ```
    ~/.alvasta-pro/agents/<agentId>/agent/auth-profiles.json
    ```

    Fix options:

    - Run `alvasta-pro agents add <id>` and configure auth during the wizard.
...
<AccordionGroup>
  <Accordion title="How does failover work?">
    ...
    Cooldowns apply to failing profiles (exponential backoff), so Alvasta Pro can keep responding even when a provider is rate-limited or temporarily failing.

    The rate-limit bucket includes more than plain `429` responses. Alvasta Pro
    ...
    explicit billing text on `401` or `403`, Alvasta Pro can still keep that in
    ...
    `organization spending limit exceeded`), Alvasta Pro treats it as
    ...
    unknown/error in it". Alvasta Pro does treat provider-scoped transient shapes
  ...
  <Accordion title='What does "No credentials found for profile anthropic:default" mean?'>
    ...
    **Fix checklist:**
    ...
    - **Confirm where auth profiles live** (new vs legacy paths)
      - Current: `~/.alvasta-pro/agents/<agentId>/agent/auth-profiles.json`
      - Legacy: `~/.alvasta-pro/agent/*` (migrated by `alvasta-pro doctor`)
    - **Confirm your env var is loaded by the Gateway**
      - If you set `ANTHROPIC_API_KEY` in your shell but run the Gateway via systemd/launchd, it may not inherit it. Put it in `~/.alvasta-pro/.env` or enable `env.shellEnv`.
    - **Make sure you're editing the correct agent**
      - Multi-agent setups mean there can be multiple `auth-profiles.json` files.
    - **Sanity-check model/auth status**
      - Use `alvasta-pro models status` to see configured models and whether providers are authenticated.
    ...
    **Fix checklist for "No credentials found for profile anthropic"**
    ...
    - **Use Claude CLI**
      - Run `alvasta-pro models auth login --provider anthropic --method cli --set-default` on the gateway host.
    - **If you want to use an API key instead**
      - Put `ANTHROPIC_API_KEY` in `~/.alvasta-pro/.env` on the **gateway host**.
      - Clear any pinned order that forces a missing profile:

        ```bash
        alvasta-pro models auth order clear --provider anthropic
        ```
  ...
  <Accordion title="Why did it also try Google Gemini and fail?">
    If your model config includes Google Gemini as a fallback (or you switched to a Gemini shorthand), Alvasta Pro will try it during model fallback. If you haven't configured Google credentials, you'll see `No API key found for provider "google"`.
    ...
    **LLM request rejected: thinking signature required (Google Antigravity)**
    ...
    Fix: Alvasta Pro now strips unsigned thinking blocks for Google Antigravity Claude. If it still appears, start a **new session** or set `/thinking off` for that agent.
...
<AccordionGroup>
  ...
  <Accordion title="What are typical profile IDs?">
    Alvasta Pro uses provider-prefixed IDs like:
  ...
  <Accordion title="Can I control which auth profile is tried first?">
    ...
    Alvasta Pro may temporarily skip a profile if it's in a short **cooldown** (rate limits/timeouts/auth failures) or a longer **disabled** state (billing/insufficient credits). To inspect this, run `alvasta-pro models status --json` and check `auth.unusableProfiles`. Tuning: `auth.cooldowns.billingBackoffHours*`.
    ...
    alvasta-pro models auth order get --provider anthropic

    # Lock rotation to a single profile (only try this one)
    alvasta-pro models auth order set --provider anthropic anthropic:default

    # Or set an explicit order (fallback within provider)
    alvasta-pro models auth order set --provider anthropic anthropic:work anthropic:default
    ...
    ```bash
    alvasta-pro models auth order set --provider anthropic --agent main anthropic:default
    ```

    To verify what will actually be tried, use:

    ```bash
    alvasta-pro models status --probe
    ```
...
<AccordionGroup>
  ...
  <Accordion title='Why does alvasta-pro gateway status say "Runtime: running" but "RPC probe: failed"?'>
    Because "running" is the **supervisor's** view (launchd/systemd/schtasks). The RPC probe is the CLI actually connecting to the gateway WebSocket and calling `status`.

    Use `alvasta-pro gateway status` and trust these lines:
  ...
  <Accordion title='Why does alvasta-pro gateway status show "Config (cli)" and "Config (service)" different?'>
    You're editing one config file while the service is running another (often a `--profile` / `OPENCLAW_STATE_DIR` mismatch).

    Fix:

    ```bash
    alvasta-pro gateway install --force
    ```
  ...
  <Accordion title='What does "another gateway instance is already listening" mean?'>
    Alvasta Pro enforces a runtime lock by binding the WebSocket listener immediately on startup (default `ws://127.0.0.1:18789`). If the bind fails with `EADDRINUSE`, it throws `GatewayLockError` indicating another instance is already listening.

    Fix: stop the other instance, free the port, or run with `alvasta-pro gateway --port <port>`.

  </Accordion>

  <Accordion title="How do I run Alvasta Pro in remote mode (client connects to a Gateway elsewhere)?">
    ...
    - `alvasta-pro gateway` only starts when `gateway.mode` is `local` (or you pass the override flag).
  ...
  <Accordion title='The Control UI says "unauthorized" (or keeps reconnecting). What now?'>
    ...
    - Fastest: `alvasta-pro dashboard` (prints + copies the dashboard URL, tries to open; shows SSH hint if headless).
    - If you don't have a token yet: `alvasta-pro doctor --generate-gateway-token`.
    ...
    - If mismatch persists after the one retry, rotate/re-approve the paired device token:
      - `alvasta-pro devices list`
      - `alvasta-pro devices rotate --device <id> --role operator`
    ...
    - Still stuck? Run `alvasta-pro status --all` and follow [Troubleshooting](/gateway/troubleshooting). See [Dashboard](/web/dashboard) for auth details.
  ...
  <Accordion title="Can I run multiple Gateways on the same host?">
    ...
    - Use `alvasta-pro --profile <name> ...` per instance (auto-creates `~/.alvasta-pro-<name>`).
    - Set a unique `gateway.port` in each profile config (or pass `--port` for manual runs).
    - Install a per-profile service: `alvasta-pro --profile <name> gateway install`.

    Profiles also suffix service names (`ai.alvasta-pro.<profile>`; legacy `com.alvasta-pro.*`, `alvasta-pro-gateway-<profile>.service`, `Alvasta Pro Gateway (<profile>)`).
  ...
  <Accordion title='What does "invalid handshake" / code 1008 mean?'>
    ...
    ```
    alvasta-pro tui --url ws://<host>:18789 --token <token>
    ```
...
<AccordionGroup>
  <Accordion title="Where are logs?">
    ...
    - macOS: `$OPENCLAW_STATE_DIR/logs/gateway.log` and `gateway.err.log` (default: `~/.alvasta-pro/logs/...`; profiles use `~/.alvasta-pro-<profile>/logs/...`)
    - Linux: `journalctl --user -u alvasta-pro-gateway[-<profile>].service -n 200 --no-pager`
    - Windows: `schtasks /Query /TN "Alvasta Pro Gateway (<profile>)" /V /FO LIST`
  ...
  <Accordion title="How do I start/stop/restart the Gateway service?">
    ...
    If you run the gateway manually, `alvasta-pro gateway --force` can reclaim the port. See [Gateway](/gateway).

  </Accordion>

  <Accordion title="I closed my terminal on Windows - how do I restart Alvasta Pro?">
  ...
  <Accordion title='"Disconnected from gateway: no reason" - what now?'>
    ...
    1. Is the Gateway running? `alvasta-pro gateway status`
    2. Is the Gateway healthy? `alvasta-pro status`
    3. Does the UI have the right token? `alvasta-pro dashboard`
  ...
  <Accordion title="Telegram setMyCommands fails. What should I check?">
    ...
    ```bash
    alvasta-pro channels status
    alvasta-pro channels logs --channel telegram
    ```

    Then match the error:

    - `BOT_COMMANDS_TOO_MUCH`: the Telegram menu has too many entries. Alvasta Pro already trims to the Telegram limit and retries with fewer commands, but some menu entries still need to be dropped. Reduce plugin/skill/custom commands, or disable `channels.telegram.commands.native` if you do not need the menu.
  ...
  <Accordion title="ELI5: alvasta-pro gateway restart vs alvasta-pro gateway">
    - `alvasta-pro gateway restart`: restarts the **background service** (launchd/systemd).
    - `alvasta-pro gateway`: runs the gateway **in the foreground** for this terminal session.

    If you installed the service, use the gateway commands. Use `alvasta-pro gateway` when
...
<AccordionGroup>
  <Accordion title="My skill generated an image/PDF, but nothing was sent">
    Outbound attachments from the agent must include a `MEDIA:<path-or-url>` line (on its own line). See [Alvasta Pro assistant setup](/start/alvasta-pro) and [Agent send](/tools/agent-send).

    CLI sending:

    ```bash
    alvasta-pro message send --target +15555550123 --message "Here you go" --media /path/to/file.png
    ```
...
<AccordionGroup>
  <Accordion title="Is it safe to expose Alvasta Pro to inbound DMs?">
    Treat inbound DMs as untrusted input. Defaults are designed to reduce risk:

    - Default behavior on DM-capable channels is **pairing**:
      - Unknown senders receive a pairing code; the bot does not process their message.
      - Approve with: `alvasta-pro pairing approve --channel <channel> [--account <id>] <code>`
      - Pending requests are capped at **3 per channel**; check `alvasta-pro pairing list --channel <channel> [--account <id>]` if a code didn't arrive.
    - Opening DMs publicly requires explicit opt-in (`dmPolicy: "open"` and allowlist `"*"`).

    Run `alvasta-pro doctor` to surface risky DM policies.
  ...
  <Accordion title="WhatsApp: will it message my contacts? How does pairing work?">
    No. Default WhatsApp DM policy is **pairing**. Unknown senders only get a pairing code and their message is **not processed**. Alvasta Pro only replies to chats it receives or to explicit sends you trigger.

    Approve pairing with:

    ```bash
    alvasta-pro pairing approve whatsapp <code>
    ```

    List pending requests:

    ```bash
    alvasta-pro pairing list whatsapp
    ```
...
<AccordionGroup>
  ...
  <Accordion title='How do I send a Discord message from Telegram? ("Cross-context messaging denied")'>
    Alvasta Pro blocks **cross-provider** messaging by default. If a tool call is bound
