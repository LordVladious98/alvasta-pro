---
summary: "Run Alvasta Pro with vLLM (OpenAI-compatible local server)"
read_when:
  - You want to run Alvasta Pro against a local vLLM server
...
vLLM can serve open-source (and some custom) models via an **OpenAI-compatible** HTTP API. Alvasta Pro can connect to vLLM using the `openai-completions` API.

Alvasta Pro can also **auto-discover** available models from vLLM when you opt in with `VLLM_API_KEY` (any value works if your server doesn’t enforce auth) and you do not define an explicit `models.providers.vllm` entry.
...
When `VLLM_API_KEY` is set (or an auth profile exists) and you **do not** define `models.providers.vllm`, Alvasta Pro will query:
