---
summary: "Run Alvasta Pro with SGLang (OpenAI-compatible self-hosted server)"
read_when:
  - You want to run Alvasta Pro against a local SGLang server
...
Alvasta Pro can connect to SGLang using the `openai-completions` API.

Alvasta Pro can also **auto-discover** available models from SGLang when you opt
...
define `models.providers.sglang`, Alvasta Pro will query:
