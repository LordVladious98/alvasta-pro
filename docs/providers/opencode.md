---
summary: "Use OpenCode Zen and Go catalogs with Alvasta Pro"
read_when:
...
OpenCode exposes two hosted catalogs in Alvasta Pro:

- `opencode/...` for the **Zen** catalog
- `opencode-go/...` for the **Go** catalog

Both catalogs use the same OpenCode API key. Alvasta Pro keeps the runtime provider ids
...
alvasta-pro onboard --opencode-zen-api-key "$OPENCODE_API_KEY"
```

### Go catalog

```bash
alvasta-pro onboard --auth-choice opencode-go
alvasta-pro onboard --opencode-go-api-key "$OPENCODE_API_KEY"
```
...
- Gemini-backed OpenCode refs stay on the proxy-Gemini path, so Alvasta Pro keeps
