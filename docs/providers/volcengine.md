...
read_when:
  - You want to use Volcano Engine or Doubao models with Alvasta Pro
...
```bash
alvasta-pro onboard --auth-choice volcengine-api-key
```
...
`alvasta-pro onboard --auth-choice volcengine-api-key` currently sets
...
loaded yet, Alvasta Pro falls back to the unfiltered catalog instead of showing an
...
`~/.alvasta-pro/.env` or via `env.shellEnv`).
