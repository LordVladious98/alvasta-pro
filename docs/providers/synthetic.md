---
summary: "Use Synthetic's Anthropic-compatible API in Alvasta Pro"
read_when:
...
Synthetic exposes Anthropic-compatible endpoints. Alvasta Pro registers it as the
...
```bash
alvasta-pro onboard --auth-choice synthetic-api-key
```
...
Note: Alvasta Pro's Anthropic client appends `/v1` to the base URL, so use
