...
read_when:
  - You want to use Chutes with Alvasta Pro
...
OpenAI-compatible API. Alvasta Pro supports both browser OAuth and direct API-key
...
- Auth:
  - OAuth via `alvasta-pro onboard --auth-choice chutes`
  - API key via `alvasta-pro onboard --auth-choice chutes-api-key`
...
```bash
alvasta-pro onboard --auth-choice chutes
```

Alvasta Pro launches the browser flow locally, or shows a URL + redirect-paste
flow on remote/headless hosts. OAuth tokens auto-refresh through Alvasta Pro auth
profiles.
...
```bash
alvasta-pro onboard --auth-choice chutes-api-key
```
...
When Chutes auth is available, Alvasta Pro queries the Chutes catalog with that
credential and uses the discovered models. If discovery fails, Alvasta Pro falls
back to a bundled static catalog so onboarding and startup still work.

## Default aliases

Alvasta Pro also registers three convenience aliases for the bundled Chutes
catalog:
