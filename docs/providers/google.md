...
read_when:
  - You want to use Google Gemini models with Alvasta Pro
...
```bash
alvasta-pro onboard --auth-choice gemini-api-key
```
...
```bash
alvasta-pro models auth login --provider google-gemini-cli --set-default
```
...
command is installed and on `PATH`. Alvasta Pro supports both Homebrew installs
...
For direct Gemini API runs (`api: "google-generative-ai"`), Alvasta Pro now
...
- Gemini cache-hit usage is normalized into Alvasta Pro `cacheRead` from
