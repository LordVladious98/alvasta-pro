---
summary: "Use Venice AI privacy-focused models in Alvasta Pro"
read_when:
  - You want privacy-focused inference in Alvasta Pro
...
```bash
alvasta-pro onboard --auth-choice venice-api-key
```
...
```bash
alvasta-pro agent --model venice/kimi-k2-5 --message "Hello, are you working?"
```

## Model Selection

After setup, Alvasta Pro shows all available Venice models. Pick based on your needs:
...
```bash
alvasta-pro models set venice/kimi-k2-5
alvasta-pro models set venice/claude-opus-4-6
```

List all available models:

```bash
alvasta-pro models list | grep venice
```
...
Alvasta Pro automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
alvasta-pro agent --model venice/kimi-k2-5 --message "Quick health check"

# Use Claude Opus via Venice (anonymized)
alvasta-pro agent --model venice/claude-opus-4-6 --message "Summarize this task"

# Use uncensored model
alvasta-pro agent --model venice/venice-uncensored --message "Draft options"

# Use vision model with image
alvasta-pro agent --model venice/qwen3-vl-235b-a22b --message "Review attached image"

# Use coding model
alvasta-pro agent --model venice/qwen3-coder-480b-a35b-instruct --message "Refactor this function"
```
...
The Venice model catalog updates dynamically. Run `alvasta-pro models list` to see currently available models. Some models may be temporarily offline.
