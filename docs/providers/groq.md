...
read_when:
  - You want to use Groq with Alvasta Pro
...
(Llama, Gemma, Mistral, and more) using custom LPU hardware. Alvasta Pro connects
...
media-understanding provider, Alvasta Pro uses Groq's `whisper-large-v3-turbo`
...
Groq's model catalog changes frequently. Run `alvasta-pro models list | grep groq`
