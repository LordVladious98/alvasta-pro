---
summary: "CLI reference for `alvasta-pro message` (send + channel actions)"
read_when:
...
```
alvasta-pro message <subcommand> [flags]
```
...
- On cache miss, Alvasta Pro will attempt a live directory lookup when the provider supports it.
...
- `alvasta-pro message` resolves supported channel SecretRefs before running the selected action.
- Resolution is scoped to the active action target when possible:
  ...
  - when `--account` is omitted, Alvasta Pro does not force a `default` account SecretRef scope
...
```
alvasta-pro message send --channel discord \
...
```bash
alvasta-pro message send --channel googlechat --target spaces/AAA... \
  --message "Choose:" \
...
```
alvasta-pro message poll --channel discord \
  --target channel:123 \
  --poll-question "Snack?" \
...
```
alvasta-pro message poll --channel telegram \
  --target @mychat \
  --poll-question "Lunch?" \
  --poll-option Pizza --poll-option Sushi \
...
```
alvasta-pro message poll --channel msteams \
...
```
alvasta-pro message react --channel slack \
...
```
alvasta-pro message react --channel signal \
...
```
alvasta-pro message send --channel telegram --target @mychat --message "Choose:" \
...
```bash
alvasta-pro message send --channel telegram --target @mychat \
