---
summary: "Expose Alvasta Pro channel conversations over MCP and manage saved MCP server definitions"
read_when:
  - Connecting Codex, Claude Code, or another MCP client to Alvasta Pro-backed channels
  - Running `alvasta-pro mcp serve`
  - Managing Alvasta Pro-saved MCP server definitions
title: "mcp"
---

# mcp

`alvasta-pro mcp` has two jobs:

- run Alvasta Pro as an MCP server with `alvasta-pro mcp serve`
- manage Alvasta Pro-owned outbound MCP server definitions with `list`, `show`,
  `set`, and `unset`

In other words:

- `serve` is Alvasta Pro acting as an MCP server
- `list` / `show` / `set` / `unset` is Alvasta Pro acting as an MCP client-side
  registry for other MCP servers its runtimes may consume later

Use [`alvasta-pro acp`](/cli/acp) when Alvasta Pro should host a coding harness
session itself and route that runtime through ACP.

## Alvasta Pro as an MCP server

This is the `alvasta-pro mcp serve` path.

## When to use `serve`

Use `alvasta-pro mcp serve` when:

- Codex, Claude Code, or another MCP client should talk directly to
  Alvasta Pro-backed channel conversations
- you already have a local or remote Alvasta Pro Gateway with routed sessions
- you want one MCP server that works across Alvasta Pro's channel backends instead
  of running separate per-channel bridges

Use [`alvasta-pro acp`](/cli/acp) instead when Alvasta Pro should host the coding
runtime itself and keep the agent session inside Alvasta Pro.

## How it works

`alvasta-pro mcp serve` starts a stdio MCP server. The MCP client owns that
process. While the client keeps the stdio session open, the bridge connects to a
local or remote Alvasta Pro Gateway over WebSocket and exposes routed channel
conversations over MCP.

Lifecycle:

1. the MCP client spawns `alvasta-pro mcp serve`
...
conversations. A conversation appears when Alvasta Pro already has session state
...
```bash
# Local Gateway
alvasta-pro mcp serve

# Remote Gateway
alvasta-pro mcp serve --url wss://gateway-host:18789 --token-file ~/.alvasta-pro/gateway.token

# Remote Gateway with password auth
alvasta-pro mcp serve --url wss://gateway-host:18789 --password-file ~/.alvasta-pro/gateway.password

# Enable verbose bridge logs
alvasta-pro mcp serve --verbose

# Disable Claude-specific push notifications
alvasta-pro mcp serve --claude-channel-mode off
```
...
Alvasta Pro equivalent of a Claude Code channel adapter: standard MCP tools remain
...
`alvasta-pro mcp serve` supports:
...
- sender allowlists, pairing, and channel-level trust still belong to the
  underlying Alvasta Pro channel configuration
...
Alvasta Pro ships a deterministic Docker smoke for this bridge:
...
- starts a second container that spawns `alvasta-pro mcp serve`
...
## Alvasta Pro as an MCP client registry

This is the `alvasta-pro mcp list`, `show`, `set`, and `unset` path.

These commands do not expose Alvasta Pro over MCP. They manage Alvasta Pro-owned MCP
server definitions under `mcp.servers` in Alvasta Pro config.

Those saved definitions are for runtimes that Alvasta Pro launches or configures
later, such as embedded Pi and other runtime adapters. Alvasta Pro stores the
definitions centrally so those runtimes do not need to keep their own duplicate
MCP server lists.

Important behavior:

- these commands only read or write Alvasta Pro config
...
Alvasta Pro also stores a lightweight MCP server registry in config for surfaces
that want Alvasta Pro-managed MCP definitions.

Commands:

- `alvasta-pro mcp list`
- `alvasta-pro mcp show [name]`
- `alvasta-pro mcp set <name> <json>`
- `alvasta-pro mcp unset <name>`
...
```bash
alvasta-pro mcp list
alvasta-pro mcp show context7 --json
alvasta-pro mcp set context7 '{"command":"uvx","args":["context7-mcp"]}'
alvasta-pro mcp set docs '{"url":"https://mcp.example.com"}'
alvasta-pro mcp unset context7
```
...
| --------------------- | -------------------------------------------------------------------------------------- |
| `url`                 | HTTP or HTTPS URL of the remote server (required)                                      |
| `transport`           | Set to `"streamable-http"` to select this transport; when omitted, Alvasta Pro uses `sse` |
| `headers`             | Optional key-value map of HTTP headers (for example auth tokens)                       |
| `connectionTimeoutMs` | Per-server connection timeout in ms (optional)                                         |
