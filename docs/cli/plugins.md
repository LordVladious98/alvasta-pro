---
summary: "CLI reference for `alvasta-pro plugins` (list, install, marketplace, uninstall, enable/disable, doctor)"
read_when:
...
alvasta-pro plugins list --enabled
alvasta-pro plugins list --verbose
alvasta-pro plugins list --json
alvasta-pro plugins install <path-or-spec>
alvasta-pro plugins inspect <id>
alvasta-pro plugins inspect <id> --json
alvasta-pro plugins inspect --all
alvasta-pro plugins info <id>
alvasta-pro plugins enable <id>
alvasta-pro plugins disable <id>
alvasta-pro plugins uninstall <id>
alvasta-pro plugins doctor
alvasta-pro plugins update <id>
alvasta-pro plugins update --all
alvasta-pro plugins marketplace list <marketplace>
alvasta-pro plugins marketplace list <marketplace> --json
```

Bundled plugins ship with Alvasta Pro. Some are enabled by default (for example
bundled model providers, bundled speech providers, and the bundled browser
plugin); others require `plugins enable`.

Native Alvasta Pro plugins must ship `alvasta-pro.plugin.json` with an inline JSON
Schema (`configSchema`, even if empty). Compatible bundles use their own bundle
manifests instead.

`plugins list` shows `Format: alvasta-pro` or `Format: bundle`. Verbose list/info
...
alvasta-pro plugins install clawhub:<package>              # ClawHub only
alvasta-pro plugins install <package> --force              # overwrite existing install
alvasta-pro plugins install <package> --pin                # pin version
alvasta-pro plugins install <package> --dangerously-force-unsafe-install
alvasta-pro plugins install <path>                         # local path
alvasta-pro plugins install <plugin>@<marketplace>         # marketplace
alvasta-pro plugins install <plugin> --marketplace <name>  # marketplace (explicit)
...
run `alvasta-pro doctor --fix` first. The only documented exception is a narrow
...
override, while `alvasta-pro skills install` remains a separate ClawHub skill
download/install flow.

`plugins install` is also the install surface for hook packs that expose
`alvasta-pro.hooks` in `package.json`. Use `alvasta-pro hooks` for filtered hook
...
If a bare install spec matches a bundled plugin id (for example `diffs`), Alvasta Pro
...
```bash
alvasta-pro plugins install clawhub:alvasta-pro-codex-app-server
alvasta-pro plugins install clawhub:alvasta-pro-codex-app-server@1.2.3
```

Alvasta Pro now also prefers ClawHub for bare npm-safe plugin specs. It only falls
back to npm if ClawHub does not have that package or version:

```bash
alvasta-pro plugins install alvasta-pro-codex-app-server
```

Alvasta Pro downloads the package archive from ClawHub, checks the advertised
...
```bash
alvasta-pro plugins marketplace list <marketplace-name>
alvasta-pro plugins install <plugin-name>@<marketplace-name>
```

Use `--marketplace` when you want to pass the marketplace source explicitly:

```bash
alvasta-pro plugins install <plugin-name> --marketplace <marketplace-name>
alvasta-pro plugins install <plugin-name> --marketplace <owner/repo>
alvasta-pro plugins install <plugin-name> --marketplace https://github.com/<owner>/<repo>
alvasta-pro plugins install <plugin-name> --marketplace ./my-marketplace
```
...
inside the cloned marketplace repo. Alvasta Pro accepts relative path sources from
that repo and rejects HTTP(S), absolute-path, git, GitHub, and other non-path
plugin sources from remote manifests.

For local paths and archives, Alvasta Pro auto-detects:

- native Alvasta Pro plugins (`alvasta-pro.plugin.json`)
...
```bash
alvasta-pro plugins install -l ./my-plugin
```
...
```bash
alvasta-pro plugins uninstall <id>
alvasta-pro plugins uninstall <id> --dry-run
...
```bash
alvasta-pro plugins update <id-or-npm-spec>
alvasta-pro plugins update --all
alvasta-pro plugins update <id-or-npm-spec> --dry-run
alvasta-pro plugins update @alvasta-pro/voice-call@beta
alvasta-pro plugins update alvasta-pro-codex-app-server --dangerously-force-unsafe-install
```

Updates apply to tracked installs in `plugins.installs` and tracked hook-pack
installs in `hooks.internal.installs`.

When you pass a plugin id, Alvasta Pro reuses the recorded install spec for that
...
or exact version. Alvasta Pro resolves that package name back to the tracked plugin
