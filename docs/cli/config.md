---
summary: "CLI reference for `alvasta-pro config` (get/set/unset/file/schema/validate)"
read_when:
...
Config helpers for non-interactive edits in `alvasta-pro.json`: get/set/unset/file/schema/validate
values by path and print the active config file. Run without a subcommand to
open the configure wizard (same as `alvasta-pro configure`).

Root options:

- `--section <section>`: repeatable guided-setup section filter when you run `alvasta-pro config` without a subcommand
...
```bash
alvasta-pro config file
alvasta-pro config --section model
alvasta-pro config --section gateway --section daemon
alvasta-pro config schema
alvasta-pro config get browser.executablePath
...
alvasta-pro config set secrets.providers.vaultfile --provider-source file --provider-path /etc/alvasta-pro/secrets.json --provider-mode json
...
alvasta-pro config validate
alvasta-pro config validate --json
```

### `config schema`

Print the generated JSON schema for `alvasta-pro.json` to stdout as JSON.
...
```bash
alvasta-pro config schema > alvasta-pro.schema.json
```
...
alvasta-pro config get agents.list[0].id
```
...
alvasta-pro config set agents.list[1].tools.exec.node "node-id-or-name"
```
...
```bash
alvasta-pro config set agents.defaults.heartbeat.every "0m"
alvasta-pro config set gateway.port 19001 --strict-json
alvasta-pro config set channels.whatsapp.groups '["*"]' --strict-json
```
...
`alvasta-pro config set` supports four assignment styles:

1. Value mode: `alvasta-pro config set <path> <value>`
2. SecretRef builder mode:

```bash
alvasta-pro config set channels.discord.token \
...
```bash
alvasta-pro config set secrets.providers.vault \
  --provider-source exec \
  --provider-command /usr/local/bin/alvasta-pro-vault \
  --provider-arg read \
...
```bash
alvasta-pro config set --batch-json '[
  {
...
```bash
alvasta-pro config set --batch-file ./config-set.batch.json --dry-run
```
...
alvasta-pro config set secrets.providers.vaultfile \
  '{"source":"file","path":"/etc/alvasta-pro/secrets.json","mode":"json"}' \
  --strict-json
```
...
Use `--dry-run` to validate changes without writing `alvasta-pro.json`.
...
```json
{
  "ok": true,
  "operations": 1,
  "configPath": "~/.alvasta-pro/alvasta-pro.json",
  "inputModes": ["builder"],
  "checks": {
    "schema": false,
    "resolvability": true,
