---
summary: "CLI reference for `alvasta-pro agent` (send one agent turn via the Gateway)"
read_when:
...
```bash
alvasta-pro agent --to +15555550123 --message "status update" --deliver
...
alvasta-pro agent --agent ops --message "Generate report" --deliver --reply-channel slack --reply-to "#reports"
alvasta-pro agent --agent ops --message "Run locally" --local
```
...
- Marker writes are source-authoritative: Alvasta Pro persists markers from the active source config snapshot, not from resolved runtime secret values.
