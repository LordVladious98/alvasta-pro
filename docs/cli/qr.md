---
summary: "CLI reference for `alvasta-pro qr` (generate mobile pairing QR + setup code)"
read_when:
...
```bash
alvasta-pro qr
alvasta-pro qr --setup-code-only
alvasta-pro qr --json
alvasta-pro qr --remote
alvasta-pro qr --url wss://gateway.example/ws
```
...
- With `--remote`, Alvasta Pro requires either `gateway.remote.url` or
