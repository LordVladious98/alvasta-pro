---
summary: "CLI reference for `alvasta-pro dns` (wide-area discovery helpers)"
read_when:
  - You want wide-area discovery (DNS-SD) via Tailscale + CoreDNS
  - You’re setting up split DNS for a custom discovery domain (example: alvasta-pro.internal)
title: "dns"
---
...
```bash
alvasta-pro dns setup
alvasta-pro dns setup --domain alvasta-pro.internal
...
- `--domain <domain>`: wide-area discovery domain (for example `alvasta-pro.internal`)
...
- zone file path
- current tailnet IPs
- recommended `alvasta-pro.json` discovery config
...
- If `--domain` is omitted, Alvasta Pro uses `discovery.wideArea.domain` from config.
