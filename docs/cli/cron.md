---
summary: "CLI reference for `alvasta-pro cron` (schedule and run background jobs)"
read_when:
...
Tip: run `alvasta-pro cron --help` for the full command surface.
...
Note: `alvasta-pro cron run` now returns as soon as the manual run is queued for execution. Successful responses include `{ ok: true, enqueued: true, runId }`; use `alvasta-pro cron runs --id <job-id>` to follow the eventual outcome.

Note: `alvasta-pro cron run <job-id>` force-runs by default. Use `--due` to keep the
...
- `cron.runLog.maxBytes` + `cron.runLog.keepLines` prune `~/.alvasta-pro/cron/runs/<jobId>.jsonl`.

Upgrade note: if you have older cron jobs from before the current delivery/store format, run
`alvasta-pro doctor --fix`. Doctor now normalizes legacy cron fields (`jobId`, `schedule.cron`,
...
```bash
alvasta-pro cron edit <job-id> --announce --channel telegram --to "123456789"
```

Disable delivery for an isolated job:

```bash
alvasta-pro cron edit <job-id> --no-deliver
```

Enable lightweight bootstrap context for an isolated job:

```bash
alvasta-pro cron edit <job-id> --light-context
```

Announce to a specific channel:

```bash
alvasta-pro cron edit <job-id> --announce --channel slack --to "channel:C1234567890"
```
...
```bash
alvasta-pro cron run <job-id>
alvasta-pro cron run <job-id> --due
alvasta-pro cron runs --id <job-id> --limit 50
```

Agent/session retargeting:

```bash
alvasta-pro cron edit <job-id> --agent ops
alvasta-pro cron edit <job-id> --clear-agent
alvasta-pro cron edit <job-id> --session current
alvasta-pro cron edit <job-id> --session "session:daily-brief"
```
...
```bash
alvasta-pro cron edit <job-id> --announce --channel slack --to "channel:C1234567890"
alvasta-pro cron edit <job-id> --best-effort-deliver
alvasta-pro cron edit <job-id> --no-best-effort-deliver
alvasta-pro cron edit <job-id> --no-deliver
```
