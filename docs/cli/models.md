---
summary: "CLI reference for `alvasta-pro models` (status/list/set/scan, aliases, fallbacks, auth)"
read_when:
...
alvasta-pro models set <model-or-alias>
alvasta-pro models scan
```

`alvasta-pro models status` shows the resolved default/fallbacks plus an auth overview.
...
when available; otherwise Alvasta Pro falls back to matching OAuth/API-key
...
- If you omit the provider, Alvasta Pro resolves the input as an alias first, then
...
- `no_model`: provider auth exists, but Alvasta Pro could not resolve a probeable
...
```bash
alvasta-pro models auth add
alvasta-pro models auth login --provider <id>
alvasta-pro models auth setup-token --provider <id>
alvasta-pro models auth paste-token
```
...
`alvasta-pro plugins list` to see which providers are installed.

Examples:

```bash
alvasta-pro models auth login --provider openai-codex --set-default
```
