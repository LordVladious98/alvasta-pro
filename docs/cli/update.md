---
summary: "CLI reference for `alvasta-pro update` (safe-ish source update + gateway auto-restart)"
read_when:
...
Safely update Alvasta Pro and switch between stable/beta/dev channels.
...
alvasta-pro update --yes
alvasta-pro update --json
alvasta-pro --update
```
...
- `--tag <dist-tag|version|spec>`: override the package target for this update only. For package installs, `main` maps to `github:alvasta-pro/alvasta-pro#main`.
...
```bash
alvasta-pro update status
alvasta-pro update status --json
alvasta-pro update status --timeout 10
```
...
When you switch channels explicitly (`--channel ...`), Alvasta Pro also keeps the
install method aligned:

- `dev` → ensures a git checkout (default: `~/alvasta-pro`, override with `OPENCLAW_GIT_DIR`),
...
8. Runs `alvasta-pro doctor` as the final “safe update” check.
...
`alvasta-pro --update` rewrites to `alvasta-pro update` (useful for shells and launcher scripts).

## See also

- `alvasta-pro doctor` (offers to run update first on git checkouts)
