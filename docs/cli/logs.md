---
summary: "CLI reference for `alvasta-pro logs` (tail gateway logs via RPC)"
read_when:
...
`alvasta-pro logs` also accepts the standard Gateway client flags:
...
alvasta-pro logs --follow --interval 2000
alvasta-pro logs --limit 500 --max-bytes 500000
alvasta-pro logs --json
alvasta-pro logs --plain
alvasta-pro logs --no-color
alvasta-pro logs --limit 500
alvasta-pro logs --local-time
alvasta-pro logs --follow --local-time
alvasta-pro logs --url ws://127.0.0.1:18789 --token "$OPENCLAW_GATEWAY_TOKEN"
```

## Notes

- Use `--local-time` to render timestamps in your local timezone.
- If the local loopback Gateway asks for pairing, `alvasta-pro logs` falls back to the configured local log file automatically. Explicit `--url` targets do not use this fallback.
