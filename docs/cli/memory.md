---
summary: "CLI reference for `alvasta-pro memory` (status/index/search/promote/promote-explain/rem-harness)"
read_when:
...
```bash
alvasta-pro memory status
alvasta-pro memory status --deep
alvasta-pro memory status --fix
alvasta-pro memory index --force
alvasta-pro memory search "meeting notes"
alvasta-pro memory search --query "deployment" --max-results 20
alvasta-pro memory promote --limit 10 --min-score 0.75
alvasta-pro memory promote --apply
alvasta-pro memory promote --json --min-recall-count 0 --min-unique-queries 0
alvasta-pro memory promote-explain "router vlan"
alvasta-pro memory promote-explain "router vlan" --json
alvasta-pro memory rem-harness
alvasta-pro memory rem-harness --json
alvasta-pro memory status --json
alvasta-pro memory status --deep --index
alvasta-pro memory status --deep --index --verbose
alvasta-pro memory status --agent main
alvasta-pro memory index --agent main --verbose
```
...
```bash
alvasta-pro memory promote [--apply] [--limit <n>] [--include-promoted]
```
...
- When dreaming is enabled, `memory-core` auto-manages one cron job that runs a full sweep (`light -> REM -> deep`) in the background (no manual `alvasta-pro cron add` required).
- `--agent <id>`: scope to a single agent (default: the default agent).
...
```bash
alvasta-pro memory promote-explain <selector> [--agent <id>] [--include-promoted] [--json]
```
...
```bash
alvasta-pro memory rem-harness [--agent <id>] [--include-promoted] [--json]
```
