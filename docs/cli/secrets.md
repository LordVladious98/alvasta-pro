---
summary: "CLI reference for `alvasta-pro secrets` (reload, audit, configure, apply)"
read_when:
...
Use `alvasta-pro secrets` to manage SecretRefs and keep the active runtime snapshot healthy.
...
```bash
alvasta-pro secrets reload
alvasta-pro secrets reload --json
alvasta-pro secrets reload --url ws://127.0.0.1:18789 --token <token>
```
...
Scan Alvasta Pro state for:

- plaintext secret storage
- unresolved refs
- precedence drift (`auth-profiles.json` credentials shadowing `alvasta-pro.json` refs)
...
```bash
alvasta-pro secrets audit
alvasta-pro secrets audit --check
alvasta-pro secrets audit --json
alvasta-pro secrets audit --allow-exec
```
...
```bash
alvasta-pro secrets configure
alvasta-pro secrets configure --plan-out /tmp/alvasta-pro-secrets-plan.json
alvasta-pro secrets configure --apply --yes
alvasta-pro secrets configure --providers-only
alvasta-pro secrets configure --skip-provider-setup
alvasta-pro secrets configure --agent ops
alvasta-pro secrets configure --json
```
...
- `configure` targets secret-bearing fields in `alvasta-pro.json` plus `auth-profiles.json` for the selected agent scope.
...
- On Windows, if ACL verification is unavailable for a provider path, Alvasta Pro fails closed. For trusted paths only, set `allowInsecurePath: true` on that provider to bypass path security checks.
...
alvasta-pro secrets apply --from /tmp/alvasta-pro-secrets-plan.json --json
```
...
- `alvasta-pro.json` (SecretRef targets + provider upserts/deletes)
- `auth-profiles.json` (provider-target scrubbing)
- legacy `auth.json` residues
- `~/.alvasta-pro/.env` known secret keys whose values were migrated
