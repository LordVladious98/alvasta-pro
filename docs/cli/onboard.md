---
summary: "CLI reference for `alvasta-pro onboard` (interactive onboarding)"
read_when:
...
```bash
alvasta-pro onboard
alvasta-pro onboard --flow quickstart
alvasta-pro onboard --flow manual
alvasta-pro onboard --mode remote --remote-url wss://gateway-host:18789
```
...
- `--install-daemon` starts the managed gateway install path first. Without it, you must already have a local gateway running, for example `alvasta-pro gateway run`.
...
- Fastest first chat: `alvasta-pro dashboard` (Control UI, no channel setup).
