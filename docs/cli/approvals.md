---
summary: "CLI reference for `alvasta-pro approvals` (exec approvals for gateway or node hosts)"
read_when:
...
# `alvasta-pro approvals`

Manage exec approvals for the **local host**, **gateway host**, or a **node host**.
By default, commands target the local approvals file on disk. Use `--gateway` to target the gateway, or `--node` to target a specific node.

Alias: `alvasta-pro exec-approvals`
...
```bash
alvasta-pro approvals get
alvasta-pro approvals get --node <id|name|ip>
alvasta-pro approvals get --gateway
```

`alvasta-pro approvals get` now shows the effective exec policy for local, gateway, and node targets:
...
```bash
alvasta-pro approvals set --file ./exec-approvals.json
alvasta-pro approvals set --stdin <<'EOF'
{ version: 1, defaults: { security: "full", ask: "off" } }
EOF
alvasta-pro approvals set --node <id|name|ip> --file ./exec-approvals.json
alvasta-pro approvals set --gateway --file ./exec-approvals.json
```
...
This changes the **host approvals file** only. To keep the requested Alvasta Pro policy aligned, also set:
...
```bash
alvasta-pro approvals allowlist add "~/Projects/**/bin/rg"
alvasta-pro approvals allowlist add --agent main --node <id|name|ip> "/usr/bin/uptime"
alvasta-pro approvals allowlist add --agent "*" "/usr/bin/uname"

alvasta-pro approvals allowlist remove "~/Projects/**/bin/rg"
```
...
- `--node` uses the same resolver as `alvasta-pro nodes` (id, name, ip, or id prefix).
- `--agent` defaults to `"*"`, which applies to all agents.
- The node host must advertise `system.execApprovals.get/set` (macOS app or headless node host).
- Approvals files are stored per host at `~/.alvasta-pro/exec-approvals.json`.
