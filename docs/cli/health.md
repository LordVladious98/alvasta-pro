---
summary: "CLI reference for `alvasta-pro health` (gateway health snapshot via RPC)"
read_when:
...
alvasta-pro health --timeout 2500
alvasta-pro health --verbose
alvasta-pro health --debug
```

Notes:

- Default `alvasta-pro health` asks the running gateway for its health snapshot. When the
