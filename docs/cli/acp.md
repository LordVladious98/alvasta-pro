...
Run the [Agent Client Protocol (ACP)](https://agentclientprotocol.com/) bridge that talks to an Alvasta Pro Gateway.

This command speaks ACP over stdio for IDEs and forwards prompts to the Gateway
over WebSocket. It keeps ACP sessions mapped to Gateway session keys.

`alvasta-pro acp` is a Gateway-backed ACP bridge, not a full ACP-native editor
runtime. It focuses on session routing, prompt delivery, and basic streaming
updates.

If you want an external MCP client to talk directly to Alvasta Pro channel
conversations instead of hosting an ACP harness session, use
[`alvasta-pro mcp serve`](/cli/mcp) instead.

## What this is not

This page is often confused with ACP harness sessions.

`alvasta-pro acp` means:

- Alvasta Pro acts as an ACP server
- an IDE or ACP client connects to Alvasta Pro
- Alvasta Pro forwards that work into a Gateway session

This is different from [ACP Agents](/tools/acp-agents), where Alvasta Pro runs an
external harness such as Codex or Claude Code through `acpx`.

Quick rule:

- editor/client wants to talk ACP to Alvasta Pro: use `alvasta-pro acp`
- Alvasta Pro should launch Codex/Claude/Gemini as an ACP harness: use `/acp spawn` and [ACP Agents](/tools/acp-agents)
...
| Per-session MCP servers (`mcpServers`)                                | Unsupported | Bridge mode rejects per-session MCP server requests. Configure MCP on the Alvasta Pro gateway or agent instead.                                                                                                                                     |
...
# Remote Gateway
alvasta-pro acp --url wss://gateway-host:18789 --token <token>

# Remote Gateway (token from file)
alvasta-pro acp --url wss://gateway-host:18789 --token-file ~/.alvasta-pro/gateway.token

# Attach to an existing session key
alvasta-pro acp --session agent:main:main

# Attach by label (must already exist)
alvasta-pro acp --session-label "support inbox"

# Reset the session key before the first prompt
alvasta-pro acp --session agent:main:main --reset-session
```
...
```bash
alvasta-pro acp client

# Point the spawned bridge at a remote Gateway
alvasta-pro acp client --server-args --url wss://gateway-host:18789 --token-file ~/.alvasta-pro/gateway.token

# Override the server command (default: alvasta-pro)
alvasta-pro acp client --server "node" --server-args alvasta-pro.mjs acp --url ws://127.0.0.1:19001
```
...
- This ACP bridge policy is separate from ACPX harness permissions. If you run Alvasta Pro through the `acpx` backend, `plugins.entries.acpx.config.permissionMode=approve-all` is the break-glass “yolo” switch for that harness session.

## How to use this

Use ACP when an IDE (or other client) speaks Agent Client Protocol and you want
it to drive an Alvasta Pro Gateway session.

1. Ensure the Gateway is running (local or remote).
2. Configure the Gateway target (config or flags).
3. Point your IDE to run `alvasta-pro acp` over stdio.

Example config (persisted):

```bash
alvasta-pro config set gateway.remote.url wss://gateway-host:18789
alvasta-pro config set gateway.remote.token <token>
```
...
```bash
alvasta-pro acp --session agent:main:main
alvasta-pro acp --session agent:design:main
alvasta-pro acp --session agent:qa:bug-123
```
...
If you want ACPX-backed sessions to see Alvasta Pro plugin tools, enable the
...
Alvasta Pro bot over ACP, use `acpx` with its built-in `alvasta-pro` target.

Typical flow:

1. Run the Gateway and make sure the ACP bridge can reach it.
2. Point `acpx alvasta-pro` at `alvasta-pro acp`.
3. Target the Alvasta Pro session key you want the coding agent to use.

Examples:

```bash
# One-shot request into your default Alvasta Pro ACP session
acpx alvasta-pro exec "Summarize the active Alvasta Pro session state."

# Persistent named session for follow-up turns
acpx alvasta-pro sessions ensure --name codex-bridge
acpx alvasta-pro -s codex-bridge --cwd /path/to/repo \
  "Ask my Alvasta Pro work agent for recent context relevant to this repo."
```

If you want `acpx alvasta-pro` to target a specific Gateway and session key every
time, override the `alvasta-pro` agent command in `~/.acpx/config.json`:

```json
{
  "agents": {
    "alvasta-pro": {
      "command": "env OPENCLAW_HIDE_BANNER=1 OPENCLAW_SUPPRESS_NOTES=1 alvasta-pro acp --url ws://127.0.0.1:18789 --token-file ~/.alvasta-pro/gateway.token --session agent:main:main"
    }
  }
}
```

For a repo-local Alvasta Pro checkout, use the direct CLI entrypoint instead of the
dev runner so the ACP stream stays clean. For example:

```bash
env OPENCLAW_HIDE_BANNER=1 OPENCLAW_SUPPRESS_NOTES=1 node alvasta-pro.mjs acp ...
```

This is the easiest way to let Codex, Claude Code, or another ACP-aware client
pull contextual information from an Alvasta Pro agent without scraping a terminal.
...
In Zed, open the Agent panel and select “Alvasta Pro ACP” to start a thread.
...
- `alvasta-pro acp client` sets `OPENCLAW_SHELL=acp-client` on the spawned bridge process.

### `acp client` options

- `--cwd <dir>`: working directory for the ACP session.
- `--server <command>`: ACP server command (default: `alvasta-pro`).
