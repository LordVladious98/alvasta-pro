...
Bind a Task Flow runtime to an existing Alvasta Pro session key or trusted tool
...
trusted Alvasta Pro session key from your own binding layer. Do not bind from raw
user input.
...
const result = await api.runtime.webSearch.search({
  config: api.config,
  args: { query: "Alvasta Pro plugin SDK", count: 5 },
});
```
