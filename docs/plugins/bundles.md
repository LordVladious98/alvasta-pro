---
summary: "Install and use Codex, Claude, and Cursor bundles as Alvasta Pro plugins"
read_when:
  - You want to install a Codex, Claude, or Cursor-compatible bundle
  - You need to understand how Alvasta Pro maps bundle content into native features
...
Alvasta Pro can install plugins from three external ecosystems: **Codex**, **Claude**,
and **Cursor**. These are called **bundles** — content and metadata packs that
Alvasta Pro maps into native features like skills, hooks, and MCP tools.

<Info>
  Bundles are **not** the same as native Alvasta Pro plugins. Native plugins run
...
of requiring authors to rewrite them as native Alvasta Pro plugins, Alvasta Pro
...
<Steps>
  <Step title="Install from a directory, archive, or marketplace">
    ```bash
    # Local directory
    alvasta-pro plugins install ./my-bundle

    # Archive
    alvasta-pro plugins install ./my-bundle.tgz
...
## What Alvasta Pro maps from bundles

Not every bundle feature runs in Alvasta Pro today. Here is what works and what
...
| ------------- | ------------------------------------------------------------------------------------------- | -------------- |
| Skill content | Bundle skill roots load as normal Alvasta Pro skills                                           | All formats    |
| Commands      | `commands/` and `.cursor/commands/` treated as skill roots                                  | Claude, Cursor |
| Hook packs    | Alvasta Pro-style `HOOK.md` + `handler.ts` layouts                                             | Codex          |
...
- bundle skill roots load as normal Alvasta Pro skill roots
- Claude `commands` roots are treated as additional skill roots
- Cursor `.cursor/commands` roots are treated as additional skill roots

This means Claude markdown command files work through the normal Alvasta Pro skill
loader. Cursor command markdown works through the same path.

#### Hook packs

- bundle hook roots work **only** when they use the normal Alvasta Pro hook-pack
...
- Alvasta Pro merges bundle MCP config into the effective embedded Pi settings as
  `mcpServers`
- Alvasta Pro exposes supported bundle MCP tools during embedded Pi agent turns by
...
- `transport` may be set to `"streamable-http"` or `"sse"`; when omitted, Alvasta Pro uses `sse`
...
Alvasta Pro registers bundle MCP tools with provider-safe names in the form
...
- Claude `settings.json` is imported as default embedded Pi settings when the
  bundle is enabled
- Alvasta Pro sanitizes shell override keys before applying them
...
- Alvasta Pro loads `.lsp.json` plus any manifest-declared `lspServers` paths
...
- only supported stdio-backed LSP servers are runnable today; unsupported
  transports still show up in `alvasta-pro plugins inspect <id>`

### Detected but not executed

These are recognized and shown in diagnostics, but Alvasta Pro does not run them:
...
<AccordionGroup>
  <Accordion title="Codex bundles">
    ...
    Codex bundles fit Alvasta Pro best when they use skill roots and Alvasta Pro-style
...
Alvasta Pro checks for native plugin format first:

1. `alvasta-pro.plugin.json` or valid `package.json` with `alvasta-pro.extensions` — treated as **native plugin**
2. Bundle markers (`.codex-plugin/`, `.claude-plugin/`, or default Claude/Cursor layout) — treated as **bundle**

If a directory contains both, Alvasta Pro uses the native path. This prevents
...
- Alvasta Pro does **not** load arbitrary bundle runtime modules in-process
...
<AccordionGroup>
  <Accordion title="Bundle is detected but capabilities do not run">
    Run `alvasta-pro plugins inspect <id>`. If a capability is listed but marked as
  ...
  <Accordion title="Claude settings do not apply">
    Only embedded Pi settings from `settings.json` are supported. Alvasta Pro does
  ...
  <Accordion title="Claude hooks do not execute">
    ...
    Alvasta Pro hook-pack layout or ship a native plugin.
  </Accordion>
</AccordionGroup>
