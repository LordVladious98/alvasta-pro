...
read_when:
  ...
  - You are defining plugin config schemas or package.json alvasta-pro metadata
---

# Plugin Setup and Config

Reference for plugin packaging (`package.json` metadata), manifests
(`alvasta-pro.plugin.json`), setup entries, and config schemas.
...
Your `package.json` needs an `alvasta-pro` field that tells the plugin system what
...
```json
{
  "name": "@myorg/alvasta-pro-my-channel",
  "version": "1.0.0",
  "type": "module",
  "alvasta-pro": {
    "extensions": ["./index.ts"],
...
```json alvasta-pro-clawhub-package.json
{
  "name": "@myorg/alvasta-pro-my-plugin",
  "version": "1.0.0",
  "type": "module",
  "alvasta-pro": {
    "extensions": ["./index.ts"],
    ...
    "build": {
      "alvasta-proVersion": "2026.3.24-beta.2",
...
`alvasta-pro.channel` is cheap package metadata for channel discovery and setup
...
`alvasta-pro.install` is package metadata, not manifest metadata.
...
| `minHostVersion`             | `string`             | Minimum supported Alvasta Pro version in the form `>=x.y.z`.                        |
...
still fails closed and tells the operator to run `alvasta-pro doctor --fix`.
...
When enabled, Alvasta Pro loads only `setupEntry` during the pre-listen startup
...
Every native plugin must ship an `alvasta-pro.plugin.json` in the package root.
Alvasta Pro uses this to validate config without executing plugin code.

```json
{
  "id": "my-plugin",
  "name": "My Plugin",
  "description": "Adds My Plugin capabilities to Alvasta Pro",
  "configSchema": {
    "type": "object",
...
Alvasta Pro loads when it only needs setup surfaces (onboarding, config repair,
...
**When Alvasta Pro uses `setupEntry` instead of the full entry:**
...
Use `buildChannelConfigSchema` from `alvasta-pro/plugin-sdk/core` to convert a
Zod schema into the `ChannelConfigSchema` wrapper that Alvasta Pro validates:

```typescript
import { z } from "zod";
import { buildChannelConfigSchema } from "alvasta-pro/plugin-sdk/core";
...
Channel plugins can provide interactive setup wizards for `alvasta-pro onboard`.
The wizard is a `ChannelSetupWizard` object on the `ChannelPlugin`:

```typescript
import type { ChannelSetupWizard } from "alvasta-pro/plugin-sdk/channel-setup";
...
helpers from `alvasta-pro/plugin-sdk/setup`: `createPromptParsedAllowFromForAccount(...)`,
...
`alvasta-pro/plugin-sdk/setup` instead of hand-rolling the same `status` object in
each plugin.

For optional setup surfaces that should only appear in certain contexts, use
`createOptionalChannelSetupSurface` from `alvasta-pro/plugin-sdk/channel-setup`:

```typescript
import { createOptionalChannelSetupSurface } from "alvasta-pro/plugin-sdk/channel-setup";

const setupSurface = createOptionalChannelSetupSurface({
  channel: "my-channel",
  label: "My Channel",
  npmSpec: "@myorg/alvasta-pro-my-channel",
  docsPath: "/channels/my-channel",
});
...
```bash
alvasta-pro plugins install @myorg/alvasta-pro-my-plugin
```

Alvasta Pro tries ClawHub first and falls back to npm automatically. You can also
force ClawHub explicitly:

```bash
alvasta-pro plugins install clawhub:@myorg/alvasta-pro-my-plugin   # ClawHub only
```
...
```bash
alvasta-pro plugins install <package-name>
```

<Info>
  For npm-sourced installs, `alvasta-pro plugins install` runs
