...
read_when:
  - You want to place an outbound voice call from Alvasta Pro
...
Voice calls for Alvasta Pro via a plugin. Supports outbound notifications and
...
- Use `alvasta-pro voicecall ...` or the `voice_call` tool
...
alvasta-pro plugins install "$PLUGIN_SRC"
...
- If older configs still use `provider: "log"`, `twilio.from`, or legacy `streaming.*` OpenAI keys, run `alvasta-pro doctor --fix` to rewrite them.
...
- Runtime fallback still accepts those old voice-call keys for now, but the rewrite path is `alvasta-pro doctor --fix` and the compat shim is temporary.
...
Legacy keys are still auto-migrated by `alvasta-pro doctor --fix`:
...
```bash
alvasta-pro voicecall call --to "+15555550123" --message "Hello from Alvasta Pro"
alvasta-pro voicecall start --to "+15555550123"   # alias for call
alvasta-pro voicecall continue --call-id <id> --message "Any questions?"
alvasta-pro voicecall speak --call-id <id> --message "One moment"
alvasta-pro voicecall end --call-id <id>
alvasta-pro voicecall status --call-id <id>
alvasta-pro voicecall tail
alvasta-pro voicecall latency                     # summarize turn latency from logs
