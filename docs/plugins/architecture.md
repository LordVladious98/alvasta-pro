...
read_when:
  - Building or debugging native Alvasta Pro plugins
...
This page covers the internal architecture of the Alvasta Pro plugin system.

## Public capability model

Capabilities are the public **native plugin** model inside Alvasta Pro. Every
native Alvasta Pro plugin registers against one or more capability types:
...
Alvasta Pro classifies every loaded plugin into a shape based on its actual
...
Use `alvasta-pro plugins inspect <id>` to see a plugin's shape and capability
...
When you run `alvasta-pro doctor` or `alvasta-pro plugins inspect <id>`, you may see
one of these labels:
...
signals also appear in `alvasta-pro status --all` and `alvasta-pro plugins doctor`.

## Architecture overview

Alvasta Pro's plugin system has four layers:

1. **Manifest + discovery**
   Alvasta Pro finds candidate plugins from configured paths, workspace roots,
   global extension roots, and bundled extensions. Discovery reads native
   `alvasta-pro.plugin.json` manifests plus supported bundle manifests first.
2. **Enablement + validation**
   Core decides whether a discovered plugin is enabled, disabled, blocked, or
   selected for an exclusive slot such as memory.
3. **Runtime loading**
   Native Alvasta Pro plugins are loaded in-process via jiti and register
...
4. **Surface consumption**
   The rest of Alvasta Pro reads the registry to expose tools, channels, provider
...
That keeps plugin-owned CLI code inside the plugin while still letting Alvasta Pro
...
That split lets Alvasta Pro validate config, explain missing/disabled plugins, and
...
normal chat actions. Alvasta Pro keeps one shared `message` tool in core, and
...
Alvasta Pro treats a native plugin as the ownership boundary for a **company** or a
**feature**, not as a grab bag of unrelated integrations.

That means:

- a company plugin should usually own all of that company's Alvasta Pro-facing
  surfaces
...
So if Alvasta Pro adds a new domain such as video, the first question is not
...
A company plugin should feel cohesive from the outside. If Alvasta Pro has shared
...
```ts
import type { Alvasta ProPluginDefinition } from "alvasta-pro/plugin-sdk/plugin-entry";
import {
  describeImageWithModel,
  transcribeOpenAiCompatibleAudio,
} from "alvasta-pro/plugin-sdk/media-understanding";

const plugin: Alvasta ProPluginDefinition = {
  id: "exampleai",
  name: "ExampleAI",
  register(api) {
...
Alvasta Pro already treats image/audio/video understanding as one shared
...
`Alvasta ProPluginApi`. That contract defines the supported registration points and
...
2. **contract tests**
   ...
   Alvasta Pro can assert ownership explicitly. Today this is used for model
   providers, speech providers, web search providers, and bundled registration
   ownership.

The practical effect is that Alvasta Pro knows, up front, which plugin owns which
...
- ad hoc runtime objects that are not part of `Alvasta ProPluginApi` or
  `api.runtime`
...
Native Alvasta Pro plugins run **in-process** with the Gateway. They are not
...
- a malicious native plugin is equivalent to arbitrary code execution inside
  the Alvasta Pro process

Compatible bundles are safer by default because Alvasta Pro currently treats them
...
name: `@alvasta-pro/<id>` by default, or an approved typed suffix such as
...
Alvasta Pro exports capabilities, not implementation convenience.
...
At startup, Alvasta Pro does roughly this:
...
The manifest is the control-plane source of truth. Alvasta Pro uses it to:
...
Alvasta Pro keeps short in-process caches for:
...
Alvasta Pro still owns the generic agent loop, failover, transcript handling, and
...
For model/provider plugins, Alvasta Pro calls hooks in this rough order.
...
| --  | _(built-in model lookup)_         | Alvasta Pro tries the normal registry/catalog path first                                                          | _(not a plugin hook)_                                                                                                                       |
...
that still runs on Alvasta Pro's normal inference loop.
...
- OpenRouter uses `catalog` plus `resolveDynamicModel` and
  ...
  model ids before Alvasta Pro's static catalog updates; it also uses
...
- The preferred ownership model is company-oriented: one vendor plugin can own
  text, speech, image, and future media providers as Alvasta Pro adds those
...
- Alvasta Pro only honors those override fields for trusted callers.
...
const result = await api.runtime.webSearch.search({
  config: api.config,
  args: {
    query: "Alvasta Pro plugin runtime helpers",
    count: 5,
  },
});
```
...
- `auth: "gateway"` routes run inside a Gateway request runtime scope, but that scope is intentionally conservative:
  - shared-secret bearer auth (`gateway.auth.mode = "token"` / `"password"`) keeps plugin-route runtime scopes pinned to `operator.write`, even if the caller sends `x-alvasta-pro-scopes`
  - trusted identity-bearing HTTP modes (for example `trusted-proxy` or `gateway.auth.mode = "none"` on a private ingress) honor `x-alvasta-pro-scopes` only when the header is explicitly present
  - if `x-alvasta-pro-scopes` is absent on those identity-bearing plugin-route requests, runtime scope falls back to `operator.write`
- Practical rule: do not assume a gateway-auth plugin route is an implicit admin surface. If your route needs admin-only behavior, require an identity-bearing auth mode and document the explicit `x-alvasta-pro-scopes` header contract.

## Plugin SDK import paths

Use SDK subpaths instead of the monolithic `alvasta-pro/plugin-sdk` import when
authoring plugins:

- `alvasta-pro/plugin-sdk/plugin-entry` for plugin registration primitives.
- `alvasta-pro/plugin-sdk/core` for the generic shared plugin-facing contract.
- `alvasta-pro/plugin-sdk/config-schema` for the root `alvasta-pro.json` Zod schema
  export (`Alvasta ProSchema`).
- Stable channel primitives such as `alvasta-pro/plugin-sdk/channel-setup`,
  `alvasta-pro/plugin-sdk/setup-runtime`,
  `alvasta-pro/plugin-sdk/setup-adapter-runtime`,
  `alvasta-pro/plugin-sdk/setup-tools`,
  `alvasta-pro/plugin-sdk/channel-pairing`,
  `alvasta-pro/plugin-sdk/channel-contract`,
  `alvasta-pro/plugin-sdk/channel-feedback`,
  `alvasta-pro/plugin-sdk/channel-inbound`,
  `alvasta-pro/plugin-sdk/channel-lifecycle`,
  `alvasta-pro/plugin-sdk/channel-reply-pipeline`,
  `alvasta-pro/plugin-sdk/command-auth`,
  `alvasta-pro/plugin-sdk/secret-input`, and
  `alvasta-pro/plugin-sdk/webhook-ingress` for shared setup/auth/reply/webhook
  ...
  `CONFIG_DIR`).
- Domain subpaths such as `alvasta-pro/plugin-sdk/channel-config-helpers`,
  `alvasta-pro/plugin-sdk/allow-from`,
  `alvasta-pro/plugin-sdk/channel-config-schema`,
  `alvasta-pro/plugin-sdk/telegram-command-config`,
  `alvasta-pro/plugin-sdk/channel-policy`,
  `alvasta-pro/plugin-sdk/approval-runtime`,
  `alvasta-pro/plugin-sdk/config-runtime`,
  `alvasta-pro/plugin-sdk/infra-runtime`,
  `alvasta-pro/plugin-sdk/agent-runtime`,
  `alvasta-pro/plugin-sdk/lazy-runtime`,
  `alvasta-pro/plugin-sdk/reply-history`,
  `alvasta-pro/plugin-sdk/routing`,
  `alvasta-pro/plugin-sdk/status-helpers`,
  `alvasta-pro/plugin-sdk/text-runtime`,
  `alvasta-pro/plugin-sdk/runtime-store`, and
  `alvasta-pro/plugin-sdk/directory-runtime` for shared runtime/config helpers.
...
- `alvasta-pro/plugin-sdk/channel-runtime` is deprecated and remains only as a
...
- Bundled extension internals remain private. External plugins should use only
  `alvasta-pro/plugin-sdk/*` subpaths. Alvasta Pro core/test code may use the repo
...
- Facade-loaded public entry points prefer the active runtime config snapshot
  ...
  Alvasta Pro is not yet serving a runtime snapshot.
...
- Avoid the root `alvasta-pro/plugin-sdk` barrel for new code.
- Prefer the narrow stable primitives first. The newer setup/pairing/reply/
  ...
  Target parsing/matching belongs on `alvasta-pro/plugin-sdk/channel-targets`.
  Message action gates and reaction message-id helpers belong on
  `alvasta-pro/plugin-sdk/channel-actions`.
- Bundled extension-specific helper barrels are not stable by default. If a
  ...
  `alvasta-pro/plugin-sdk/<extension>`.
- New shared helper seams should be generic, not channel-branded. Shared target
  parsing belongs on `alvasta-pro/plugin-sdk/channel-targets`; channel-specific
...
`alvasta-pro/plugin-sdk/directory-runtime`.
...
`catalog.run(...)` returns the same shape Alvasta Pro writes into
`models.providers`:
...
`catalog.order` controls when a plugin's catalog merges relative to Alvasta Pro's
...
- if both `catalog` and `discovery` are registered, Alvasta Pro uses `catalog`
...
- Read-only command paths such as `alvasta-pro status`, `alvasta-pro status --all`,
  `alvasta-pro channels status`, `alvasta-pro channels resolve`, and doctor/config
...
A plugin directory may include a `package.json` with `alvasta-pro.extensions`:
...
Security guardrail: every `alvasta-pro.extensions` entry must stay inside the plugin
directory after symlink resolution. Entries that escape the package directory are
rejected.

Security note: `alvasta-pro plugins install` installs plugin dependencies with
...
Optional: `alvasta-pro.setupEntry` can point at a lightweight setup-only module.
When Alvasta Pro needs setup surfaces for a disabled channel plugin, or
...
Optional: `alvasta-pro.startup.deferConfiguredChannelFullLoadUntilAfterListen`
...
this flag. Keep the plugin on the default behavior and let Alvasta Pro load the
...
Channel plugins can advertise setup/discovery metadata via `alvasta-pro.channel` and
install hints via `alvasta-pro.install`. This keeps the core catalog data-free.

Example:

```json
{
  "name": "@alvasta-pro/nextcloud-talk",
  "alvasta-pro": {
    "extensions": ["./index.ts"],
    "channel": {
      "id": "nextcloud-talk",
    ...
    "install": {
      "npmSpec": "@alvasta-pro/nextcloud-talk",
...
Useful `alvasta-pro.channel` fields beyond the minimal example:
...
Alvasta Pro can also merge **external channel catalogs** (for example, an MPM
registry export). Drop a JSON file at one of:

- `~/.alvasta-pro/mpm/plugins.json`
- `~/.alvasta-pro/mpm/catalog.json`
- `~/.alvasta-pro/plugins/catalog.json`

Or point `OPENCLAW_PLUGIN_CATALOG_PATHS` (or `OPENCLAW_MPM_CATALOG_PATHS`) at
one or more JSON files (comma/semicolon/`PATH`-delimited). Each file should
contain `{ "entries": [ { "name": "@scope/pkg", "alvasta-pro": { "channel": {...}, "install": {...} } } ] }`. The parser also accepts `"packages"` or `"plugins"` as legacy aliases for the `"entries"` key.
...
```ts
import { buildMemorySystemPromptAddition } from "alvasta-pro/plugin-sdk/core";
...
} from "alvasta-pro/plugin-sdk/core";
...
2. add typed plugin registration/runtime surfaces
   Extend `Alvasta ProPluginApi` and/or `api.runtime` with the smallest useful
...
This is how Alvasta Pro stays opinionated without becoming hardcoded to one
