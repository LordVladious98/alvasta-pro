...
summary: "Create your first Alvasta Pro plugin in minutes"
read_when:
  - You want to create a new Alvasta Pro plugin
  - You need a quick-start for plugin development
  - You are adding a new channel, provider, tool, or other capability to Alvasta Pro
---
...
You do not need to add your plugin to the Alvasta Pro repository. Publish to
[ClawHub](/tools/clawhub) or npm and users install with
`alvasta-pro plugins install <package-name>`. Alvasta Pro tries ClawHub first and
...
<CardGroup cols={3}>
  <Card title="Channel plugin" icon="messages-square" href="/plugins/sdk-channel-plugins">
    Connect Alvasta Pro to a messaging platform (Discord, IRC, etc.)
  </Card>
...
`alvasta-pro/plugin-sdk/channel-setup`. It produces a setup adapter + wizard pair
...
<Steps>
  <Step title="Create the package and manifest">
    ...
    ```json alvasta-pro.plugin.json
    {
      "id": "my-plugin",
      "name": "My Plugin",
      "description": "Adds a custom tool to Alvasta Pro",
      "configSchema": {
        "type": "object",
  ...
  <Step title="Write the entry point">
    ...
    ```typescript
    // index.ts
    import { definePluginEntry } from "alvasta-pro/plugin-sdk/plugin-entry";
  ...
  <Step title="Test and publish">
    ...
    **External plugins:** validate and publish with ClawHub, then install:
    ...
    alvasta-pro plugins install clawhub:@myorg/alvasta-pro-my-plugin
    ```

    Alvasta Pro also checks ClawHub before npm for bare package specs like
    `@myorg/alvasta-pro-my-plugin`.
...
The `/approve` command handles both exec and plugin approvals with bounded fallback: when an exec approval id is not found, Alvasta Pro retries the same id through plugin approvals. Plugin approval forwarding can be configured independently via `approvals.plugin` in config.

If custom approval plumbing needs to detect that same bounded fallback case,
prefer `isApprovalNotFoundError` from `alvasta-pro/plugin-sdk/error-runtime`
...
Always import from focused `alvasta-pro/plugin-sdk/<subpath>` paths:

```typescript
import { definePluginEntry } from "alvasta-pro/plugin-sdk/plugin-entry";
import { createPluginRuntimeStore } from "alvasta-pro/plugin-sdk/runtime-store";

// Wrong: monolithic root (deprecated, will be removed)
import { ... } from "alvasta-pro/plugin-sdk";
```
...
package-root seam instead of promoting it into `alvasta-pro/plugin-sdk/*`.

Some generated `alvasta-pro/plugin-sdk/<bundled-id>` helper seams still exist for
...
<Check>**package.json** has correct `alvasta-pro` metadata</Check>
<Check>**alvasta-pro.plugin.json** manifest is present and valid</Check>
...
1. Watch for GitHub release tags on [alvasta-pro/alvasta-pro](https://github.com/alvasta-pro/alvasta-pro/releases) and subscribe via `Watch` > `Releases`. Beta tags look like `v2026.3.N-beta.1`. You can also turn on notifications for the official Alvasta Pro X account [@alvasta-pro](https://x.com/alvasta-pro) for release announcements.
