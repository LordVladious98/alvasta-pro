...
summary: "Step-by-step guide to building a model provider plugin for Alvasta Pro"
read_when:
  - You are building a new model provider plugin
  - You want to add an OpenAI-compatible proxy or custom LLM to Alvasta Pro
...
(LLM) to Alvasta Pro. By the end you will have a provider with a model catalog,
...
<Steps>
  ...
  <Step title="Package and manifest">
    ...
    ```json package.json
    {
      "name": "@myorg/alvasta-pro-acme-ai",
      "version": "1.0.0",
      "type": "module",
      "alvasta-pro": {
        "extensions": ["./index.ts"],
    ...
    The manifest declares `providerAuthEnvVars` so Alvasta Pro can detect
    credentials without loading your plugin runtime. `modelSupport` is optional
    and lets Alvasta Pro auto-load your provider plugin from shorthand model ids
    like `acme-large` before runtime hooks exist. If you publish the
    provider on ClawHub, those `alvasta-pro.compat` and `alvasta-pro.build` fields
  ...
  <Step title="Register the provider">
    ...
    import { createProviderApiKeyAuthMethod } from "alvasta-pro/plugin-sdk/provider-auth";
    ...
    `alvasta-pro onboard --acme-ai-api-key <key>` and select
    ...
    ```typescript
    import { defineSingleProviderPluginEntry } from "alvasta-pro/plugin-sdk/provider-entry";
    ...
    `alvasta-pro/plugin-sdk/provider-onboard`. The narrowest helpers are
    ...
    `alvasta-pro/plugin-sdk/provider-catalog-shared` instead of hardcoding
  ...
  <Step title="Add runtime hooks (as needed)">
    ...
    ```typescript
    import { buildProviderReplayFamilyHooks } from "alvasta-pro/plugin-sdk/provider-model-shared";
    import { buildProviderStreamFamilyHooks } from "alvasta-pro/plugin-sdk/provider-stream";
    import { buildProviderToolCompatFamilyHooks } from "alvasta-pro/plugin-sdk/provider-tools";
    ...
    `alvasta-pro/plugin-sdk/provider-model-shared` also exports the replay-family
    ...
    `alvasta-pro/plugin-sdk/provider-stream` exposes both the family builder and
    ...
    example: `@alvasta-pro/anthropic-provider` exports
    ...
    `alvasta-pro/plugin-sdk/provider-tools` currently exposes one shared
    ...
    - `@alvasta-pro/openai-provider`: `api.ts` exports provider builders,
      default-model helpers, and realtime provider builders
    - `@alvasta-pro/openrouter-provider`: `api.ts` exports the provider builder
    ...
    <Accordion title="All available provider hooks">
      Alvasta Pro calls hooks in this order. Most providers only use 2-3:
  ...
  <Step title="Add extra capabilities (optional)">
    ...
    Alvasta Pro classifies this as a **hybrid-capability** plugin. This is the
...
├── package.json              # alvasta-pro.providers metadata
├── alvasta-pro.plugin.json      # Manifest with providerAuthEnvVars
