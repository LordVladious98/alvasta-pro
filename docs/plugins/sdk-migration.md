...
read_when:
  ...
  - You maintain an external Alvasta Pro plugin
---

# Plugin SDK Migration

Alvasta Pro has moved from a broad backwards-compatibility layer to a modern plugin
...
- **`alvasta-pro/plugin-sdk/compat`** — a single import that re-exported dozens of
  helpers. It was introduced to keep older hook-based plugins working while the
  new plugin architecture was being built.
- **`alvasta-pro/extension-api`** — a bridge that gave plugins direct access to
...
The modern plugin SDK fixes this: each import path (`alvasta-pro/plugin-sdk/\<subpath\>`)
is a small, self-contained module with a clear purpose and documented contract.

Legacy provider convenience seams for bundled channels are also gone. Imports
such as `alvasta-pro/plugin-sdk/slack`, `alvasta-pro/plugin-sdk/discord`,
`alvasta-pro/plugin-sdk/signal`, `alvasta-pro/plugin-sdk/whatsapp`,
channel-branded helper seams, and
`alvasta-pro/plugin-sdk/telegram-core` were private mono-repo shortcuts, not
...
<Steps>
  <Step title="Audit Windows wrapper fallback behavior">
    If your plugin uses `alvasta-pro/plugin-sdk/windows-spawn`, unresolved Windows
  ...
  <Step title="Find deprecated imports">
    ...
    grep -r "alvasta-pro/extension-api" my-plugin/
    ```
  ...
  <Step title="Replace with focused imports">
    ...
    } from "alvasta-pro/plugin-sdk/compat";

    // After (modern focused imports)
    import { createChannelReplyPipeline } from "alvasta-pro/plugin-sdk/channel-reply-pipeline";
    import { createPluginRuntimeStore } from "alvasta-pro/plugin-sdk/runtime-store";
    import { resolveControlCommandGate } from "alvasta-pro/plugin-sdk/command-auth";
    ```
    ...
    import { runEmbeddedPiAgent } from "alvasta-pro/extension-api";
...
<Accordion title="Common import path table">
  ...
  | `plugin-sdk/config-schema` | Root config schema export | `Alvasta ProSchema` |
...
```bash
OPENCLAW_SUPPRESS_PLUGIN_SDK_COMPAT_WARNING=1 alvasta-pro gateway run
OPENCLAW_SUPPRESS_EXTENSION_API_WARNING=1 alvasta-pro gateway run
```
