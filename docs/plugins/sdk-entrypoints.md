...
**Import:** `alvasta-pro/plugin-sdk/plugin-entry`
...
| `kind`         | `string`                                                         | No       | —                   |
| `configSchema` | `Alvasta ProPluginConfigSchema \| () => Alvasta ProPluginConfigSchema` | No       | Empty object schema |
| `register`     | `(api: Alvasta ProPluginApi) => void`                               | Yes      | —                   |

- `id` must match your `alvasta-pro.plugin.json` manifest.
- `kind` is for exclusive slots: `"memory"` or `"context-engine"`.
- `configSchema` can be a function for lazy evaluation.
- Alvasta Pro resolves and memoizes that schema on first access, so expensive schema
  builders only run once.

## `defineChannelPluginEntry`

**Import:** `alvasta-pro/plugin-sdk/channel-core`
...
| `registerCliMetadata` | `(api: Alvasta ProPluginApi) => void`                               | No       | —                   |
| `registerFull`        | `(api: Alvasta ProPluginApi) => void`                               | No       | —                   |
...
- Like `definePluginEntry`, `configSchema` can be a lazy factory and Alvasta Pro
...
Alvasta Pro loads this instead of the full entry when a channel is disabled,
...
- `alvasta-pro/plugin-sdk/setup-runtime` for runtime-safe setup helpers such as
  import-safe setup patch adapters, lookup-note output,
  `promptResolvedAllowFrom`, `splitSetupEntries`, and delegated setup proxies
- `alvasta-pro/plugin-sdk/channel-setup` for optional-install setup surfaces
- `alvasta-pro/plugin-sdk/setup-tools` for setup/install CLI/archive/docs helpers
...
- use `descriptors` when the registrar owns one or more root commands and you
  want Alvasta Pro to lazy-load the real CLI module on first invocation
...
Alvasta Pro classifies loaded plugins by their registration behavior:
...
Use `alvasta-pro plugins inspect <id>` to see a plugin's shape.
