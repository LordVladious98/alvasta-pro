...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in Alvasta Pro
...
Running Alvasta Pro via Node with `tsx` fails at startup with:

```
[alvasta-pro] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `alvasta-pro status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch alvasta-pro.mjs status
  ```

- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node alvasta-pro.mjs status` works on Node 25.
