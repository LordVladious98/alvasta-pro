...
read_when:
  - You want to connect Alvasta Pro to IRC channels or DMs
  - You are configuring IRC allowlists, group policy, or mention gating
---

# IRC

Use IRC when you want Alvasta Pro in classic channels (`#room`) and direct messages.
IRC ships as an extension plugin, but it is configured in the main config under `channels.irc`.

## Quick start

1. Enable IRC config in `~/.alvasta-pro/alvasta-pro.json`.
2. Set at least:
...
```json5
{
  channels: {
    irc: {
      ...
      port: 6697,
      tls: true,
      nick: "alvasta-pro-bot",
      channels: ["#alvasta-pro"],
    },
  },
}
```
...
Even if a channel is allowed (via `groupPolicy` + `groups`) and the sender is allowed, Alvasta Pro defaults to **mention-gating** in group contexts.
