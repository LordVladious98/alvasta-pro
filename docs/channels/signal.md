...
- Alvasta Pro installed on your server (Linux flow below tested on Ubuntu 24).
...
3. Choose one setup path:
   - **Path A (QR link):** `signal-cli link -n "Alvasta Pro"` and scan with Signal.
   - **Path B (SMS register):** register a dedicated number with captcha + SMS verification.
4. Configure Alvasta Pro and restart the gateway.
5. Send a first DM and approve pairing (`alvasta-pro pairing approve signal <CODE>`).
...
2. Link a bot account:
   - `signal-cli link -n "Alvasta Pro"` then scan the QR in Signal.
...
4. Configure Alvasta Pro, restart gateway, verify channel:
...
5. Pair your DM sender:
   ...
   - Approve code on the server: `alvasta-pro pairing approve signal <PAIRING_CODE>`.
...
If you want to manage `signal-cli` yourself (slow JVM cold starts, container init, or shared CPUs), run the daemon separately and point Alvasta Pro at it:
...
This skips auto-spawn and the startup wait inside Alvasta Pro. For slow starts when auto-spawning, set `channels.signal.startupTimeoutMs`.
...
- Approve via:
  - `alvasta-pro pairing list signal`
  - `alvasta-pro pairing approve signal <CODE>`
...
- **Typing indicators**: Alvasta Pro sends typing signals via `signal-cli sendTyping` and refreshes them while a reply is running.
- **Read receipts**: when `channels.signal.sendReadReceipts` is true, Alvasta Pro forwards read receipts for allowed DMs.
...
- Config validation errors after edits: run `alvasta-pro doctor --fix`.
...
alvasta-pro pairing list signal
pgrep -af signal-cli
grep -i "signal" "/tmp/alvasta-pro/alvasta-pro-$(date +%Y-%m-%d).log" | tail -20
```
