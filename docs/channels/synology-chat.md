---
summary: "Synology Chat webhook setup and Alvasta Pro config"
read_when:
  - Setting up Synology Chat with Alvasta Pro
...
Synology Chat ships as a bundled plugin in current Alvasta Pro releases, so normal
...
```bash
alvasta-pro plugins install ./path/to/local/synology-chat-plugin
```
...
1. Ensure the Synology Chat plugin is available.
   ...
   - `alvasta-pro onboard` now shows Synology Chat in the same channel setup list as `alvasta-pro channels add`.
   - Non-interactive setup: `alvasta-pro channels add --channel synology-chat --token <token> --url <incoming-webhook-url>`
2. In Synology Chat integrations:
   - Create an incoming webhook and copy its URL.
   - Create an outgoing webhook with your secret token.
3. Point the outgoing webhook URL to your Alvasta Pro gateway:
   - `https://gateway-host/webhook/synology` by default.
   - Or your custom `channels.synology-chat.webhookPath`.
4. Finish setup in Alvasta Pro.
   - Guided: `alvasta-pro onboard`
   - Direct: `alvasta-pro channels add --channel synology-chat --token <token> --url <incoming-webhook-url>`
5. Restart gateway and send a DM to the Synology Chat bot.

Webhook auth details:

- Alvasta Pro accepts the outgoing webhook token from `body.token`, then
  `?token=...`, then headers.
...
- Pairing approvals work with:
  - `alvasta-pro pairing list synology-chat`
  - `alvasta-pro pairing approve synology-chat <CODE>`
...
```bash
alvasta-pro message send --channel synology-chat --target 123456 --text "Hello from Alvasta Pro"
alvasta-pro message send --channel synology-chat --target synology-chat:123456 --text "Hello again"
```
...
Give each enabled account a distinct `webhookPath`. Alvasta Pro now rejects duplicate exact paths
...
- `Invalid token`:
  ...
  - a reverse proxy stripped the token header before the request reached Alvasta Pro
- `Rate limit exceeded`:
