...
Feishu (Lark) is a team chat platform used by companies for messaging and collaboration. This plugin connects Alvasta Pro to a Feishu/Lark bot using the platform’s WebSocket event subscription so messages can be received without exposing a public webhook URL.

---

## Bundled plugin

Feishu ships bundled with current Alvasta Pro releases, so no separate plugin install
is required.
...
```bash
alvasta-pro plugins install @alvasta-pro/feishu
```
...
If you just installed Alvasta Pro, run onboarding:
...
2. Configuring app credentials in Alvasta Pro
...
1. You already ran `alvasta-pro channels add` for Feishu
2. The gateway is running (`alvasta-pro gateway status`)
...
Edit `~/.alvasta-pro/alvasta-pro.json`:
...
```bash
alvasta-pro pairing approve feishu <CODE>
```
...
- **Approve pairing**:
  ...
  ```bash
  alvasta-pro pairing list feishu
  alvasta-pro pairing approve feishu <CODE>
  ```
...
2. Run `alvasta-pro logs --follow` and look for `chat_id`
...
2. Run `alvasta-pro logs --follow` and look for `open_id`
...
| Command                    | Description                   |
| -------------------------- | ----------------------------- |
| `alvasta-pro gateway status`  | Show gateway status           |
| `alvasta-pro gateway install` | Install/start gateway service |
| `alvasta-pro gateway stop`    | Stop gateway service          |
| `alvasta-pro gateway restart` | Restart gateway service       |
| `alvasta-pro logs --follow`   | Tail gateway logs             |
...
4. Check logs: `alvasta-pro logs --follow`
...
5. Ensure the gateway is running: `alvasta-pro gateway status`
6. Check logs: `alvasta-pro logs --follow`
  ...
  agents: {
    list: [
      { id: "main" },
      {
        ...
        agentDir: "/home/user/.alvasta-pro/agents/clawd-fan/agent",
      },
      {
        id: "clawd-xi",
        workspace: "/home/user/clawd-xi",
        agentDir: "/home/user/.alvasta-pro/agents/clawd-xi/agent",
      },
    ],
  },
  bindings: [
