...
Nextcloud Talk ships as a bundled plugin in current Alvasta Pro releases, so
...
```bash
alvasta-pro plugins install @alvasta-pro/nextcloud-talk
```

Local checkout (when running from a git repo):

```bash
alvasta-pro plugins install ./path/to/local/nextcloud-talk-plugin
```
...
2. On your Nextcloud server, create a bot:
   ...
   ```bash
   ./occ talk:bot:install "Alvasta Pro" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `alvasta-pro pairing list nextcloud-talk`
  - `alvasta-pro pairing approve nextcloud-talk <CODE>`
