...
read_when:
  - Setting up Zalo Personal for Alvasta Pro
...
Status: experimental. This integration automates a **personal Zalo account** via native `zca-js` inside Alvasta Pro.

> **Warning:** This is an unofficial integration and may result in account suspension/ban. Use at your own risk.

## Bundled plugin

Zalo Personal ships as a bundled plugin in current Alvasta Pro releases, so normal
packaged builds do not need a separate install.

If you are on an older build or a custom install that excludes Zalo Personal,
install it manually:

- Install via CLI: `alvasta-pro plugins install @alvasta-pro/zalouser`
- Or from a source checkout: `alvasta-pro plugins install ./path/to/local/zalouser-plugin`
...
2. Login (QR, on the Gateway machine):
   - `alvasta-pro channels login --channel zalouser`
...
- `alvasta-pro pairing list zalouser`
- `alvasta-pro pairing approve zalouser <code>`
...
- On startup, Alvasta Pro resolves group/user names in allowlists to IDs and logs the mapping.
...
- When a group message is skipped because mention is required, Alvasta Pro stores it as pending group history and includes it on the next processed group message.
...
Accounts map to `zalouser` profiles in Alvasta Pro state. Example:
...
- Alvasta Pro sends a typing event before dispatching a reply (best-effort).
...
- For inbound messages that include event metadata, Alvasta Pro sends delivered + seen acknowledgements (best-effort).
...
- Re-login: `alvasta-pro channels logout --channel zalouser && alvasta-pro channels login --channel zalouser`
...
- The channel now runs fully in Alvasta Pro without external CLI binaries.
