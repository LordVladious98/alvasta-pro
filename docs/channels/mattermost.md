---
summary: "Mattermost bot setup and Alvasta Pro config"
read_when:
...
Mattermost ships as a bundled plugin in current Alvasta Pro releases, so normal
...
```bash
alvasta-pro plugins install @alvasta-pro/mattermost
```

Local checkout (when running from a git repo):

```bash
alvasta-pro plugins install ./path/to/local/mattermost-plugin
```
...
4. Configure Alvasta Pro and start the gateway.
...
Native slash commands are opt-in. When enabled, Alvasta Pro registers `oc_*` slash commands via
...
- If `callbackUrl` is omitted, Alvasta Pro derives one from gateway host/port + `callbackPath`.
...
- Command callbacks are validated with the per-command tokens returned by
  Mattermost when Alvasta Pro registers `oc_*` commands.
...
- Reachability requirement: the callback endpoint must be reachable from the Mattermost server.
  - Do not set `callbackUrl` to `localhost` unless Mattermost runs on the same host/network namespace as Alvasta Pro.
  - Do not set `callbackUrl` to your Mattermost base URL unless that URL reverse-proxies `/api/channels/mattermost/command` to Alvasta Pro.
  - A quick check is `curl https://<gateway-host>/api/channels/mattermost/command`; a GET should return `405 Method Not Allowed` from Alvasta Pro, not `404`.
...
- Approve via:
  - `alvasta-pro pairing list mattermost`
  - `alvasta-pro pairing approve mattermost <CODE>`
...
Use these target formats with `alvasta-pro message send` or cron/webhooks:
...
Alvasta Pro resolves them **user-first**:

- If the ID exists as a user (`GET /api/v4/users/<id>` succeeds), Alvasta Pro sends a **DM** by resolving the direct channel via `/api/v4/channels/direct`.
...
When Alvasta Pro sends to a Mattermost DM target and needs to resolve the direct channel first, it
...
- If `interactions.callbackBaseUrl` is omitted, Alvasta Pro derives the callback URL from
...
- Reachability rule: the button callback URL must be reachable from the Mattermost server.
  `localhost` only works when Mattermost and Alvasta Pro run on the same host/network namespace.
...
1. Derive the secret from the bot token:
   `HMAC-SHA256(key="alvasta-pro-mattermost-interactions", data=botToken)`
...
secret = hmac.new(
    b"alvasta-pro-mattermost-interactions",
...
`alvasta-pro message send` and cron/webhook deliveries.
...
- Native slash commands return `Unauthorized: invalid command token.`: Alvasta Pro
...
- If `callbackUrl` is omitted and logs warn that the callback resolved to
  ...
  Mattermost runs on the same host/network namespace as Alvasta Pro. Set an
