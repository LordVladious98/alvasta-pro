...
read_when:
  - You want to connect Alvasta Pro to LINE
...
LINE connects to Alvasta Pro via the LINE Messaging API. The plugin runs as a webhook
...
LINE ships as a bundled plugin in current Alvasta Pro releases, so normal
...
```bash
alvasta-pro plugins install @alvasta-pro/line
```

Local checkout (when running from a git repo):

```bash
alvasta-pro plugins install ./path/to/local/line-plugin
```
...
- LINE signature verification is body-dependent (HMAC over the raw body), so Alvasta Pro applies strict pre-auth body limits and timeout before verification.
- Alvasta Pro processes webhook events from the verified raw request bytes. Upstream middleware-transformed `req.body` values are ignored for signature-integrity safety.
...
```bash
alvasta-pro pairing list line
alvasta-pro pairing approve line <CODE>
```
