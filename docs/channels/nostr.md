...
read_when:
  - You want Alvasta Pro to receive DMs via Nostr
...
Nostr is a decentralized protocol for social networking. This channel enables Alvasta Pro to receive and respond to encrypted direct messages (DMs) via NIP-04.

## Bundled plugin

Current Alvasta Pro releases ship Nostr as a bundled plugin, so normal packaged
builds do not need a separate install.

### Older/custom installs

- Onboarding (`alvasta-pro onboard`) and `alvasta-pro channels add` still surface
  Nostr from the shared channel catalog.
- If your build excludes bundled Nostr, install it manually.

```bash
alvasta-pro plugins install @alvasta-pro/nostr
```

Use a local checkout (dev workflows):

```bash
alvasta-pro plugins install --link <path-to-local-nostr-plugin>
```
...
alvasta-pro channels add --channel nostr --private-key "$NOSTR_PRIVATE_KEY" --relay-urls "wss://relay.damus.io,wss://relay.primal.net"
```
...
```json5
{
  channels: {
    nostr: {
      ...
      profile: {
        name: "alvasta-pro",
        displayName: "Alvasta Pro",
        about: "Personal assistant DM bot",
        picture: "https://example.com/avatar.png",
        banner: "https://example.com/banner.png",
        website: "https://example.com",
        nip05: "alvasta-pro@example.com",
        lud16: "alvasta-pro@example.com",
      },
    },
  },
}
