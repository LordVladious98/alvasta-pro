...
Tlon is a decentralized messenger built on Urbit. Alvasta Pro connects to your Urbit ship and can
...
Tlon ships as a bundled plugin in current Alvasta Pro releases, so normal packaged
...
```bash
alvasta-pro plugins install @alvasta-pro/tlon
```

Local checkout (when running from a git repo):

```bash
alvasta-pro plugins install ./path/to/local/tlon-plugin
```
...
By default, Alvasta Pro blocks private/internal hostnames and IP ranges for SSRF protection.
...
Use these with `alvasta-pro message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, Alvasta Pro replies in-thread.
