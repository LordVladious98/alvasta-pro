...
Zalo ships as a bundled plugin in current Alvasta Pro releases, so normal packaged
builds do not need a separate install.

If you are on an older build or a custom install that excludes Zalo, install it
manually:

- Install via CLI: `alvasta-pro plugins install @alvasta-pro/zalo`
- Or from a source checkout: `alvasta-pro plugins install ./path/to/local/zalo-plugin`
...
This page reflects current Alvasta Pro behavior for **Zalo Bot Creator / Marketplace bots**.
...
- Approve via:
  - `alvasta-pro pairing list zalo`
  - `alvasta-pro pairing approve zalo <CODE>`
...
This table summarizes current **Zalo Bot Creator / Marketplace bot** behavior in Alvasta Pro.
...
- Example: `alvasta-pro message send --channel zalo --target 123456789 --message "hi"`.

## Troubleshooting

**Bot doesn't respond:**

- Check that the token is valid: `alvasta-pro channels status --probe`
- Verify the sender is approved (pairing or allowFrom)
- Check gateway logs: `alvasta-pro logs --follow`
