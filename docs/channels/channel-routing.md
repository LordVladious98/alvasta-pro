...
Alvasta Pro routes replies **back to the channel where a message came from**. The
...
Alvasta Pro infers a pinned owner from `allowFrom` when all of these are true:
...
In that mismatch case, Alvasta Pro still records inbound session metadata, but it
...
Broadcast groups let you run **multiple agents** for the same peer **when Alvasta Pro would normally reply** (for example: in WhatsApp groups, after mention/activation gating).
...
{
  agents: {
    list: [{ id: "support", name: "Support", workspace: "~/.alvasta-pro/workspace-support" }],
  },
  bindings: [
...
Session stores live under the state directory (default `~/.alvasta-pro`):

- `~/.alvasta-pro/agents/<agentId>/sessions/sessions.json`
