...
read_when:
  ...
  - Reviewing Alvasta Pro security posture
title: "Pairing"
---

# Pairing

“Pairing” is Alvasta Pro’s explicit **owner approval** step.
...
Supported channels: `bluebubbles`, `discord`, `feishu`, `googlechat`, `imessage`, `irc`, `line`, `matrix`, `mattermost`, `msteams`, `nextcloud-talk`, `nostr`, `alvasta-pro-weixin`, `signal`, `slack`, `synology-chat`, `telegram`, `twitch`, `whatsapp`, `zalo`, `zalouser`.

### Where the state lives

Stored under `~/.alvasta-pro/credentials/`:
...
3. On your phone, open the Alvasta Pro iOS app → Settings → Gateway.
...
Stored under `~/.alvasta-pro/devices/`:
...
- The legacy `node.pair.*` API (CLI: `alvasta-pro nodes pending|approve|reject|rename`) is a
