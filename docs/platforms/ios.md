...
- Network path:
  ...
  - Tailnet via unicast DNS-SD (example domain: `alvasta-pro.internal.`), **or**
...
Run `alvasta-pro devices list` again before approval.

4. Verify connection:

```bash
alvasta-pro nodes status
alvasta-pro gateway call node.list --params "{}"
```
...
- Only genuine Alvasta Pro iOS builds distributed through Apple can use the hosted relay.
...
- To allow hosted relay usage only for official/TestFlight Alvasta Pro builds.
...
```bash
mkdir -p ~/.alvasta-pro/credentials/apns
chmod 700 ~/.alvasta-pro/credentials/apns
mv /path/to/AuthKey_KEYID.p8 ~/.alvasta-pro/credentials/apns/AuthKey_KEYID.p8
chmod 600 ~/.alvasta-pro/credentials/apns/AuthKey_KEYID.p8
export OPENCLAW_APNS_PRIVATE_KEY_PATH="$HOME/.alvasta-pro/credentials/apns/AuthKey_KEYID.p8"
```
...
The iOS app browses `_alvasta-pro-gw._tcp` on `local.` and, when configured, the same
...
`alvasta-pro.internal.`) and Tailscale split DNS.
...
```bash
alvasta-pro nodes invoke --node "iOS Node" --command canvas.navigate --params '{"url":"http://<gateway-host>:18789/__alvasta-pro__/canvas/"}'
```

Notes:

- The Gateway canvas host serves `/__alvasta-pro__/canvas/` and `/__alvasta-pro__/a2ui/`.
...
```bash
alvasta-pro nodes invoke --node "iOS Node" --command canvas.eval --params '{"javaScript":"(() => { const {ctx} = window.__alvasta-pro; ctx.clearRect(0,0,innerWidth,innerHeight); ctx.lineWidth=6; ctx.strokeStyle=\"#ff2d55\"; ctx.beginPath(); ctx.moveTo(40,40); ctx.lineTo(innerWidth-40, innerHeight-40); ctx.stroke(); return \"ok\"; })()"}'
```

```bash
alvasta-pro nodes invoke --node "iOS Node" --command canvas.snapshot --params '{"maxWidth":900,"format":"jpeg"}'
```
...
- Pairing prompt never appears: run `alvasta-pro devices list` and approve manually.
