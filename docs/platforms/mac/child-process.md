...
service via the external `alvasta-pro` CLI (no embedded runtime). This gives you
...
- The app installs a per‑user LaunchAgent labeled `ai.alvasta-pro.gateway`
  (or `ai.alvasta-pro.<profile>` when using `--profile`/`OPENCLAW_PROFILE`; legacy `com.alvasta-pro.*` is supported).
