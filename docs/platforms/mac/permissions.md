...
- Same path: run the app from a fixed location (for Alvasta Pro, `dist/Alvasta Pro.app`).
...
```bash
sudo tccutil reset Accessibility ai.alvasta-pro.mac
sudo tccutil reset ScreenCapture ai.alvasta-pro.mac
...
Workaround: move files into the Alvasta Pro workspace (`~/.alvasta-pro/workspace`) if you want to avoid per-folder grants.
