...
The macOS app surfaces Alvasta Pro skills via the gateway; it does not parse skills locally.

## Data source

- `skills.status` (gateway) returns all skills plus eligibility and missing requirements
  (including allowlist blocks for bundled skills).
- Requirements are derived from `metadata.alvasta-pro.requires` in each `SKILL.md`.

## Install actions

- `metadata.alvasta-pro.install` defines install options (brew/node/go/uv).
...
- The app stores keys in `~/.alvasta-pro/alvasta-pro.json` under `skills.entries.<skillKey>`.
