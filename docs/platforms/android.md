...
> **Note:** The Android app has not been publicly released yet. The source code is available in the [Alvasta Pro repository](https://github.com/alvasta-pro/alvasta-pro) under `apps/android`. You can build it yourself using Java 17 and the Android SDK (`./gradlew :app:assemblePlayDebug`). See [apps/android/README.md](https://github.com/alvasta-pro/alvasta-pro/blob/main/apps/android/README.md) for build instructions.
...
- You can run the CLI (`alvasta-pro`) on the gateway machine (or via SSH).
...
```bash
dns-sd -B _alvasta-pro-gw._tcp local.
```
...
```bash
alvasta-pro gateway discover --json
```
...
1. Set up a DNS-SD zone (example `alvasta-pro.internal.`) on the gateway host and publish `_alvasta-pro-gw._tcp` records.
...
1. Create `~/.alvasta-pro/workspace/canvas/index.html` on the gateway host.

2. Navigate the node to it (LAN):

```bash
alvasta-pro nodes invoke --node "<Android Node>" --command canvas.navigate --params '{"url":"http://<gateway-hostname>.local:18789/__alvasta-pro__/canvas/"}'
```

Tailnet (optional): if both devices are on Tailscale, use a MagicDNS name or tailnet IP instead of `.local`, e.g. `http://<gateway-magicdns>:18789/__alvasta-pro__/canvas/`.

This server injects a live-reload client into HTML and reloads on file changes.
The A2UI host lives at `http://<gateway-host>:18789/__alvasta-pro__/a2ui/`.
...
Android supports launching Alvasta Pro from the system assistant trigger (Google
Assistant). When configured, holding the home button or saying "Hey Google, ask
Alvasta Pro..." opens the app and hands the prompt into the chat composer.
...
and whether the user has set Alvasta Pro as the default assistant app.
</Note>
