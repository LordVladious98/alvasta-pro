---
summary: "Alvasta Pro on DigitalOcean (simple paid VPS option)"
read_when:
  - Setting up Alvasta Pro on DigitalOcean
  - Looking for cheap VPS hosting for Alvasta Pro
title: "DigitalOcean (Platform)"
---

# Alvasta Pro on DigitalOcean

## Goal

Run a persistent Alvasta Pro Gateway on DigitalOcean for **$6/month** (or $4/mo with reserved pricing).
...
```bash
alvasta-pro config set gateway.bind tailnet
alvasta-pro gateway restart
```
...
```bash
alvasta-pro channels login whatsapp
# Scan QR code
```
...
- `~/.alvasta-pro/workspace/` — workspace (SOUL.md, memory, etc.)
...
journalctl --user -u alvasta-pro-gateway.service --no-pager -n 50
```
