---
summary: "How Alvasta Pro builds prompt context and reports token usage + costs"
read_when:
...
Alvasta Pro tracks **tokens**, not characters. Tokens are model-specific, but most
OpenAI-style models average ~4 characters per token for English text.

## How the system prompt is built

Alvasta Pro assembles its own system prompt on every run. It includes:
...
For images, Alvasta Pro downscales transcript/tool image payloads before provider calls.
...
- `/usage cost` → shows a local cost summary from Alvasta Pro session logs.

Other surfaces:

- **TUI/Web TUI:** `/status` + `/usage` are supported.
- **CLI:** `alvasta-pro status --usage` and `alvasta-pro channels list` show
...
available; otherwise Alvasta Pro falls back to matching OAuth/API-key credentials
...
`cacheWrite`. If pricing is missing, Alvasta Pro shows tokens only. OAuth tokens
never show dollar cost.

## Cache TTL and pruning impact

Provider prompt caching only applies within the cache TTL window. Alvasta Pro can
...
Anthropic's 1M context window is currently beta-gated. Alvasta Pro can inject the
...
Alvasta Pro skips the `context-1m-*` beta header because Anthropic currently
