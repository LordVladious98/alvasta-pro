...
Alvasta Pro features that can generate provider usage or paid API calls.
...
- Gemini CLI note: when the CLI returns JSON output, Alvasta Pro reads usage from
...
Anthropic still does not expose a per-message dollar estimate that Alvasta Pro can
show in `/usage full`.

**CLI usage windows (provider quotas)**

- `alvasta-pro status --usage` and `alvasta-pro channels list` show provider **usage windows**
...
- MiniMax note: its raw `usage_percent` / `usagePercent` fields mean remaining
  quota, so Alvasta Pro inverts them before display. Count-based fields still win
  when present. If the provider returns `model_remains`, Alvasta Pro prefers the
...
- Usage auth for those quota windows comes from provider-specific hooks when
  available; otherwise Alvasta Pro falls back to matching OAuth/API-key
...
Alvasta Pro can pick up credentials from:
...
Alvasta Pro's local UI, such as **OpenAI Codex**, **Alibaba Cloud Model Studio
Coding Plan**, **MiniMax Coding Plan**, **Z.AI / GLM Coding Plan**, and
Anthropic's Alvasta Pro Claude-login path with **Extra Usage** enabled.
...
- `alvasta-pro status --usage`
- `alvasta-pro models status --json`
...
`alvasta-pro models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
