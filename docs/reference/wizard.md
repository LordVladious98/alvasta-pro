...
This is the full reference for `alvasta-pro onboard`.
...
<Steps>
  <Step title="Existing config detection">
    - If `~/.alvasta-pro/alvasta-pro.json` exists, choose **Keep / Modify / Reset**.
    ...
    - If the config is invalid or contains legacy keys, the wizard stops and asks
      you to run `alvasta-pro doctor` before continuing.
  ...
  <Step title="Model/Auth">
    ...
    - **Anthropic setup-token**: still available in onboarding/configure, though Alvasta Pro now prefers Claude CLI reuse when available.
    - **OpenAI Code (Codex) subscription (Codex CLI)**: if `~/.codex/auth.json` exists, onboarding can reuse it. Reused Codex CLI credentials stay managed by Codex CLI; on expiry Alvasta Pro re-reads that source first and, when the provider can refresh it, writes the refreshed credential back to Codex storage instead of taking ownership itself.
    ...
    - Auth profiles live in `~/.alvasta-pro/agents/<agentId>/agent/auth-profiles.json` (API keys + OAuth). `~/.alvasta-pro/credentials/oauth.json` is legacy import-only.
    - More detail: [/concepts/oauth](/concepts/oauth)
    <Note>
    Headless/server tip: complete OAuth on a machine with a browser, then copy
    that agent's `auth-profiles.json` (for example
    `~/.alvasta-pro/agents/<agentId>/agent/auth-profiles.json`, or the matching
    `$OPENCLAW_STATE_DIR/...` path) to the gateway host. `credentials/oauth.json`
    is only a legacy import source.
    </Note>
  </Step>
  <Step title="Workspace">
    - Default `~/.alvasta-pro/workspace` (configurable).
  ...
  <Step title="Channels">
    ...
    - DM security: default is pairing. First DM sends a code; approve via `alvasta-pro pairing approve <channel> <code>` or use allowlists.
  </Step>
  <Step title="Web search">
    ...
    - Configure later: `alvasta-pro configure --section web`.
  </Step>
  ...
  <Step title="Health check">
    - Starts the Gateway (if needed) and runs `alvasta-pro health`.
    - Tip: `alvasta-pro status --deep` adds the live gateway health probe to status output, including channel probes when supported (requires a reachable gateway).
  </Step>
...
- Stores it under `~/.alvasta-pro/tools/signal-cli/<version>/`.
...
Typical fields in `~/.alvasta-pro/alvasta-pro.json`:
...
`alvasta-pro agents add` writes `agents.list[]` and optional `bindings`.

WhatsApp credentials go under `~/.alvasta-pro/credentials/whatsapp/<accountId>/`.
Sessions are stored under `~/.alvasta-pro/agents/<agentId>/sessions/`.
