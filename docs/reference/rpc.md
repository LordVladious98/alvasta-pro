...
Alvasta Pro integrates external CLIs via JSON-RPC. Two patterns are used today.
...
- Alvasta Pro owns lifecycle when `channels.signal.autoStart=true`.
...
- Alvasta Pro spawns `imsg rpc` as a child process (legacy iMessage integration).
