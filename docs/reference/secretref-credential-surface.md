...
- In scope: strictly user-supplied credentials that Alvasta Pro does not mint or rotate.
- Out of scope: runtime-minted or rotating credentials, OAuth refresh material, and session-like artifacts.

## Supported credentials

### `alvasta-pro.json` targets (`secrets configure` + `secrets apply` + `secrets audit`)
