---
summary: "What the Alvasta Pro system prompt contains and how it is assembled"
read_when:
...
Alvasta Pro builds a custom system prompt for every agent run. The prompt is **Alvasta Pro-owned** and does not use the pi-coding-agent default prompt.

The prompt is assembled by Alvasta Pro and injected into each agent run.

Provider plugins can contribute cache-aware prompt guidance without replacing
the full Alvasta Pro-owned prompt. The provider runtime can:
...
- **Alvasta Pro Self-Update**: how to inspect config safely with
...
- **Documentation**: local path to Alvasta Pro docs (repo or npm package) and when to read them.
...
Alvasta Pro can render smaller system prompts for sub-agents. The runtime sets a
`promptMode` for each run (not a user-facing config):

- `full` (default): includes all sections above.
- `minimal`: used for sub-agents; omits **Skills**, **Memory Recall**, **Alvasta Pro
...
occurs, Alvasta Pro can inject a warning block in Project Context; control this with
...
When eligible skills exist, Alvasta Pro injects a compact **available skills list**
...
local Alvasta Pro docs directory (either `docs/` in the repo workspace or the bundled npm
...
for Alvasta Pro behavior, commands, configuration, or architecture, and to run
`alvasta-pro status` itself when possible (asking the user only when it lacks access).
