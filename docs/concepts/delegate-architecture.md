---
summary: "Delegate architecture: running Alvasta Pro as a named agent on behalf of an organization"
...
Goal: run Alvasta Pro as a **named delegate** — an agent with its own identity that acts "on behalf of" people in an organization. The agent never impersonates a human. It sends, reads, and schedules under its own account with explicit delegation permissions.

This extends [Multi-Agent Routing](/concepts/multi-agent) from personal use into organizational deployments.

## What is a delegate?

A **delegate** is an Alvasta Pro agent that:
...
Alvasta Pro's default mode is a **personal assistant** — one human, one agent. Delegates extend this to organizations:
...
2. **Scope control**: the identity provider enforces what the delegate can access, independent of Alvasta Pro's own tool policy.
...
{
  id: "delegate",
  workspace: "~/.alvasta-pro/workspace-delegate",
  tools: {
...
- Cron run history: `~/.alvasta-pro/cron/runs/<jobId>.jsonl`
- Session transcripts: `~/.alvasta-pro/agents/delegate/sessions`
- Identity provider audit logs (Exchange, Google Workspace)

All delegate actions flow through Alvasta Pro's session store. For compliance, ensure these logs are retained and reviewed.
...
```bash
alvasta-pro agents add delegate
```

This creates:

- Workspace: `~/.alvasta-pro/workspace-delegate`
- State: `~/.alvasta-pro/agents/delegate/agent`
- Sessions: `~/.alvasta-pro/agents/delegate/sessions`
...
```json5
{
  agents: {
    list: [
      { id: "main", workspace: "~/.alvasta-pro/workspace" },
      {
        id: "delegate",
        workspace: "~/.alvasta-pro/workspace-delegate",
        tools: {
...
~/.alvasta-pro/agents/delegate/agent/auth-profiles.json
```
...
```json5
{
  agents: {
    list: [
      { id: "main", default: true, workspace: "~/.alvasta-pro/workspace" },
      {
        id: "org-assistant",
        name: "[Organization] Assistant",
        workspace: "~/.alvasta-pro/workspace-org",
        agentDir: "~/.alvasta-pro/agents/org-assistant/agent",
...
recall view. Alvasta Pro redacts credential/token-like text, truncates long
