---
summary: "How Alvasta Pro summarizes long conversations to stay within model limits"
read_when:
...
When a conversation approaches that limit, Alvasta Pro **compacts** older messages
...
When Alvasta Pro splits history into compaction chunks, it keeps assistant tool
calls paired with their matching `toolResult` entries. If a split point lands
inside a tool block, Alvasta Pro moves the boundary so the pair stays together and
...
Alvasta Pro compacts and retries). Typical overflow signatures include
...
<Info>
Before compacting, Alvasta Pro automatically reminds the agent to save important
