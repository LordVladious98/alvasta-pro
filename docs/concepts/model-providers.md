...
- CLI helpers: `alvasta-pro onboard`, `alvasta-pro models list`, `alvasta-pro models set <provider/model>`.
...
- Provider plugins can inject model catalogs via `registerProvider({ catalog })`;
  Alvasta Pro merges that output into `models.providers` before writing
  `models.json`.
...
Provider plugins can now own most provider-specific logic while Alvasta Pro keeps
the generic inference loop.

Typical split:

- `auth[].run` / `auth[].runNonInteractive`: provider owns onboarding/login
  flows for `alvasta-pro onboard`, `alvasta-pro models auth`, and headless setup
...
- `normalizeTransport`: provider normalizes transport-family `api` / `baseUrl`
  before generic model assembly; Alvasta Pro checks the matched provider first,
...
- `normalizeConfig`: provider normalizes `models.providers.<id>` config before
  runtime uses it; Alvasta Pro checks the matched provider first, then other
...
That covers providers that still fit Alvasta Pro's normal transports. A provider
...
Alvasta Pro ships with the pi‑ai catalog. These providers require **no**
...
- CLI: `alvasta-pro onboard --auth-choice openai-api-key`
...
- Hidden Alvasta Pro attribution headers (`originator`, `version`,
...
- `openai/gpt-5.3-codex-spark` is intentionally suppressed in Alvasta Pro because the live OpenAI API rejects it; Spark is treated as Codex-only
...
- CLI: `alvasta-pro onboard --auth-choice apiKey`
- Direct public Anthropic requests support the shared `/fast` toggle and `params.fastMode`, including API-key and OAuth-authenticated traffic sent to `api.anthropic.com`; Alvasta Pro maps that to Anthropic `service_tier` (`auto` vs `standard_only`)
- Anthropic note: Anthropic staff told us Alvasta Pro-style Claude CLI usage is allowed again, so Alvasta Pro treats Claude CLI reuse and `claude -p` usage as sanctioned for this integration unless Anthropic publishes a new policy.
- Anthropic setup-token remains available as a supported Alvasta Pro token path, but Alvasta Pro now prefers Claude CLI reuse and `claude -p` when available.
...
- CLI: `alvasta-pro onboard --auth-choice openai-codex` or `alvasta-pro models auth login --provider openai-codex`
- Default transport is `auto` (WebSocket-first, SSE fallback)
...
- Shares the same `/fast` toggle and `params.fastMode` config as direct `openai/*`; Alvasta Pro maps that to `service_tier=priority`
...
- Policy note: OpenAI Codex OAuth is explicitly supported for external tools/workflows like Alvasta Pro.
...
- CLI: `alvasta-pro onboard --auth-choice opencode-zen` or `alvasta-pro onboard --auth-choice opencode-go`
...
- Compatibility: legacy Alvasta Pro config using `google/gemini-3.1-flash-preview` is normalized to `google/gemini-3-flash-preview`
- CLI: `alvasta-pro onboard --auth-choice gemini-api-key`
- Direct Gemini runs also accept `agents.defaults.models["google/<model>"].params.cachedContent`
  ...
  `cachedContents/...` handle; Gemini cache hits surface as Alvasta Pro `cacheRead`

### Google Vertex and Gemini CLI

- Providers: `google-vertex`, `google-gemini-cli`
- Auth: Vertex uses gcloud ADC; Gemini CLI uses its OAuth flow
- Caution: Gemini CLI OAuth in Alvasta Pro is an unofficial integration. Some users have reported Google account restrictions after using third-party clients. Review Google terms and use a non-critical account if you choose to proceed.
- Gemini CLI OAuth is shipped as part of the bundled `google` plugin.
  - Install Gemini CLI first:
    - `brew install gemini-cli`
    - or `npm install -g @google/gemini-cli`
  - Enable: `alvasta-pro plugins enable google`
  - Login: `alvasta-pro models auth login --provider google-gemini-cli --set-default`
  - Default model: `google-gemini-cli/gemini-3-flash-preview`
  - Note: you do **not** paste a client id or secret into `alvasta-pro.json`. The CLI login flow stores
  ...
  - Gemini CLI JSON replies are parsed from `response`; usage falls back to
    `stats`, with `stats.cached` normalized into Alvasta Pro `cacheRead`.
...
- CLI: `alvasta-pro onboard --auth-choice zai-api-key`
...
- CLI: `alvasta-pro onboard --auth-choice ai-gateway-api-key`
...
- CLI: `alvasta-pro onboard --auth-choice kilocode-api-key`
...
- Alvasta Pro applies OpenRouter's documented app-attribution headers only when
...
- CLI: `alvasta-pro onboard --auth-choice mistral-api-key`
- Groq: `groq` (`GROQ_API_KEY`)
- Cerebras: `cerebras` (`CEREBRAS_API_KEY`)
  - GLM models on Cerebras use ids `zai-glm-4.7` and `zai-glm-4.6`.
  - OpenAI-compatible base URL: `https://api.cerebras.ai/v1`.
- GitHub Copilot: `github-copilot` (`COPILOT_GITHUB_TOKEN` / `GH_TOKEN` / `GITHUB_TOKEN`)
- Hugging Face Inference example model: `huggingface/deepseek-ai/DeepSeek-R1`; CLI: `alvasta-pro onboard --auth-choice huggingface-api-key`. See [Hugging Face (Inference)](/providers/huggingface).
...
- Example model: `moonshot/kimi-k2.5`
- CLI: `alvasta-pro onboard --auth-choice moonshot-api-key` or `alvasta-pro onboard --auth-choice moonshot-api-key-cn`
...
- Example model: `volcengine-plan/ark-code-latest`
- CLI: `alvasta-pro onboard --auth-choice volcengine-api-key`
...
Alvasta Pro falls back to the unfiltered catalog instead of showing an empty
...
- Example model: `byteplus-plan/ark-code-latest`
- CLI: `alvasta-pro onboard --auth-choice byteplus-api-key`
...
- CLI: `alvasta-pro onboard --auth-choice synthetic-api-key`
...
On MiniMax's Anthropic-compatible streaming path, Alvasta Pro disables thinking by
...
`alvasta-pro onboard` and the model picker. See [/providers/ollama](/providers/ollama)
...
- For custom providers, `reasoning`, `input`, `cost`, `contextWindow`, and `maxTokens` are optional.
  When omitted, Alvasta Pro defaults to:
  - `reasoning: false`
  - `input: ["text"]`
  - `cost: { input: 0, output: 0, cacheRead: 0, cacheWrite: 0 }`
  - `contextWindow: 200000`
...
- For `api: "openai-completions"` on non-native endpoints (any non-empty `baseUrl` whose host is not `api.openai.com`), Alvasta Pro forces `compat.supportsDeveloperRole: false` to avoid provider 400 errors for unsupported `developer` roles.
- Proxy-style OpenAI-compatible routes also skip native OpenAI-only request
  ...
  OpenAI reasoning-compat payload shaping, and no hidden Alvasta Pro attribution
  headers.
- If `baseUrl` is empty/omitted, Alvasta Pro keeps the default OpenAI behavior (which resolves to `api.openai.com`).
- For safety, an explicit `compat.supportsDeveloperRole: true` is still overridden on non-native `openai-completions` endpoints.

## CLI examples

```bash
alvasta-pro onboard --auth-choice opencode-zen
alvasta-pro models set opencode/claude-opus-4-6
