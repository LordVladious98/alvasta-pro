...
alongside Alvasta Pro. It combines BM25, vector search, and reranking in a single
...
- **Automatic fallback** -- if QMD is unavailable, Alvasta Pro falls back to the
...
Alvasta Pro creates a self-contained QMD home under
`~/.alvasta-pro/agents/<agentId>/qmd/` and manages the sidecar lifecycle
...
- Alvasta Pro creates collections from your workspace memory files and any
...
- Searches use the configured `searchMode` (default: `search`; also supports
  `vsearch` and `query`). If a mode fails, Alvasta Pro retries with `qmd query`.
- If QMD fails entirely, Alvasta Pro falls back to the builtin SQLite engine.
...
process, so you can tune QMD globally without adding new Alvasta Pro config:
...
collection under `~/.alvasta-pro/agents/<id>/qmd/sessions/`.
...
When scope denies a search, Alvasta Pro logs a warning with the derived channel and
...
**QMD not found?** Ensure the binary is on the gateway's `PATH`. If Alvasta Pro
runs as a service, create a symlink:
`sudo ln -s ~/.bun/bin/qmd /usr/local/bin/qmd`.

**First search very slow?** QMD downloads GGUF models on first use. Pre-warm
with `qmd query "test"` using the same XDG dirs Alvasta Pro uses.
...
Alvasta Pro's builtin symlink rules. Keep temporary monorepo checkouts under
