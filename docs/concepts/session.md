---
summary: "How Alvasta Pro manages conversation sessions"
read_when:
...
Alvasta Pro organizes conversations into **sessions**. Each message is routed to a
...
Verify your setup with `alvasta-pro security audit`.
...
Alvasta Pro automatically bounds session storage over time. By default, it runs
...
Preview with `alvasta-pro sessions cleanup --dry-run`.

## Inspecting sessions

- `alvasta-pro status` -- session store path and recent activity.
- `alvasta-pro sessions --json` -- all sessions (filter with `--active <minutes>`).
