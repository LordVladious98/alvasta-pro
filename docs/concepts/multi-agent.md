...
- **Session store** (chat history + routing state) under `~/.alvasta-pro/agents/<agentId>/sessions`.
...
`~/.alvasta-pro/skills`, then filtered by the effective agent skill allowlist when
...
- State dir: `~/.alvasta-pro` (or `OPENCLAW_STATE_DIR`)
- Workspace: `~/.alvasta-pro/workspace` (or `~/.alvasta-pro/workspace-<agentId>`)
- Agent dir: `~/.alvasta-pro/agents/<agentId>/agent` (or `agents.list[].agentDir`)
- Sessions: `~/.alvasta-pro/agents/<agentId>/sessions`

### Single-agent mode (default)

If you do nothing, Alvasta Pro runs a single agent:

- `agentId` defaults to **`main`**.
- Sessions are keyed as `agent:main:<mainKey>`.
- Workspace defaults to `~/.alvasta-pro/workspace` (or `~/.alvasta-pro/workspace-<profile>` when `OPENCLAW_PROFILE` is set).
- State defaults to `~/.alvasta-pro/agents/main/agent`.
...
```bash
alvasta-pro agents add coding
alvasta-pro agents add social
```

Each agent gets its own workspace with `SOUL.md`, `AGENTS.md`, and optional `USER.md`, plus a dedicated `agentDir` and session store under `~/.alvasta-pro/agents/<agentId>`.
...
```bash
alvasta-pro channels login --channel whatsapp --account work
```
...
```json5
{
  agents: {
    list: [
      { id: "alex", workspace: "~/.alvasta-pro/workspace-alex" },
      { id: "mia", workspace: "~/.alvasta-pro/workspace-mia" },
    ],
  },
  bindings: [
    {
...
- If you later add the same binding for the same agent with an explicit account id, Alvasta Pro upgrades the existing channel-only binding to account-scoped instead of duplicating it.
...
`channels.<channel>.defaultAccount` (optional). When unset, Alvasta Pro falls back
...
```json5
{
  agents: {
    list: [
      { id: "main", workspace: "~/.alvasta-pro/workspace-main" },
      { id: "coding", workspace: "~/.alvasta-pro/workspace-coding" },
    ],
  },
  bindings: [
...
```json5
{
  agents: {
    list: [
      { id: "main", workspace: "~/.alvasta-pro/workspace-main" },
      { id: "alerts", workspace: "~/.alvasta-pro/workspace-alerts" },
    ],
  },
  bindings: [
...
```bash
alvasta-pro channels login --channel whatsapp --account personal
alvasta-pro channels login --channel whatsapp --account biz
```

`~/.alvasta-pro/alvasta-pro.json` (JSON5):
...
```js
{
    ...
    list: [
      {
        ...
        default: true,
        name: "Home",
        workspace: "~/.alvasta-pro/workspace-home",
        agentDir: "~/.alvasta-pro/agents/home/agent",
      },
      {
        id: "work",
        name: "Work",
        workspace: "~/.alvasta-pro/workspace-work",
        agentDir: "~/.alvasta-pro/agents/work/agent",
      },
    ],
  },
  ...
  channels: {
    whatsapp: {
      accounts: {
        personal: {
          // Optional override. Default: ~/.alvasta-pro/credentials/whatsapp/personal
          // authDir: "~/.alvasta-pro/credentials/whatsapp/personal",
        },
        biz: {
          // Optional override. Default: ~/.alvasta-pro/credentials/whatsapp/biz
    ...
    list: [
      {
        id: "chat",
        name: "Everyday",
        workspace: "~/.alvasta-pro/workspace-chat",
        model: "anthropic/claude-sonnet-4-6",
      },
      {
        id: "opus",
        name: "Deep Work",
        workspace: "~/.alvasta-pro/workspace-opus",
