...
Alvasta Pro selects models in this order:
...
- `agents.defaults.models` is the allowlist/catalog of models Alvasta Pro can use (plus aliases).
...
- If you omit the provider, Alvasta Pro resolves the input in this order:
  1. alias match
  ...
  3. deprecated fallback to the configured default provider
     If that provider no longer exposes the configured default model, Alvasta Pro
...
alvasta-pro models set <provider/model>
alvasta-pro models set-image <provider/model>

alvasta-pro models aliases list
alvasta-pro models aliases add <alias> <provider/model>
alvasta-pro models aliases remove <alias>

alvasta-pro models fallbacks list
alvasta-pro models fallbacks add <provider/model>
alvasta-pro models fallbacks remove <provider/model>
alvasta-pro models fallbacks clear

alvasta-pro models image-fallbacks list
alvasta-pro models image-fallbacks add <provider/model>
alvasta-pro models image-fallbacks remove <provider/model>
alvasta-pro models image-fallbacks clear
```

`alvasta-pro models` (no subcommand) is a shortcut for `models status`.
...
`alvasta-pro models scan` inspects OpenRouter’s **free model catalog** and can
...
agent directory (default `~/.alvasta-pro/agents/<agentId>/agent/models.json`). This file
...
Marker persistence is source-authoritative: Alvasta Pro writes markers from the active source config snapshot (pre-resolution), not from resolved runtime secret values.
This applies whenever Alvasta Pro regenerates `models.json`, including command-driven paths like `alvasta-pro agent`.
