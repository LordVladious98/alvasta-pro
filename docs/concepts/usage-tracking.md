...
- `/usage cost` in chats: local cost summary aggregated from Alvasta Pro session logs.
- CLI: `alvasta-pro status --usage` prints a full per-provider breakdown.
- CLI: `alvasta-pro channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
...
- **MiniMax**: API key or MiniMax OAuth auth profile. Alvasta Pro treats
  ...
  quota, so Alvasta Pro inverts them before display; count-based fields win when
  present.
  ...
  - If the coding-plan endpoint returns `model_remains`, Alvasta Pro prefers the
...
can supply plugin-specific usage auth logic; otherwise Alvasta Pro falls back to
