...
- `commands.config` (default `false`) enables `/config` (reads/writes `alvasta-pro.json`).
- `commands.mcp` (default `false`) enables `/mcp` (reads/writes Alvasta Pro-managed MCP config under `mcp.servers`).
...
- `/mcp show|get|set|unset` (manage Alvasta Pro MCP server config, owner-only; requires `commands.mcp: true`)
- `/plugins list|show|get|install|enable|disable` (inspect discovered plugins, install new ones, and toggle enablement; owner-only for writes; requires `commands.plugins: true`)
  ...
  - `/plugin install <spec>` accepts the same plugin specs as `alvasta-pro plugins install`: local path/archive, npm package, or `clawhub:<pkg>`.
  - Enable/disable writes still reply with a restart hint. On a watched foreground gateway, Alvasta Pro may perform that restart automatically right after the write.
...
- For full provider usage breakdown, use `alvasta-pro status --usage`.
- `/allowlist add|remove` requires `commands.config=true` and honors channel `configWrites`.
- In multi-account channels, config-targeted `/allowlist --account <id>` and `/config set channels.<provider>.accounts.<id>...` also honor the target account's `configWrites`.
- `/usage` controls the per-response usage footer; `/usage cost` prints a local cost summary from Alvasta Pro session logs.
...
- If a run is already active, Alvasta Pro marks a live switch as pending and only restarts into the new model at a clean retry point.
...
- **Provider usage/quota** (example: “Claude 80% left”) shows up in `/status` for the current model provider when usage tracking is enabled. Alvasta Pro normalizes provider windows to `% left`; for MiniMax, remaining-only percent fields are inverted before display, and `model_remains` responses prefer the chat-model entry plus a model-tagged plan label.
...
```
/debug show
/debug set messages.responsePrefix="[alvasta-pro]"
...
- Overrides apply immediately to new config reads, but do **not** write to `alvasta-pro.json`.
- Use `/debug reset` to clear all overrides and return to the on-disk config.

## Config updates

`/config` writes to your on-disk config (`alvasta-pro.json`). Owner-only. Disabled by default; enable with `commands.config: true`.
...
/config set messages.responsePrefix="[alvasta-pro]"
...
`/mcp` writes Alvasta Pro-managed MCP server definitions under `mcp.servers`. Owner-only. Disabled by default; enable with `commands.mcp: true`.
...
- `/mcp` stores config in Alvasta Pro config, not Pi-owned project settings.
...
- **Slack:** `channels.slack.slashCommand` is still supported for a single `/alvasta-pro`-style command. If you enable `commands.native`, you must create one Slack slash command per built-in command (same names as `/help`). Command argument menus for Slack are delivered as ephemeral Block Kit buttons.
