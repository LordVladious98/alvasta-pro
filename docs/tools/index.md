---
summary: "Alvasta Pro tools and plugins overview: what the agent can do and how to extend it"
read_when:
  - You want to understand what tools Alvasta Pro provides
...
Alvasta Pro has three layers that work together:

<Steps>
  <Step title="Tools are what the agent calls">
    A tool is a typed function the agent can invoke (e.g. `exec`, `browser`,
    `web_search`, `message`). Alvasta Pro ships a set of **built-in tools** and
  ...
  <Step title="Plugins package everything together">
    ...
    Alvasta Pro), others are **external** (published on npm by the community).
...
These tools ship with Alvasta Pro and are available without installing any plugins:
...
| `group:alvasta-pro`   | All built-in Alvasta Pro tools (excludes plugin tools)                                                       |
