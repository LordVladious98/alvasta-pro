...
Alvasta Pro supports MiniMax as a `web_search` provider through the MiniMax
...
Alvasta Pro also accepts `MINIMAX_CODING_API_KEY` as an env alias. `MINIMAX_API_KEY`
...
If `plugins.entries.minimax.config.webSearch.region` is unset, Alvasta Pro resolves
...
- `query`
- `count` (Alvasta Pro trims the returned result list to the requested count)
