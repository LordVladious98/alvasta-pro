...
Alvasta Pro can convert outbound replies into audio using ElevenLabs, Microsoft, MiniMax, or OpenAI.
It works anywhere Alvasta Pro can send audio.
...
When `messages.tts.provider` is unset, Alvasta Pro picks the first configured
speech provider in registry auto-select order.

## Config

TTS config lives under `messages.tts` in `alvasta-pro.json`.
...
- If `provider` is **unset**, Alvasta Pro uses the first configured speech provider in registry auto-select order.
...
`~/.alvasta-pro/settings/tts.json`, override with `OPENCLAW_TTS_PREFS` or
...
- **Microsoft**: uses `microsoft.outputFormat` (default `audio-24khz-48kbitrate-mono-mp3`).
  ...
  - If the configured Microsoft output format fails, Alvasta Pro retries with MP3.

OpenAI/ElevenLabs output formats are fixed per channel (see above).

## Auto-TTS behavior

When enabled, Alvasta Pro:
...
Discord note: `/tts` is a built-in Discord command, so Alvasta Pro registers
...
/tts limit 2000
/tts summary off
/tts audio Hello from Alvasta Pro
```
