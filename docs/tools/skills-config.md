...
`~/.alvasta-pro/alvasta-pro.json`. Agent-specific skill visibility lives under
...
- Built-in skill roots always include `~/.alvasta-pro/skills`, `~/.agents/skills`,
...
- `install.nodeManager`: node installer preference (`npm` | `pnpm` | `yarn` | `bun`, default: npm).
  ...
  - `alvasta-pro setup --node-manager` is narrower and currently accepts `npm`,
...
- Keys under `entries` map to the skill name by default. If a skill defines
  `metadata.alvasta-pro.skillKey`, use that key instead.
- Load precedence is `<workspace>/skills` → `<workspace>/.agents/skills` →
  `~/.agents/skills` → `~/.alvasta-pro/skills` → bundled skills →
