...
Alvasta Pro supports Kimi as a `web_search` provider, using Moonshot web search
...
`https://api.moonshot.cn/v1`), Alvasta Pro reuses that same host for Kimi
...
Gateway environment. For a gateway install, put it in `~/.alvasta-pro/.env`.

If you omit `baseUrl`, Alvasta Pro defaults to `https://api.moonshot.ai/v1`.
If you omit `model`, Alvasta Pro defaults to `kimi-k2.5`.
