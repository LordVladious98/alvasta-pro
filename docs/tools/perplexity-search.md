...
Alvasta Pro supports Perplexity Search API as a `web_search` provider.
It returns structured results with `title`, `url`, and `snippet` fields.

For compatibility, Alvasta Pro also supports legacy Perplexity Sonar/OpenRouter setups.
...
**Via config:** run `alvasta-pro configure --section web`. It stores the key in
`~/.alvasta-pro/alvasta-pro.json` under `plugins.entries.perplexity.config.webSearch.apiKey`.
...
`~/.alvasta-pro/.env` (or your service environment). See [Env vars](/help/faq#env-vars-and-env-loading).
