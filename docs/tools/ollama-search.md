...
Alvasta Pro supports **Ollama Web Search** as a bundled `web_search` provider.
...
- an Ollama host that is reachable from Alvasta Pro
- `ollama signin`
...
If no explicit Ollama base URL is set, Alvasta Pro uses `http://127.0.0.1:11434`.

If your Ollama host expects bearer auth, Alvasta Pro reuses
...
- If the Ollama host is auth-protected, Alvasta Pro reuses the normal Ollama
  provider API key when present.
- Alvasta Pro warns during setup if Ollama is unreachable or not signed in, but
