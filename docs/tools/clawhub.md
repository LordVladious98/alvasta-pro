---
summary: "ClawHub guide: public registry, native Alvasta Pro install flows, and ClawHub CLI workflows"
read_when:
...
ClawHub is the public registry for **Alvasta Pro skills and plugins**.

- Use native `alvasta-pro` commands to search/install/update skills and install
...
```bash
alvasta-pro plugins install clawhub:<package>
...
Native `alvasta-pro` commands install into your active workspace and persist source
...
`alvasta-pro plugins install clawhub:...` only accepts installable plugin families.
If a ClawHub package is actually a skill, Alvasta Pro stops and points you at
`alvasta-pro skills install <slug>` instead.

## What ClawHub is

- A public registry for Alvasta Pro skills and plugins.
...
4. Users browse, download, and install skills in Alvasta Pro.
...
If you want to add new capabilities to your Alvasta Pro agent, ClawHub is the easiest way to find and install skills. You do not need to know how the backend works. You can:
...
2. Install a skill:
   - `alvasta-pro skills install <skill-slug>`
3. Start a new Alvasta Pro session so it picks up the new skill.
...
## How it fits into Alvasta Pro

Native `alvasta-pro skills install` installs into the active workspace `skills/`
directory. `alvasta-pro plugins install clawhub:...` records a normal managed
...
Community or other non-official channels can still install, but Alvasta Pro warns
...
current working directory. If an Alvasta Pro workspace is configured, `clawhub`
falls back to that workspace unless you override `--workdir` (or
`CLAWHUB_WORKDIR`). Alvasta Pro loads workspace skills from `<workspace>/skills`
and will pick them up in the **next** session. If you already use
`~/.alvasta-pro/skills` or bundled skills, workspace skills take precedence.
...
A skill is a versioned bundle of files that teaches Alvasta Pro how to perform a
...
Interested in becoming a moderator? Ask in the Alvasta Pro Discord and contact a
...
- `--workdir <dir>`: Working directory (default: current dir; falls back to Alvasta Pro workspace).
...
Code plugins must include the required Alvasta Pro metadata in `package.json`:
...
`clawhub sync` scans your current workdir first. If no skills are found, it falls back to known legacy locations (for example `~/alvasta-pro/skills` and `~/.alvasta-pro/skills`). This is designed to find older skill installs without extra flags.
