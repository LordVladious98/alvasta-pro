...
- **Debugging blocked tools**: see [Sandbox vs Tool Policy vs Elevated](/gateway/sandbox-vs-tool-policy-vs-elevated) and `alvasta-pro sandbox explain`.
...
Legacy `agent.*` configs are migrated by `alvasta-pro doctor`; prefer `agents.defaults` + `agents.list` going forward.
...
2. **Verify sandbox containers:**
   ...
   ```exec
   docker ps --filter "name=alvasta-pro-sbx-"
   ```
...
4. **Monitor logs:**
   ...
   ```exec
   tail -f "${OPENCLAW_STATE_DIR:-$HOME/.alvasta-pro}/logs/gateway.log" | grep -E "routing|sandbox|tools"
   ```
