...
read_when:
  - Running Alvasta Pro Gateway in WSL2 while Chrome lives on Windows
...
- Alvasta Pro Gateway runs inside WSL2
- Chrome runs on Windows
- browser control must cross the WSL2/Windows boundary

It also covers the layered failure pattern from [issue #39369](https://github.com/alvasta-pro/alvasta-pro/issues/39369): several independent problems can show up at once, which makes the wrong layer look broken first.
...
- Alvasta Pro and Chrome are on the same machine
...
- Alvasta Pro points a browser profile at the address that is reachable from WSL2
...
If this fails on Windows, Alvasta Pro is not the problem yet.
...
Fix that before touching Alvasta Pro config.

### Layer 3: Configure the correct browser profile

For raw remote CDP, point Alvasta Pro at the address that is reachable from WSL2:
...
- use HTTP(S) when you want Alvasta Pro to discover `/json/version`
- use WS(S) only when the browser provider gives you a direct DevTools socket URL
- test the same URL with `curl` before expecting Alvasta Pro to succeed
...
```bash
alvasta-pro browser open https://example.com --browser-profile remote
alvasta-pro browser tabs --browser-profile remote
```

Good result:

- the tab opens in Windows Chrome
- `alvasta-pro browser tabs` returns the target
...
- stale viewport / dark-mode / locale / offline overrides after a remote session
  - run `alvasta-pro browser stop --browser-profile remote`
...
3. Alvasta Pro config: does `browser.profiles.<name>.cdpUrl` use that exact WSL2-reachable address?
...
- only then debug Alvasta Pro config or Control UI auth
