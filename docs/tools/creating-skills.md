...
<Steps>
  <Step title="Create the skill directory">
    ...
    ```bash
    mkdir -p ~/.alvasta-pro/workspace/skills/hello-world
    ```
  ...
  <Step title="Load the skill">
    Start a new session so Alvasta Pro picks up the skill:
  ...
  <Step title="Test it">
    ...
    ```bash
    alvasta-pro agent --message "give me a greeting"
    ```
...
| `metadata.alvasta-pro.os`              | No       | OS filter (`["darwin"]`, `["linux"]`, etc.) |
| `metadata.alvasta-pro.requires.bins`   | No       | Required binaries on PATH                   |
| `metadata.alvasta-pro.requires.config` | No       | Required config keys                        |
...
- **Test locally** — use `alvasta-pro agent --message "..."` to test before sharing
...
| `~/.alvasta-pro/skills/`           | Medium     | Shared (all agents)   |
| Bundled (shipped with Alvasta Pro) | Low        | Global                |
