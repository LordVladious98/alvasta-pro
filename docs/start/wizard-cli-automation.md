---
summary: "Scripted onboarding and agent setup for the Alvasta Pro CLI"
read_when:
...
Use `--non-interactive` to automate `alvasta-pro onboard`.
...
Anthropic setup-token remains available as a supported onboarding token path, but Alvasta Pro now prefers Claude CLI reuse when available.
