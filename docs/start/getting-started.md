---
summary: "Get Alvasta Pro installed and run your first chat in minutes."
read_when:
...
Install Alvasta Pro, run onboarding, and chat with your AI assistant — all in
...
```bash
mkdir -p "$HOME/.alvasta-pro/control-ui-custom"
...
```json
{
  "gateway": {
    "controlUi": {
      "enabled": true,
      "root": "$HOME/.alvasta-pro/control-ui-custom"
    }
  }
}
```
...
<Accordion title="Advanced: environment variables">
  If you run Alvasta Pro as a service account or want custom paths:
