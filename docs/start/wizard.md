...
CLI onboarding is the **recommended** way to set up Alvasta Pro on macOS,
...
`alvasta-pro dashboard` and chat in the browser. Docs: [Dashboard](/web/dashboard).
</Info>
...
`alvasta-pro configure --section web`. Docs: [Web tools](/tools/web).
</Tip>
...
2. **Workspace** — Location for agent files (default `~/.alvasta-pro/workspace`). Seeds bootstrap files.
...
If the config is invalid or contains legacy keys, onboarding asks you to run `alvasta-pro doctor` first.
</Note>
...
Use `alvasta-pro agents add <name>` to create a separate agent with its own workspace,
...
- Default workspaces follow `~/.alvasta-pro/workspace-<agentId>`.
...
- CLI command reference: [`alvasta-pro onboard`](/cli/onboard)
