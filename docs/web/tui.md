...
```bash
alvasta-pro tui --url ws://<host>:<port> --token <gateway-token>
```
...
- Turn delivery on:
  ...
  - or start with `alvasta-pro tui --deliver`
...
- If your terminal uses a light background and auto-detection is wrong, set `OPENCLAW_THEME=light` before launching `alvasta-pro tui`.
...
- Check the Gateway logs: `alvasta-pro logs --follow`.
- Confirm the agent can run: `alvasta-pro status` and `alvasta-pro models status`.
- If you expect messages in a chat channel, enable delivery (`/deliver on` or `--deliver`).

## Connection troubleshooting

- `disconnected`: ensure the Gateway is running and your `--url/--token/--password` are correct.
- No agents in picker: check `alvasta-pro agents list` and your routing config.
