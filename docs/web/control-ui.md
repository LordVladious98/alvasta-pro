...
- optional prefix: set `gateway.controlUi.basePath` (e.g. `/alvasta-pro`)
...
If the page fails to load, start the Gateway first: `alvasta-pro gateway`.
...
created. Re-run `alvasta-pro devices list` before approval.

Once approved, the device is remembered and won't require re-approval unless
you revoke it with `alvasta-pro devices revoke --device <id> --role <role>`. See
...
- Config: view/edit `~/.alvasta-pro/alvasta-pro.json` (`config.get`, `config.set`)
...
- Stop:
  ...
  - Type `/stop` (or standalone abort phrases like `stop`, `stop action`, `stop run`, `stop alvasta-pro`, `please stop`) to abort out-of-band
...
(`tailscale-user-login`) when `gateway.auth.allowTailscale` is `true`. Alvasta Pro
...
```bash
alvasta-pro gateway --bind tailnet --token "$(openssl rand -hex 32)"
```
...
Alvasta Pro **blocks** Control UI connections without device identity.
...
```bash
OPENCLAW_CONTROL_UI_BASE_PATH=/alvasta-pro/ pnpm ui:build
```
