...
- Re-open anytime: `alvasta-pro dashboard` (copies link, opens browser if possible, shows SSH hint if headless).
...
- **Shared-secret token source**: `gateway.auth.token` (or
  `OPENCLAW_GATEWAY_TOKEN`); `alvasta-pro dashboard` can pass it via URL fragment
...
- If `gateway.auth.token` is SecretRef-managed, `alvasta-pro dashboard`
...
- If `gateway.auth.token` is configured as a SecretRef and is unresolved in your
  current shell, `alvasta-pro dashboard` still prints a non-tokenized URL plus
...
- Ensure the gateway is reachable (local: `alvasta-pro status`; remote: SSH tunnel `ssh -N -L 18789:127.0.0.1:18789 user@host` then open `http://127.0.0.1:18789/`).
...
- Retrieve or supply the shared secret from the gateway host:
  - Token: `alvasta-pro config get gateway.auth.token`
  ...
  - SecretRef-managed token: resolve the external secret provider or export
    `OPENCLAW_GATEWAY_TOKEN` in this shell, then rerun `alvasta-pro dashboard`
  - No shared secret configured: `alvasta-pro doctor --generate-gateway-token`
