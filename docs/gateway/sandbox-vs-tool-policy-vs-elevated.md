...
Alvasta Pro has three related (but different) controls:
...
Use the inspector to see what Alvasta Pro is _actually_ doing:
...
- Alvasta Pro validates bind sources twice: first on the normalized source path, then again after resolving through the deepest existing ancestor. Symlink-parent escapes do not bypass blocked-path or allowed-root checks.
...
- `group:alvasta-pro`: all built-in Alvasta Pro tools (excludes provider plugins)
