---
summary: "How Alvasta Pro sandboxing works: modes, scopes, workspace access, and images"
title: Sandboxing
...
Alvasta Pro can run **tools inside sandbox backends** to reduce blast radius.
...
- Optional sandboxed browser (`agents.defaults.sandbox.browser`).
  ...
  - By default, sandbox browser containers use a dedicated Docker network (`alvasta-pro-sandbox-browser`) instead of the global `bridge` network.
  ...
  - noVNC observer access is password-protected by default; Alvasta Pro emits a short-lived token URL that serves a local bootstrap page and opens noVNC with password in URL fragment (not query/header logs).
...
Use `backend: "ssh"` when you want Alvasta Pro to sandbox `exec`, file tools, and media reads on
...
- Alvasta Pro creates a per-scope remote root under `sandbox.ssh.workspaceRoot`.
- On first use after create or recreate, Alvasta Pro seeds that remote workspace from the local workspace once.
...
- Alvasta Pro does not sync remote changes back to the local workspace automatically.

Authentication material:

- `identityFile`, `certificateFile`, `knownHostsFile`: use existing local files and pass them through OpenSSH config.
- `identityData`, `certificateData`, `knownHostsData`: use inline strings or SecretRefs. Alvasta Pro resolves them through the normal secrets runtime snapshot, writes them to temp files with `0600`, and deletes them when the SSH session ends.
...
- Host-local edits made outside Alvasta Pro after the seed step are not visible remotely until you recreate the sandbox.
- `alvasta-pro sandbox recreate` deletes the per-scope remote root and seeds again from local on next use.
- Browser sandboxing is not supported on the SSH backend.
- `sandbox.docker.*` settings do not apply to the SSH backend.

### OpenShell backend

Use `backend: "openshell"` when you want Alvasta Pro to sandbox tools in an
...
- `mirror` (default): local workspace stays canonical. Alvasta Pro syncs local files into OpenShell before exec and syncs the remote workspace back after exec.
- `remote`: OpenShell workspace is canonical after the sandbox is created. Alvasta Pro seeds the remote workspace once from the local workspace, then file tools and exec run directly against the remote sandbox without syncing changes back.

Remote transport details:

- Alvasta Pro asks OpenShell for sandbox-specific SSH config via `openshell sandbox ssh-config <name>`.
...
- Before `exec`, Alvasta Pro syncs the local workspace into the OpenShell sandbox.
- After `exec`, Alvasta Pro syncs the remote workspace back to the local workspace.
- File tools still operate through the sandbox bridge, but the local workspace remains the source of truth between turns.

Use this when:

- you edit files locally outside Alvasta Pro and want those changes to show up in the sandbox automatically
...
- When the sandbox is first created, Alvasta Pro seeds the remote workspace from the local workspace once.
- After that, `exec`, `read`, `write`, `edit`, and `apply_patch` operate directly against the remote OpenShell workspace.
- Alvasta Pro does **not** sync remote changes back into the local workspace after exec.
...
- If you edit files on the host outside Alvasta Pro after the seed step, the remote sandbox will **not** see those changes automatically.
...
- `alvasta-pro sandbox list` shows OpenShell runtimes as well as Docker runtimes
- `alvasta-pro sandbox recreate` deletes the current runtime and lets Alvasta Pro recreate it on next use
...
- `"none"` (default): tools see a sandbox workspace under `~/.alvasta-pro/sandboxes`.
...
Alvasta Pro mirrors eligible skills into the sandbox workspace (`.../skills`) so
...
- Alvasta Pro blocks dangerous bind sources (for example: `docker.sock`, `/etc`, `/proc`, `/sys`, `/dev`, and parent mounts that would expose them).
- Alvasta Pro also blocks common home-directory credential roots such as `~/.aws`, `~/.cargo`, `~/.config`, `~/.docker`, `~/.gnupg`, `~/.netrc`, `~/.npm`, and `~/.ssh`.
- Bind validation is not just string matching. Alvasta Pro normalizes the source path, then resolves it again through the deepest existing ancestor before re-checking blocked paths and allowed roots.
...
Default Docker image: `alvasta-pro-sandbox:bookworm-slim`
...
`alvasta-pro-sandbox-common:bookworm-slim`.
...
- Use `alvasta-pro sandbox explain` to inspect effective sandbox mode, tool policy, and fix-it config keys.
