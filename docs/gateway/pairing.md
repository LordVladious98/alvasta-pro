...
alvasta-pro nodes rename --node <id|name|ip> --name "Living Room iPad"
```
...
Pairing state is stored under the Gateway state directory (default `~/.alvasta-pro`):

- `~/.alvasta-pro/nodes/paired.json`
- `~/.alvasta-pro/nodes/pending.json`
