...
Alvasta Pro has two log “surfaces”:
...
- Default rolling log file is under `/tmp/alvasta-pro/` (one file per day): `alvasta-pro-YYYY-MM-DD.log`
  - Date uses the gateway host's local timezone.
- The log file path and level can be configured via `~/.alvasta-pro/alvasta-pro.json`:
  - `logging.file`
  - `logging.level`
...
`alvasta-pro gateway` supports a per-gateway style switch:
