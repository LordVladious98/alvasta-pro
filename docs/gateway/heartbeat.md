...
- During heartbeat runs, Alvasta Pro treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `alvasta-pro sessions --json` or the [sessions CLI](/cli/sessions)).
...
- If only alert delivery is disabled, Alvasta Pro can still run the heartbeat, update due-task timestamps, restore the session idle timestamp, and suppress the outward alert payload.
...
If **all three** are false, Alvasta Pro skips the heartbeat run entirely (no model call).
...
- Alvasta Pro parses the `tasks:` block and checks each task against its own `interval`.
...
```bash
alvasta-pro system event --text "Check for urgent follow-ups" --mode now
```
