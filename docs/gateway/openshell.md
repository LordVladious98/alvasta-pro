---
title: OpenShell
summary: "Use OpenShell as a managed sandbox backend for Alvasta Pro agents"
read_when:
...
OpenShell is a managed sandbox backend for Alvasta Pro. Instead of running Docker
containers locally, Alvasta Pro delegates sandbox lifecycle to the `openshell` CLI,
...
- Alvasta Pro Gateway running on the host
...
2. Restart the Gateway. On the next agent turn, Alvasta Pro creates an OpenShell
...
- You edit files locally outside Alvasta Pro and want those changes visible in the
...
- When the sandbox is first created, Alvasta Pro seeds the remote workspace from
...
- Alvasta Pro does **not** sync remote changes back into the local workspace.
...
Important: if you edit files on the host outside Alvasta Pro after the initial seed,
the remote sandbox does **not** see those changes. Use
`alvasta-pro sandbox recreate` to re-seed.
...
| `from`                    | `string`                 | `"alvasta-pro"`  | Sandbox source for first-time create                  |
...
1. Alvasta Pro calls `openshell sandbox create` (with `--from`, `--gateway`,
   `--policy`, `--providers`, `--gpu` flags as configured).
2. Alvasta Pro calls `openshell sandbox ssh-config <name>` to get SSH connection
...
- [Sandbox CLI](/cli/sandbox) -- `alvasta-pro sandbox` commands
