...
`alvasta-pro doctor` is the repair + migration tool for Alvasta Pro. It fixes stale
...
```bash
alvasta-pro doctor --yes
```

Accept defaults without prompting (including restart/service/sandbox repair steps when applicable).

```bash
alvasta-pro doctor --repair
```

Apply recommended repairs without prompting (repairs + restarts where safe).

```bash
alvasta-pro doctor --repair --force
```

Apply aggressive repairs too (overwrites custom supervisor configs).

```bash
alvasta-pro doctor --non-interactive
```
...
```bash
cat ~/.alvasta-pro/alvasta-pro.json
```
...
- Extra workspace dir detection (`~/alvasta-pro`).
...
you to run `alvasta-pro doctor`.

Doctor will:

- Explain which legacy keys were found.
- Show the migration it applied.
- Rewrite `~/.alvasta-pro/alvasta-pro.json` with the updated schema.

The Gateway also auto-runs doctor migrations on startup when it detects a
legacy config format, so stale configs are repaired without manual intervention.
Cron job store migrations are handled by `alvasta-pro doctor --fix`.
...
- Sessions store + transcripts:
  - from `~/.alvasta-pro/sessions/` to `~/.alvasta-pro/agents/<agentId>/sessions/`
- Agent dir:
  - from `~/.alvasta-pro/agent/` to `~/.alvasta-pro/agents/<agentId>/agent/`
- WhatsApp auth state (Baileys):
  - from legacy `~/.alvasta-pro/credentials/*.json` (except `oauth.json`)
  - to `~/.alvasta-pro/credentials/whatsapp/<accountId>/...` (default account id: `default`)

These migrations are best-effort and idempotent; doctor will emit warnings when
it leaves any legacy folders behind as backups. The Gateway/CLI also auto-migrates
the legacy sessions + agent dir on startup so history/auth/models land in the
per-agent path without a manual doctor run. WhatsApp auth is intentionally only
migrated via `alvasta-pro doctor`. Talk provider/provider-map normalization now
...
Doctor also checks the cron job store (`~/.alvasta-pro/cron/jobs.json` by default,
...
- **Multiple state dirs**: warns when multiple `~/.alvasta-pro` folders exist across
  home directories or when `OPENCLAW_STATE_DIR` points elsewhere (history can
  split between installs).
- **Remote mode reminder**: if `gateway.mode=remote`, doctor reminds you to run
  it on the remote host (the state lives there).
- **Config file permissions**: warns if `~/.alvasta-pro/alvasta-pro.json` is
...
Discord plugin runtime packages) are present in the Alvasta Pro install root.
If any are missing, doctor reports the packages and installs them in
`alvasta-pro doctor --fix` / `alvasta-pro doctor --repair` mode.

### 8) Gateway service migrations and cleanup hints

Doctor detects legacy gateway services (launchd/systemd/schtasks) and
offers to remove them and install the Alvasta Pro service using the current gateway
port. It can also scan for extra gateway-like services and print cleanup hints.
Profile-named Alvasta Pro gateway services are considered first-class and are not
flagged as "extra."
...
startup continues. In read-only mode (`alvasta-pro doctor` without `--fix`) this check
is skipped entirely.
...
- **Legacy workspace dirs**: warns when `~/alvasta-pro` or other legacy workspace directories
...
- If the shell profile uses a slow dynamic completion pattern
  (`source <(alvasta-pro completion ...)`), doctor upgrades it to the faster
  cached file variant.
...
Run `alvasta-pro completion --write-state` to regenerate the cache manually.
...
- `alvasta-pro doctor --generate-gateway-token` forces generation only when no token SecretRef is configured.
...
- `alvasta-pro doctor --fix` now uses the same read-only SecretRef summary model as status-family commands for targeted config repairs.
...
Use `alvasta-pro memory status --deep` to verify embedding readiness at runtime.
...
- `alvasta-pro doctor` prompts before rewriting supervisor config.
- `alvasta-pro doctor --yes` accepts the default repair prompts.
- `alvasta-pro doctor --repair` applies recommended fixes without prompts.
- `alvasta-pro doctor --repair --force` overwrites custom supervisor configs.
...
- You can always force a full rewrite via `alvasta-pro gateway install --force`.
