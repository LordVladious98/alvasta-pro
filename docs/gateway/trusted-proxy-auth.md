...
read_when:
  - Running Alvasta Pro behind an identity-aware proxy
  - Setting up Pomerium, Caddy, or nginx with OAuth in front of Alvasta Pro
...
- You run Alvasta Pro behind an **identity-aware proxy** (Pomerium, Caddy + OAuth, nginx + oauth2-proxy, Traefik + forward auth)
...
3. Alvasta Pro checks that the request came from a **trusted proxy IP** (configured in `gateway.trustedProxies`)
4. Alvasta Pro extracts the user identity from the configured header
...
- For same-host loopback proxy setups, use token/password auth instead, or route through a non-loopback trusted proxy address that Alvasta Pro can verify.
...
- Alvasta Pro can stay on loopback HTTP behind the proxy.
...
If Alvasta Pro itself serves HTTPS directly (no TLS-terminating proxy), set:
...
```yaml
routes:
  - from: https://alvasta-pro.example.com
    to: http://alvasta-pro-gateway:18789
    policy:
      - allow:
          or:
            - email:
...
```
alvasta-pro.example.com {
    authenticate with oauth2_provider
    authorize with policy1

    reverse_proxy alvasta-pro:18789 {
...
location / {
    ...
    proxy_pass http://alvasta-pro:18789;
...
Alvasta Pro rejects ambiguous configurations where both a `gateway.auth.token` (or `OPENCLAW_GATEWAY_TOKEN`) and `trusted-proxy` mode are active at the same time. Mixed token configs can cause loopback requests to silently authenticate on the wrong auth path.
...
optionally declare operator scopes with `x-alvasta-pro-scopes`.

Examples:

- `x-alvasta-pro-scopes: operator.read`
- `x-alvasta-pro-scopes: operator.read,operator.write`
- `x-alvasta-pro-scopes: operator.admin,operator.write`

Behavior:

- When the header is present, Alvasta Pro honors the declared scope set.
...
- Gateway-auth **plugin HTTP routes** are narrower by default: when `x-alvasta-pro-scopes` is absent, their runtime scope falls back to `operator.write`.
...
- Send `x-alvasta-pro-scopes` explicitly when you want a trusted-proxy request to
...
`alvasta-pro security audit` will flag trusted-proxy auth with a **critical** severity finding. This is intentional — it's a reminder that you're delegating security to your proxy setup.
...
Alvasta Pro rejected a loopback-source trusted-proxy request.
...
3. Update Alvasta Pro config with trusted-proxy auth
4. Restart the Gateway
5. Test WebSocket connections from the Control UI
6. Run `alvasta-pro security audit` and review findings
