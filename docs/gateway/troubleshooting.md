...
- `alvasta-pro gateway status` shows `Runtime: running` and `RPC probe: ok`.
- `alvasta-pro doctor` reports no blocking config/service issues.
- `alvasta-pro channels status --probe` shows live per-account transport status and,
...
alvasta-pro config get agents.defaults.models
```
...
alvasta-pro status
alvasta-pro channels status --probe
alvasta-pro pairing list --channel <channel> [--account <id>]
alvasta-pro config get channels
alvasta-pro logs --follow
```
...
| ---------------------------- | -------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `AUTH_TOKEN_MISSING`         | Client did not send a required shared token.             | Paste/set token in the client and retry. For dashboard paths: `alvasta-pro config get gateway.auth.token` then paste into Control UI settings.                                                                                                                                              |
...
| `PAIRING_REQUIRED`           | Device identity is known but not approved for this role. | Approve pending request: `alvasta-pro devices list` then `alvasta-pro devices approve <requestId>`.                                                                                                                                                                                            |
...
If `alvasta-pro devices rotate` / `revoke` / `remove` is denied unexpectedly:

- paired-device token sessions can manage only **their own** device unless the
  caller also has `operator.admin`
- `alvasta-pro devices rotate --scope ...` can only request operator scopes that
...
alvasta-pro logs --follow
alvasta-pro doctor
alvasta-pro gateway status --deep   # also scan system-level services
```
...
- `Gateway start blocked: set gateway.mode=local` or `existing config is missing gateway.mode` → local gateway mode is not enabled, or the config file was clobbered and lost `gateway.mode`. Fix: set `gateway.mode="local"` in your config, or re-run `alvasta-pro onboard --mode local` / `alvasta-pro setup` to restamp the expected local-mode config. If you are running Alvasta Pro via Podman, the default config path is `~/.alvasta-pro/alvasta-pro.json`.
...
Use this when `alvasta-pro gateway probe` reaches something, but still prints a warning block.
...
```bash
alvasta-pro cron status
alvasta-pro cron list
alvasta-pro cron runs --id <jobId> --limit 20
alvasta-pro system heartbeat last
alvasta-pro logs --follow
```
...
- `heartbeat skipped` with `reason=empty-heartbeat-file` → `HEARTBEAT.md` exists but only contains blank lines / markdown headers, so Alvasta Pro skips the model call.
...
alvasta-pro approvals get --node <idOrNameOrIp>
alvasta-pro logs --follow
alvasta-pro status
```
...
alvasta-pro browser start --browser-profile alvasta-pro
...
- stale viewport / dark-mode / locale / offline overrides on attach-only or remote CDP profiles → run `alvasta-pro browser stop --browser-profile <name>` to close the active control session and release Playwright/CDP emulation state without restarting the whole gateway.
...
```bash
alvasta-pro gateway status
alvasta-pro config get gateway.mode
alvasta-pro config get gateway.remote.url
alvasta-pro config get gateway.auth.mode
```
...
```bash
alvasta-pro config get gateway.bind
alvasta-pro config get gateway.auth.mode
