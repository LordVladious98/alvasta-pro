---
summary: "Host Alvasta Pro on Oracle Cloud's Always Free ARM tier"
read_when:
  - Setting up Alvasta Pro on Oracle Cloud
  - Looking for free VPS hosting for Alvasta Pro
...
Run a persistent Alvasta Pro Gateway on Oracle Cloud's **Always Free** ARM tier (up to 4 OCPU, 24 GB RAM, 200 GB storage) at no cost.
...
<Steps>
  ...
  <Step title="Install Tailscale">
    ...
    From now on, connect via Tailscale: `ssh ubuntu@alvasta-pro`.
...
**Tailscale will not connect** -- Run `sudo tailscale up --ssh --hostname=alvasta-pro --reset` to re-authenticate.

**Gateway will not start** -- Run `alvasta-pro doctor --non-interactive` and check logs with `journalctl --user -u alvasta-pro-gateway.service -n 50`.
...
- [Updating](/install/updating) -- keep Alvasta Pro up to date
