---
summary: "Run Alvasta Pro Gateway 24/7 on an Azure Linux VM with durable state"
read_when:
  - You want Alvasta Pro running 24/7 on Azure with Network Security Group hardening
  - You want a production-grade, always-on Alvasta Pro Gateway on your own Azure Linux VM
  - You want secure administration with Azure Bastion SSH
title: "Azure"
---

# Alvasta Pro on Azure Linux VM

This guide sets up an Azure Linux VM with the Azure CLI, applies Network Security Group (NSG) hardening, configures Azure Bastion for SSH access, and installs Alvasta Pro.
...
- Install Alvasta Pro with the installer script
- Verify the Gateway
...
<Steps>
  ...
  <Step title="Set deployment variables">
    ...
    RG="rg-alvasta-pro"
    LOCATION="westus2"
    VNET_NAME="vnet-alvasta-pro"
    VNET_PREFIX="10.40.0.0/16"
    VM_SUBNET_NAME="snet-alvasta-pro-vm"
    VM_SUBNET_PREFIX="10.40.2.0/24"
    BASTION_SUBNET_PREFIX="10.40.1.0/26"
    NSG_NAME="nsg-alvasta-pro-vm"
    VM_NAME="vm-alvasta-pro"
    ADMIN_USERNAME="alvasta-pro"
    BASTION_NAME="bas-alvasta-pro"
    BASTION_PIP_NAME="pip-alvasta-pro-bastion"
    ```
...
<Steps>
  ...
  <Step title="Install Alvasta Pro (in the VM shell)">
    ```bash
    curl -fsSL https://alvasta-pro.ai/install.sh -o /tmp/install.sh
    bash /tmp/install.sh
    rm -f /tmp/install.sh
    ```

    The installer installs Node LTS and dependencies if not already present, installs Alvasta Pro, and launches the onboarding wizard. See [Install](/install) for details.
  ...
  <Step title="Verify the Gateway">
    ...
    Most enterprise Azure teams already have GitHub Copilot licenses. If that is your case, we recommend choosing the GitHub Copilot provider in the Alvasta Pro onboarding wizard. See [GitHub Copilot provider](/providers/github-copilot).
...
- **Deallocate the VM** when not in use (stops compute billing; disk charges remain). The Alvasta Pro Gateway will not be reachable while the VM is deallocated — restart it when you need it live again:
...
- For more details on Alvasta Pro Azure deployment with the GitHub Copilot model provider: [Alvasta Pro on Azure with GitHub Copilot](https://github.com/johnsonshi/alvasta-pro-azure-github-copilot)
