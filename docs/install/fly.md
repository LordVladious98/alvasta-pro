---
title: Fly.io
summary: "Step-by-step Fly.io deployment for Alvasta Pro with persistent storage and HTTPS"
read_when:
  - Deploying Alvasta Pro on Fly.io
  - Setting up Fly volumes, secrets, and first-run config
---

# Fly.io Deployment

**Goal:** Alvasta Pro Gateway running on a [Fly.io](https://fly.io) machine with persistent storage, automatic HTTPS, and Discord/channel access.
...
<Steps>
  <Step title="Create the Fly app">
    ...
    fly apps create my-alvasta-pro

    # Create a persistent volume (1GB is usually enough)
    fly volumes create alvasta-pro_data --size 1 --region iad
    ```
  ...
  <Step title="Configure fly.toml">
    ...
    **Security note:** The default config exposes a public URL. For a hardened deployment with no public IP, see [Private Deployment](#private-deployment-hardened) or use `fly.private.toml`.
    ...
    ```toml
    app = "my-alvasta-pro"  # Your app name
    primary_region = "iad"
  ...
  <Step title="Set secrets">
    ...
    **Notes:**
    ...
    - **Prefer env vars over config file** for all API keys and tokens. This keeps secrets out of `alvasta-pro.json` where they could be accidentally exposed or logged.
  ...
  <Step title="Create config file">
    ...
    **Note:** With `OPENCLAW_STATE_DIR=/data`, the config path is `/data/alvasta-pro.json`.
  ...
  <Step title="Access the Gateway">
    ...
    Or visit `https://my-alvasta-pro.fly.dev/`
...
`--allow-unconfigured` only bypasses the startup guard. It does not create or repair `/data/alvasta-pro.json`, so make sure your real config exists and includes `gateway.mode="local"` when you want a normal local gateway start.

Verify the config exists:

```bash
fly ssh console --command "cat /data/alvasta-pro.json"
```
...
echo '{"your":"config"}' | fly ssh console -C "tee /data/alvasta-pro.json"

# Or use sftp
fly sftp shell
> put /local/path/config.json /data/alvasta-pro.json
```

**Note:** `fly sftp` may fail if the file already exists. Delete first:

```bash
fly ssh console --command "rm /data/alvasta-pro.json"
```
...
```bash
# List current IPs
fly ips list -a my-alvasta-pro

# Release public IPs
fly ips release <public-ipv4> -a my-alvasta-pro
fly ips release <public-ipv6> -a my-alvasta-pro
...
fly ips allocate-v6 --private -a my-alvasta-pro
```
...
fly proxy 3000:3000 -a my-alvasta-pro
...
```bash
fly ssh console -a my-alvasta-pro
```
...
- Keep Alvasta Pro up to date: [Updating](/install/updating)
