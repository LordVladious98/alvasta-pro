---
summary: "Move (migrate) an Alvasta Pro install from one machine to another"
read_when:
  - You are moving Alvasta Pro to a new laptop/server
  - You want to preserve sessions, auth, and channel logins (WhatsApp, etc.)
title: "Migration Guide"
---

# Migrating Alvasta Pro to a New Machine

This guide moves an Alvasta Pro gateway to a new machine without redoing onboarding.

## What Gets Migrated

When you copy the **state directory** (`~/.alvasta-pro/` by default) and your **workspace**, you preserve:

- **Config** -- `alvasta-pro.json` and all gateway settings
...
<Tip>
Run `alvasta-pro status` on the old machine to confirm your state directory path.
Custom profiles use `~/.alvasta-pro-<profile>/` or a path set via `OPENCLAW_STATE_DIR`.
</Tip>
...
<Steps>
  <Step title="Stop the gateway and back up">
    ...
    cd ~
    tar -czf alvasta-pro-state.tgz .alvasta-pro
    ```

    If you use multiple profiles (e.g. `~/.alvasta-pro-work`), archive each separately.

  </Step>

  <Step title="Install Alvasta Pro on the new machine">
    [Install](/install) the CLI (and Node if needed) on the new machine.
    It is fine if onboarding creates a fresh `~/.alvasta-pro/` -- you will overwrite it next.
  </Step>
...
<AccordionGroup>
  <Accordion title="Profile or state-dir mismatch">
    ...
    Launch the gateway with the **same** profile or state-dir you migrated, then rerun `alvasta-pro doctor`.
  </Accordion>

  <Accordion title="Copying only alvasta-pro.json">
...
- [ ] `alvasta-pro status` shows the gateway running
