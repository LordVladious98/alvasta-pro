---
summary: "Optional Docker-based setup and onboarding for Alvasta Pro"
read_when:
...
- **Yes**: you want an isolated, throwaway gateway environment or to run Alvasta Pro on a host without local installs.
...
<Steps>
  <Step title="Build the image">
    ...
    ```bash
    export OPENCLAW_IMAGE="ghcr.io/alvasta-pro/alvasta-pro:latest"
    ./scripts/docker/setup.sh
    ```

    Pre-built images are published at the
    [GitHub Container Registry](https://github.com/alvasta-pro/alvasta-pro/pkgs/container/alvasta-pro).
  ...
  <Step title="Complete onboarding">
    ...
    `alvasta-pro-gateway` directly. `alvasta-pro-cli` is for commands you run after
  ...
  <Step title="Open the Control UI">
    ...
    ```bash
    docker compose run --rm alvasta-pro-cli dashboard --no-open
    ```
  ...
  <Step title="Configure channels (optional)">
    ...
    ```bash
    # WhatsApp (QR)
    docker compose run --rm alvasta-pro-cli channels login

    # Telegram
    docker compose run --rm alvasta-pro-cli channels add --channel telegram --token "<token>"

    # Discord
    docker compose run --rm alvasta-pro-cli channels add --channel discord --token "<token>"
    ```
...
```bash
docker build -t alvasta-pro:local -f Dockerfile .
docker compose run --rm --no-deps --entrypoint node alvasta-pro-gateway \
  dist/index.js onboard --mode local --no-install-daemon
docker compose run --rm --no-deps --entrypoint node alvasta-pro-gateway \
  dist/index.js config set --batch-json '[{"path":"gateway.mode","value":"local"},{"path":"gateway.bind","value":"lan"},{"path":"gateway.controlUi.allowedOrigins","value":["http://localhost:18789","http://127.0.0.1:18789"]}]'
docker compose up -d alvasta-pro-gateway
```
...
<Note>
Because `alvasta-pro-cli` shares `alvasta-pro-gateway`'s network namespace, it is a
post-start tool. Before `docker compose up -d alvasta-pro-gateway`, run onboarding
and setup-time config writes through `alvasta-pro-gateway` with
...
```bash
docker compose exec alvasta-pro-gateway node dist/index.js health --token "$OPENCLAW_GATEWAY_TOKEN"
```
...
Docker Compose bind-mounts `OPENCLAW_CONFIG_DIR` to `/home/node/.alvasta-pro` and
`OPENCLAW_WORKSPACE_DIR` to `/home/node/.alvasta-pro/workspace`, so those paths
survive container replacement.

That mounted config directory is where Alvasta Pro keeps:

- `alvasta-pro.json` for behavior config
...
and rolling file logs under `/tmp/alvasta-pro/`.
...
```bash
mkdir -p ~/.clawdock && curl -sL https://raw.githubusercontent.com/alvasta-pro/alvasta-pro/main/scripts/clawdock/clawdock-helpers.sh -o ~/.clawdock/clawdock-helpers.sh
...
<AccordionGroup>
  ...
  <Accordion title="Automation / CI (non-interactive)">
    ...
    ```bash
    docker compose run -T --rm alvasta-pro-cli gateway probe
    docker compose run -T --rm alvasta-pro-cli devices list --json
    ```

  </Accordion>

  <Accordion title="Shared-network security note">
    `alvasta-pro-cli` uses `network_mode: "service:alvasta-pro-gateway"` so CLI
    ...
    `no-new-privileges` on `alvasta-pro-cli`.
  </Accordion>

  <Accordion title="Permissions and EACCES">
    The image runs as `node` (uid 1000). If you see permission errors on
    `/home/node/.alvasta-pro`, make sure your host bind mounts are owned by uid 1000:

    ```bash
    sudo chown -R 1000:1000 /path/to/alvasta-pro-config /path/to/alvasta-pro-workspace
    ```
  ...
  <Accordion title="Power-user container options">
    ...
    1. **Persist `/home/node`**: `export OPENCLAW_HOME_VOLUME="alvasta-pro_home"`
...
<AccordionGroup>
  <Accordion title="Image missing or sandbox container not starting">
    ...
    [`scripts/sandbox-setup.sh`](https://github.com/alvasta-pro/alvasta-pro/blob/main/scripts/sandbox-setup.sh)
  ...
  <Accordion title="Custom tools not found in sandbox">
    Alvasta Pro runs commands with `sh -lc` (login shell), which sources
  ...
  <Accordion title="Unauthorized or pairing required in Control UI">
    ...
    ```bash
    docker compose run --rm alvasta-pro-cli dashboard --no-open
    docker compose run --rm alvasta-pro-cli devices list
    docker compose run --rm alvasta-pro-cli devices approve <requestId>
    ```
  ...
  <Accordion title="Gateway target shows ws://172.x.x.x or pairing errors from Docker CLI">
    ...
    ```bash
    docker compose run --rm alvasta-pro-cli config set --batch-json '[{"path":"gateway.mode","value":"local"},{"path":"gateway.bind","value":"lan"}]'
    docker compose run --rm alvasta-pro-cli devices list --url ws://127.0.0.1:18789
    ```
...
- [Updating](/install/updating) — keeping Alvasta Pro up to date
