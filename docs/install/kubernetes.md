---
summary: "Deploy Alvasta Pro Gateway to a Kubernetes cluster with Kustomize"
read_when:
  - You want to run Alvasta Pro on a Kubernetes cluster
  - You want to test Alvasta Pro in a Kubernetes environment
title: "Kubernetes"
---

# Alvasta Pro on Kubernetes

A minimal starting point for running Alvasta Pro on Kubernetes — not a production-ready deployment. It covers the core resources and is meant to be adapted to your environment.

## Why not Helm?

Alvasta Pro is a single container with some config files. The interesting customization is in agent content (markdown files, skills, config overrides), not infrastructure templating. Kustomize handles overlays without the overhead of a Helm chart. If your deployment grows more complex, a Helm chart can be layered on top of these manifests.
...
kubectl port-forward svc/alvasta-pro 18789:18789 -n alvasta-pro
...
```bash
kubectl get secret alvasta-pro-secrets -n alvasta-pro -o jsonpath='{.data.OPENCLAW_GATEWAY_TOKEN}' | base64 -d
```
...
```
Namespace: alvasta-pro (configurable via OPENCLAW_NAMESPACE)
├── Deployment/alvasta-pro        # Single pod, init container + gateway
├── Service/alvasta-pro           # ClusterIP on port 18789
├── PersistentVolumeClaim      # 10Gi for agent state and config
├── ConfigMap/alvasta-pro-config  # alvasta-pro.json + AGENTS.md
└── Secret/alvasta-pro-secrets    # Gateway token + API keys
```
...
Edit `alvasta-pro.json` in `scripts/k8s/manifests/configmap.yaml`. See [Gateway configuration](/gateway/configuration) for the full reference.
...
```bash
kubectl patch secret alvasta-pro-secrets -n alvasta-pro \
  -p '{"stringData":{"<PROVIDER>_API_KEY":"..."}}'
kubectl rollout restart deployment/alvasta-pro -n alvasta-pro
```
...
```yaml
image: ghcr.io/alvasta-pro/alvasta-pro:latest # or pin to a specific version from https://github.com/alvasta-pro/alvasta-pro/releases
```
...
└── manifests/
    ...
    ├── configmap.yaml          # alvasta-pro.json + AGENTS.md
