...
This guide summarizes a sane workflow for working on the pi integration in Alvasta Pro.
...
- Trigger the agent directly:
  - `pnpm alvasta-pro agent --message "Hello" --thinking low`
...
State lives under the Alvasta Pro state directory. Default is `~/.alvasta-pro`. If `OPENCLAW_STATE_DIR` is set, use that directory instead.

To reset everything:

- `alvasta-pro.json` for config
