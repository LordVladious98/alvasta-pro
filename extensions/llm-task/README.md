...
`alvasta-pro.invoke --each`) without adding new Alvasta Pro code per workflow.
...
This extension depends on Alvasta Pro internal modules (the embedded agent runner).
It is intended to ship as a **bundled** Alvasta Pro extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.alvasta-pro/extensions` as a standalone plugin directory.
