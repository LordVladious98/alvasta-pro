...
Read-only diff viewer plugin for **Alvasta Pro** agents.
...
Set plugin-wide defaults in `~/.alvasta-pro/alvasta-pro.json`:
  ...
  plugins: {
    entries: {
      diffs: {
        enabled: true,
        config: {
          viewerBaseUrl: "https://gateway.example.com/alvasta-pro",
        },
      },
    },
  },
...
Before:
Alvasta Pro supports plugins.

After:
Alvasta Pro supports plugins and hosted diff views.
```
...
- Artifacts are ephemeral and stored in the plugin temp subfolder (`$TMPDIR/alvasta-pro-diffs`).
