...
The bundled ACPX extension is a thin Alvasta Pro wrapper around the published `acpx` package. Keep reusable ACP runtime logic in `alvasta-pro/acpx`, not in this extension.
...
Use this flow when Alvasta Pro needs unreleased ACPX changes before the ACPX version is published.

1. Make the ACPX code change in the `alvasta-pro/acpx` repo first.
2. In Alvasta Pro, temporarily point `extensions/acpx/package.json` at the ACPX GitHub commit you need.
...
6. Rebuild Alvasta Pro and restart the gateway before doing live ACP validation.
...
- Do not rely on a globally installed `acpx` binary for Alvasta Pro ACP validation.
...
If a change feels like shared ACP runtime behavior instead of Alvasta Pro-specific glue, move it to `alvasta-pro/acpx` and consume it from here instead of re-implementing it inside `extensions/acpx`.
