...
- Extension production code should import from `alvasta-pro/plugin-sdk/*` and its
...
- Keep plugin metadata accurate in `alvasta-pro.plugin.json` and the package
  `alvasta-pro` block so discovery and setup work without executing plugin code.
...
- Before adding a new provider-local `wrapStreamFn`, `buildReplayPolicy`,
  ...
  whether the same behavior already exists through `alvasta-pro/plugin-sdk/*`.
