...
- Start from `alvasta-pro wiki status` to confirm the vault mode and whether the official Obsidian CLI is available.
- Use `alvasta-pro wiki obsidian status` before shelling out, then prefer the dedicated helpers like `alvasta-pro wiki obsidian search`, `alvasta-pro wiki obsidian open`, `alvasta-pro wiki obsidian command`, and `alvasta-pro wiki obsidian daily`.
