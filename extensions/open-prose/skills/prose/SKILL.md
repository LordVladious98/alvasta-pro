...
metadata: { "alvasta-pro": { "emoji": "🪶", "homepage": "https://www.prose.md" } }
---
...
- **Task tool** in the upstream spec == Alvasta Pro `sessions_spawn`
- **File I/O** == Alvasta Pro `read`/`write`
- **Remote fetch** == Alvasta Pro `web_fetch` (or `exec` with curl when POST is required)
